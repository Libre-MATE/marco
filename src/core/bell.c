...
static void bell_flash_screen(MetaDisplay *display, MetaScreen *screen) {
  ...
  if (screen->flash_window == None) {
     ...
     */
    screen->flash_window =
        XCreateWindow(display->xdisplay, root, 0, 0, width, height, 0, depth,
                      InputOutput, visual,
    ...
    XSetForeground(
        display->xdisplay, gc,
        WhitePixel(display->xdisplay, XScreenNumberOfScreen(screen->xscreen)));
    XFillRectangle(display->xdisplay, screen->flash_window, gc, 0, 0, width,
                   height);
    XSetForeground(
        display->xdisplay, gc,
        BlackPixel(display->xdisplay, XScreenNumberOfScreen(screen->xscreen)));
    XFillRectangle(display->xdisplay, screen->flash_window, gc, 0, 0, width,
                   height);
    XFlush(display->xdisplay);
    XSync(display->xdisplay, False);
...
static void bell_flash_fullscreen(MetaDisplay *display, XkbAnyEvent *xkb_ev) {
  ...
  if (xkb_bell_ev->window != None) {
    ...
    if (screen) bell_flash_screen(display, screen);
  } else {
...
static void bell_flash_window_frame(MetaWindow *window) {
  ...
  g_timeout_add_full(G_PRIORITY_DEFAULT_IDLE, 100, bell_unflash_frame,
                     window->frame, NULL);
}
...
void meta_bell_notify(MetaDisplay *display, XkbAnyEvent *xkb_ev) {
  /* flash something */
  if (meta_prefs_get_visual_bell()) bell_visual_notify(display, xkb_ev);
  ...
  if (meta_prefs_bell_is_audible()) {
    ...
    if (window) {
      int x = -1, y = -1, width = -1, height = -1, screen_width = -1,
          screen_height = -1;
      MetaScreen *screen;
      ...
      ca_proplist_setf(p, CA_PROP_WINDOW_X11_SCREEN, "%i",
      ...
      if (screen_width > 1) {
        ...
        ca_proplist_setf(p, CA_PROP_WINDOW_HPOS, "%i.%03i",
                         ...
                         (int)(1000.0 * x / (screen_width - 1)) % 1000);
      }
      if (screen_height > 1) {
        y += height / 2;
        ...
        ca_proplist_setf(p, CA_PROP_WINDOW_VPOS, "%i.%03i",
                         ...
                         (int)(1000.0 * y / (screen_height - 1)) % 1000);
      }
    }
    ...
    if (res != CA_SUCCESS && res != CA_ERROR_DISABLED) {
      ...
      XkbForceDeviceBell(display->xdisplay, xkb_bell_event->device,
                         xkb_bell_event->bell_class, xkb_bell_event->bell_id,
...
gboolean meta_bell_init(MetaDisplay *display) {
  ...
  if (!XkbQueryExtension(display->xdisplay, &xkb_opcode,
                         &display->xkb_base_event_type, &xkb_base_error_type,
                         NULL, NULL)) {
    ...
    XkbSelectEvents(display->xdisplay, XkbUseCoreKbd, XkbBellNotifyMask,
                    XkbBellNotifyMask);
    XkbChangeEnabledControls(display->xdisplay, XkbUseCoreKbd,
                             XkbAudibleBellMask, 0);
    if (visual_bell_auto_reset) {
      XkbSetAutoResetControls(display->xdisplay, XkbAudibleBellMask, &mask,
                              &mask);
    }
    return TRUE;
  }
...
void meta_bell_shutdown(MetaDisplay *display) {
  ...
  XkbChangeEnabledControls(display->xdisplay, XkbUseCoreKbd, XkbAudibleBellMask,
                           XkbAudibleBellMask);
#endif
}
