...
typedef void (*MetaKeyHandlerFunc)(MetaDisplay *display, MetaScreen *screen,
                                   MetaWindow *window, XEvent *event,
                                   MetaKeyBinding *binding);

/* Prototypes for handlers */
#define keybind(name, handler, param, flags)                    \
  static void handler(MetaDisplay *display, MetaScreen *screen, \
                      MetaWindow *window, XEvent *event,        \
                      MetaKeyBinding *binding);
...
static gboolean process_tab_grab(MetaDisplay *display, MetaScreen *screen,
                                 XEvent *event, KeySym keysym);
...
#define keybind(name, handler, param, flags) {#name, handler, param, flags},
...
static void reload_keymap(MetaDisplay *display) {
  if (display->keymap) meta_XFree(display->keymap);

  display->keymap =
      XGetKeyboardMapping(display->xdisplay, display->min_keycode,
                          display->max_keycode - display->min_keycode + 1,
...
static void reload_modmap(MetaDisplay *display) {
  ...
  if (display->modmap) XFreeModifiermap(display->modmap);
  ...
  while (i < map_size) {
    ...
    if (keycode >= display->min_keycode && keycode <= display->max_keycode) {
      int j = 0;
      KeySym *syms = display->keymap + (keycode - display->min_keycode) *
                                           display->keysyms_per_keycode;
  ...
  display->ignored_modifier_mask =
      (display->num_lock_mask | display->scroll_lock_mask | LockMask);
  ...
  meta_topic(META_DEBUG_KEYBINDINGS,
             ...
             display->scroll_lock_mask, display->hyper_mask,
             display->super_mask, display->meta_mask);
}

static void reload_keycodes(MetaDisplay *display) {
  meta_topic(META_DEBUG_KEYBINDINGS, "Reloading keycodes for binding tables\n");
  ...
  if (display->key_bindings) {
    ...
    while (i < display->n_key_bindings) {
      ...
      meta_topic(META_DEBUG_KEYBINDINGS,
                 ...
                 display->key_bindings[i].modifiers,
                 display->key_bindings[i].mask, display->key_bindings[i].name);
...
static void rebuild_binding_table(MetaDisplay *display,
                                  ...
                                  int *n_bindings_p, const MetaKeyPref *prefs,
                                  int n_prefs) {
  int n_bindings;
  int src, dest;
  ...
  *bindings_p = g_new0(MetaKeyBinding, n_bindings);
  ...
  while (src < n_prefs) {
    ...
    while (tmp) {
      ...
      if (combo && (combo->keysym != None || combo->keycode != 0)) {
        const MetaKeyHandler *handler =
            find_handler(key_handlers, prefs[src].name);
        ...
        if (prefs[src].add_shift &&
          ...
          (*bindings_p)[dest].keysym = combo->keysym;
          (*bindings_p)[dest].keycode = combo->keycode;
          (*bindings_p)[dest].modifiers =
              combo->modifiers | META_VIRTUAL_SHIFT_MASK;
          (*bindings_p)[dest].mask = 0;
  ...
  *n_bindings_p = dest;
  ...
  meta_topic(META_DEBUG_KEYBINDINGS, " %d bindings in table\n", *n_bindings_p);
}

static void rebuild_key_binding_table(MetaDisplay *display) {
  ...
  rebuild_binding_table(display, &display->key_bindings,
                        &display->n_key_bindings, prefs, n_prefs);
}
...
void meta_display_process_mapping_event(MetaDisplay *display, XEvent *event) {
  ...
  if (event->type == display->xkb_base_event_type) {
    ...
    meta_topic(META_DEBUG_KEYBINDINGS,
               "Received MappingModifier event, will reload modmap and redo "
               "keybindings\n");

    modmap_changed = TRUE;
  } else if (event->xmapping.request == MappingKeyboard) {
    meta_topic(META_DEBUG_KEYBINDINGS,
               "Received MappingKeyboard event, will reload keycodes and redo "
               "keybindings\n");
  ...
  if (keymap_changed || modmap_changed) {
    if (keymap_changed) reload_keymap(display);
    ...
    if (keymap_changed) reload_keycodes(display);
...
void meta_display_init_keys(MetaDisplay *display) {
  ...
  XDisplayKeycodes(display->xdisplay, &display->min_keycode,
                   &display->max_keycode);

  meta_topic(META_DEBUG_KEYBINDINGS, "Display has keycode range %d to %d\n",
...
static void meta_change_keygrab(MetaDisplay *display, Window xwindow,
                                gboolean grab, int keysym, unsigned int keycode,
                                int modmask) {
  ...
  meta_topic(META_DEBUG_KEYBINDINGS,
             ...
             grab ? "Grabbing" : "Ungrabbing", keysym_name(keysym), keycode,
             modmask, xwindow);
  ...
  while (ignored_mask <= display->ignored_modifier_mask) {
    ...
    if (grab)
      XGrabKey(display->xdisplay, keycode, modmask | ignored_mask, xwindow,
               True, GrabModeAsync, GrabModeSync);
    else
      XUngrabKey(display->xdisplay, keycode, modmask | ignored_mask, xwindow);
    ...
    if (meta_is_debugging()) {
      ...
      if (grab && result != Success) {
        if (result == BadAccess)
          meta_warning(_("Some other program is already using the key %s with "
                         "modifiers %x as a binding\n"),
...
static void meta_grab_key(MetaDisplay *display, Window xwindow, int keysym,
                          unsigned int keycode, int modmask) {
  meta_change_keygrab(display, xwindow, TRUE, keysym, keycode, modmask);
}

static void grab_keys(MetaKeyBinding *bindings, int n_bindings,
  ...
  while (i < n_bindings) {
    if (!!binding_per_window ==
      ...
      meta_grab_key(display, xwindow, bindings[i].keysym, bindings[i].keycode,
                    bindings[i].mask);
    }
...
static void ungrab_all_keys(MetaDisplay *display, Window xwindow) {
  ...
  XUngrabKey(display->xdisplay, AnyKey, AnyModifier, xwindow);
...
void meta_screen_grab_keys(MetaScreen *screen) {
  ...
  if (screen->keys_grabbed) return;

  grab_keys(screen->display->key_bindings, screen->display->n_key_bindings,
            screen->display, screen->xroot, FALSE);
...
void meta_window_grab_keys(MetaWindow *window) {
  if (window->all_keys_grabbed) return;

  if (window->type == META_WINDOW_DOCK) {
    if (window->keys_grabbed) ungrab_all_keys(window->display, window->xwindow);
  ...
  if (window->keys_grabbed) {
    ...
    else if (window->frame == NULL && window->grab_on_frame)
      ; /* continue to regrab on client window */
    else
      return; /* already all good */
  }

  grab_keys(window->display->key_bindings, window->display->n_key_bindings,
            window->display,
            window->frame ? window->frame->xwindow : window->xwindow, TRUE);
...
void meta_window_ungrab_keys(MetaWindow *window) {
  if (window->keys_grabbed) {
    if (window->grab_on_frame && window->frame != NULL)
...
static gboolean grab_keyboard(MetaDisplay *display, Window xwindow,
                              guint32 timestamp) {
  int result;
  int grab_status;
  ...
  grab_status = XGrabKeyboard(display->xdisplay, xwindow, True, GrabModeAsync,
                              GrabModeAsync, timestamp);

  if (grab_status != GrabSuccess) {
    meta_error_trap_pop_with_return(display, TRUE);
    meta_topic(META_DEBUG_KEYBINDINGS,
               "XGrabKeyboard() returned failure status %s time %u\n",
               grab_status_to_string(grab_status), timestamp);
    return FALSE;
  } else {
...
static void ungrab_keyboard(MetaDisplay *display, guint32 timestamp) {
  ...
  meta_topic(META_DEBUG_KEYBINDINGS, "Ungrabbing keyboard with timestamp %u\n",
             timestamp);
...
gboolean meta_screen_grab_all_keys(MetaScreen *screen, guint32 timestamp) {
  ...
  if (screen->all_keys_grabbed) return FALSE;

  if (screen->keys_grabbed) meta_screen_ungrab_keys(screen);

  meta_topic(META_DEBUG_KEYBINDINGS, "Grabbing all keys on RootWindow\n");
...
gboolean meta_window_grab_all_keys(MetaWindow *window, guint32 timestamp) {
  ...
  if (window->all_keys_grabbed) return FALSE;

  if (window->keys_grabbed) meta_window_ungrab_keys(window);
...
static gboolean is_specific_modifier(MetaDisplay *display, unsigned int keycode,
                                     unsigned int mask) {
  int i;
  int end;
  gboolean retval = FALSE;
  ...
  meta_topic(META_DEBUG_KEYBINDINGS, "Modifier has index %d\n", mod_index);
...
static unsigned int get_primary_modifier(MetaDisplay *display,
   ...
   * these.
   */
  unsigned int masks[] = {Mod5Mask, Mod4Mask,    Mod3Mask,  Mod2Mask,
                          Mod1Mask, ControlMask, ShiftMask, LockMask};

  int i;

  i = 0;
  while (i < (int)G_N_ELEMENTS(masks)) {
    if (entire_binding_mask & masks[i]) return masks[i];
    ++i;
  }
...
static gboolean primary_modifier_still_pressed(
    MetaDisplay *display, unsigned int entire_binding_mask) {
  unsigned int primary_modifier;
  ...
  XQueryPointer(display->xdisplay, random_xwindow, /* some random window */
...
static gboolean process_event(MetaKeyBinding *bindings, int n_bindings,
  ...
  for (i = 0; i < n_bindings; i++) {
    ...
    if ((!on_window && handler->flags & BINDING_PER_WINDOW) ||
        event->type != KeyPress || bindings[i].keycode != event->xkey.keycode ||
    ...
    meta_topic(META_DEBUG_KEYBINDINGS,
               ...
               bindings[i].keycode, bindings[i].mask, event->xkey.keycode,
               event->xkey.state);

    if (handler == NULL)
      meta_bug("Binding %s has no handler\n", bindings[i].name);
    else
      meta_topic(META_DEBUG_KEYBINDINGS, "Running handler for %s\n",
                 bindings[i].name);
    ...
    (*handler->func)(
        display, screen,
        bindings[i].handler->flags & BINDING_PER_WINDOW ? window : NULL, event,
        &bindings[i]);
    return TRUE;
  }
...
void meta_display_process_key_event(MetaDisplay *display, MetaWindow *window,
  ...
  XAllowEvents(display->xdisplay,
               ...
               event->xkey.time);
  if (all_bindings_disabled) return;
  ...
  if (screen == NULL)
    screen = meta_display_screen_for_xwindow(display, event->xany.window);

  if (screen == NULL) return; /* event window is destroyed */
  ...
  meta_topic(META_DEBUG_KEYBINDINGS,
             ...
             event->type == KeyPress ? "press" : "release", str ? str : "none",
             event->xkey.state, window ? window->desc : "(no window)");

  keep_grab = TRUE;
  all_keys_grabbed =
      window ? window->all_keys_grabbed : screen->all_keys_grabbed;
  if (all_keys_grabbed) {
    ...
    if (window ? (window == display->grab_window)
      ...
      switch (display->grab_op) {
        ...
        case META_GRAB_OP_RESIZING_E:
          ...
          keep_grab = process_mouse_move_resize_grab(display, screen, window,
                                                     event, keysym);
          break;
        ...
        case META_GRAB_OP_KEYBOARD_RESIZING_NW:
          ...
          g_assert(window != NULL);
          keep_grab = process_keyboard_resize_grab(display, screen, window,
                                                   event, keysym);
          break;
        ...
        case META_GRAB_OP_KEYBOARD_WORKSPACE_MOVING:
          ...
          keep_grab = process_workspace_switch_grab(display, screen, window,
                                                    event, keysym);
          break;
        default:
          break;
  ...
  process_event(display->key_bindings, display->n_key_bindings, display, screen,
                window, event, !all_keys_grabbed && window);
}
...
static gboolean process_mouse_move_resize_grab(MetaDisplay *display,
  ...
  if (event->type == KeyRelease) return TRUE;
  ...
  if (keysym == XK_Escape) {
    ...
    if (window->shaken_loose)
      ...
      meta_window_move_resize(display->grab_window, TRUE,
...
static gboolean process_keyboard_move_grab(MetaDisplay *display,
  ...
  if (is_modifier(display, event->xkey.keycode)) return TRUE;
...
#define NORMAL_INCREMENT 10
  ...
  if (handled) {
    ...
    meta_window_edge_resistance_for_move(window, old_rect.x, old_rect.y, &x, &y,
                                         NULL, smart_snap, TRUE);
...
#define NORMAL_INCREMENT 10
  ...
  if (handled) {
    ...
    meta_window_edge_resistance_for_resize(window, old_rect.width,
                                           old_rect.height, &width, &height,
                                           gravity, NULL, smart_snap, TRUE);

    if (display->grab_wireframe_active) {
      MetaRectangle new_position;
      meta_rectangle_resize_with_gravity(&display->grab_wireframe_rect,
                                         &new_position, gravity, width, height);
      meta_window_update_wireframe(window, new_position.x, new_position.y,
...
static gboolean end_keyboard_grab(MetaDisplay *display, unsigned int keycode) {
  ...
  if (display->xkb_base_event_type > 0) {
    ...
    if (!(primary_modifier & state.mods)) return TRUE;
  } else
#endif
  {
...
static gboolean process_tab_grab(MetaDisplay *display, MetaScreen *screen,
  ...
  if (screen != display->grab_screen) return FALSE;
  ...
  if (event->type == KeyRelease &&
    ...
    target_window = meta_display_lookup_x_window(display, (Window)key);
  ...
  action = display_get_keybinding_action(display, keysym, event->xkey.keycode,
                                         display->grab_mask);
  ...
  if (event->xkey.keycode == ARROW_RIGHT ||
      event->xkey.keycode == ARROW_RIGHT_PAD) {
    key_used = TRUE;
    direction = RIGHT;
  } else if (event->xkey.keycode == ARROW_LEFT ||
             event->xkey.keycode == ARROW_LEFT_PAD) {
    key_used = TRUE;
    direction = LEFT;
  } else if (event->xkey.keycode == ARROW_UP ||
             event->xkey.keycode == ARROW_UP_PAD) {
    key_used = TRUE;
    direction = UP;
  } else if (event->xkey.keycode == ARROW_DOWN ||
             event->xkey.keycode == ARROW_DOWN_PAD) {
    key_used = TRUE;
    direction = DOWN;
  }
...
static void handle_switch_to_workspace(MetaDisplay *display, MetaScreen *screen,
                                       MetaWindow *event_window, XEvent *event,
                                       MetaKeyBinding *binding) {
  ...
  if (which == META_MOTION_PREV) {
    ...
    if (workspace) meta_workspace_activate(workspace, event->xkey.time);
    return;
  }
  ...
  if (which < 0) {
     ...
     */
    handle_workspace_switch_or_move(display, screen, event_window, event,
                                    binding, FALSE);
    return;
  }
...
static void error_on_command(int command_index, const char *command,
                             const char *message, const char *screen_name,
                             guint32 timestamp) {
  if (command_index < 0)
  ...
  */
  ...
  if (command && strcmp(command, "") != 0) {
    ...
    meta_show_dialog("--error", text, NULL, screen_name, NULL, NULL, 0, NULL,
                     NULL);

    g_free(text);

  } else {
    meta_show_dialog("--error", message, NULL, screen_name, NULL, NULL, 0, NULL,
                     NULL);
  }
}
...
static gboolean meta_spawn_command_line_async_on_screen(
    const gchar *command_line, MetaScreen *screen, GError **error) {
  gboolean retval;
  gchar **argv = NULL;
  ...
  retval =
      g_spawn_async(NULL, argv, NULL, G_SPAWN_SEARCH_PATH,
                    set_display_setup_func, screen->screen_name, NULL, error);
  g_strfreev(argv);

  return retval;
}

static void handle_run_command(MetaDisplay *display, MetaScreen *screen,
                               MetaWindow *window, XEvent *event,
                               MetaKeyBinding *binding) {
  gint which = binding->handler->data;
  const char *command;
  ...
  if (!meta_spawn_command_line_async_on_screen(command, screen, &err)) {
    error_on_command(which, command, err->message, screen->screen_name,
                     event->xkey.time);
...
static void handle_maximize_vertically(MetaDisplay *display, MetaScreen *screen,
                                       MetaWindow *window, XEvent *event,
                                       MetaKeyBinding *binding) {
...
static void handle_maximize_horizontally(MetaDisplay *display,
...
static void handle_move_to_corner_backend(MetaDisplay *display,
                                          MetaScreen *screen,
                                          MetaWindow *window, gboolean xchange,
                                          gboolean ychange, gboolean to_right,
                                          gboolean to_bottom) {
  ...
  meta_window_move_resize(window, TRUE, new_x, new_y, window->rect.width,
                          window->rect.height);
}

static void handle_move_to_corner_nw(MetaDisplay *display, MetaScreen *screen,
                                     MetaWindow *window, XEvent *event,
                                     MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, TRUE, TRUE, FALSE,
                                FALSE);
}

static void handle_move_to_corner_ne(MetaDisplay *display, MetaScreen *screen,
                                     MetaWindow *window, XEvent *event,
                                     MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, TRUE, TRUE, TRUE,
                                FALSE);
}

static void handle_move_to_corner_sw(MetaDisplay *display, MetaScreen *screen,
                                     MetaWindow *window, XEvent *event,
                                     MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, TRUE, TRUE, FALSE,
                                TRUE);
...
static void handle_move_to_corner_se(MetaDisplay *display, MetaScreen *screen,
                                     MetaWindow *window, XEvent *event,
                                     MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, TRUE, TRUE, TRUE,
                                TRUE);
...
static void handle_move_to_side_n(MetaDisplay *display, MetaScreen *screen,
                                  MetaWindow *window, XEvent *event,
                                  MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, FALSE, TRUE, FALSE,
                                FALSE);
}

static void handle_move_to_side_s(MetaDisplay *display, MetaScreen *screen,
                                  MetaWindow *window, XEvent *event,
                                  MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, FALSE, TRUE, FALSE,
                                TRUE);
...
static void handle_move_to_side_e(MetaDisplay *display, MetaScreen *screen,
                                  MetaWindow *window, XEvent *event,
                                  MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, TRUE, FALSE, TRUE,
                                FALSE);
}

static void handle_move_to_side_w(MetaDisplay *display, MetaScreen *screen,
                                  MetaWindow *window, XEvent *event,
                                  MetaKeyBinding *binding) {
  handle_move_to_corner_backend(display, screen, window, TRUE, FALSE, FALSE,
                                FALSE);
}

static void handle_move_to_center(MetaDisplay *display, MetaScreen *screen,
                                  MetaWindow *window, XEvent *event,
                                  MetaKeyBinding *binding) {
...
static void handle_move_to_monitor(MetaDisplay *display, MetaScreen *screen,
                                   MetaWindow *window, XEvent *event,
                                   MetaKeyBinding *binding) {
  ...
  current = meta_screen_get_xinerama_for_window(screen, window);
  neighbour = meta_screen_get_xinerama_neighbor(screen, current->number,
                                                move_direction);

  if (neighbour == NULL || current->number == neighbour->number) return;

  meta_window_move_to_monitor(window, current, neighbour);
}
...
static gboolean process_workspace_switch_grab(MetaDisplay *display,
  ...
  if (event->type == KeyRelease &&
    ...
    meta_warning(
        "target_workspace != active_workspace.  Some other event must have "
        "occurred.\n");
  ...
  if (workspace) {
    ...
    if (target_workspace) {
      ...
      if (display->grab_op == META_GRAB_OP_KEYBOARD_WORKSPACE_SWITCHING)
        ...
        meta_workspace_activate_with_focus(target_workspace, window,
                                           event->xkey.time);
      }
  ...
  /* end grab */
  meta_topic(META_DEBUG_KEYBINDINGS,
             "Ending workspace tabbing & focusing default window; "
             "uninteresting key pressed\n");
  workspace =
      (MetaWorkspace *)meta_ui_tab_popup_get_selected(screen->tab_popup);
...
static void handle_show_desktop(MetaDisplay *display, MetaScreen *screen,
                                MetaWindow *window, XEvent *event,
                                MetaKeyBinding *binding) {
...
static void handle_panel(MetaDisplay *display, MetaScreen *screen,
                         MetaWindow *window, XEvent *event,
                         MetaKeyBinding *binding) {
  ...
  XSendEvent(display->xdisplay, screen->xroot, False, StructureNotifyMask,
             (XEvent *)&ev);

  meta_error_trap_pop(display, FALSE);
}

static void handle_activate_window_menu(MetaDisplay *display,
                                        MetaScreen *screen,
                                        MetaWindow *event_window, XEvent *event,
                                        MetaKeyBinding *binding) {
  if (display->focus_window) {
    ...
    meta_window_show_menu(display->focus_window, x, y, 0, event->xkey.time);
  }
}
...
static void do_choose_window(MetaDisplay *display, MetaScreen *screen,
                             MetaWindow *event_window, XEvent *event,
                             MetaKeyBinding *binding, gboolean backward,
                             gboolean show_popup) {
  MetaTabList type = binding->handler->data;
  MetaWindow *initial_selection;

  meta_topic(META_DEBUG_KEYBINDINGS, "Tab list = %u show_popup = %d\n", type,
             show_popup);

  /* reverse direction if shift is down */
  if (event->xkey.state & ShiftMask) backward = !backward;

  initial_selection = meta_display_get_tab_next(
      display, type, screen, screen->active_workspace, NULL, backward);

  /* Note that focus_window may not be in the tab chain, but it's OK */
  if (initial_selection == NULL)
    initial_selection = meta_display_get_tab_current(display, type, screen,
                                                     screen->active_workspace);

  meta_topic(META_DEBUG_KEYBINDINGS, "Initially selecting window %s\n",
  ...
  if (initial_selection != NULL) {
    if (binding->mask == 0) {
    ...
    } else if (meta_display_begin_grab_op(
                   display, screen, NULL,
                   show_popup ? tab_op_from_tab_type(type)
                              : cycle_op_from_tab_type(type),
                   FALSE, FALSE, 0, binding->mask, event->xkey.time, 0, 0)) {
      if (!primary_modifier_still_pressed(display, binding->mask)) {
        ...
        meta_window_activate(initial_selection, event->xkey.time);
        if (!initial_selection->on_all_workspaces)
          meta_workspace_activate(initial_selection->workspace,
                                  event->xkey.time);
      } else {
        meta_ui_tab_popup_select(screen->tab_popup,
        ...
        if (show_popup)
          ...
          initial_selection->tab_unminimized = initial_selection->minimized;
...
static void handle_cycle(MetaDisplay *display, MetaScreen *screen,
                         MetaWindow *event_window, XEvent *event,
                         MetaKeyBinding *binding) {
  gint backwards = binding->handler->flags & BINDING_IS_REVERSED;
...
static void handle_toggle_fullscreen(MetaDisplay *display, MetaScreen *screen,
                                     MetaWindow *window, XEvent *event,
                                     MetaKeyBinding *binding) {
...
static void handle_toggle_above(MetaDisplay *display, MetaScreen *screen,
                                MetaWindow *window, XEvent *event,
                                MetaKeyBinding *binding) {
...
static void handle_toggle_tiled(MetaDisplay *display, MetaScreen *screen,
                                MetaWindow *window, XEvent *event,
                                MetaKeyBinding *binding) {
  ...
  if (meta_prefs_get_allow_tile_cycling()) {
    if (window->tile_mode != mode) {
      ...
      /* Cycle through the different tile sizes: 1/2 -> 1/3 -> 1/4 -> 1/1 -> 3/4
       * -> 2/3 -> Untiled */
  ...
  if (mode == window->tile_mode && next_cycle == META_TILE_CYCLE_NONE) {
     ...
     */
    if (!META_WINDOW_MAXIMIZED(window)) window->saved_maximize = FALSE;
...
static void handle_toggle_maximized(MetaDisplay *display, MetaScreen *screen,
                                    MetaWindow *window, XEvent *event,
                                    MetaKeyBinding *binding) {
...
static void handle_maximize(MetaDisplay *display, MetaScreen *screen,
                            MetaWindow *window, XEvent *event,
                            MetaKeyBinding *binding) {
...
static void handle_unmaximize(MetaDisplay *display, MetaScreen *screen,
                              MetaWindow *window, XEvent *event,
                              MetaKeyBinding *binding) {
...
static void handle_toggle_shaded(MetaDisplay *display, MetaScreen *screen,
                                 MetaWindow *window, XEvent *event,
                                 MetaKeyBinding *binding) {
  if (window->shaded)
...
static void handle_close(MetaDisplay *display, MetaScreen *screen,
                         MetaWindow *window, XEvent *event,
                         MetaKeyBinding *binding) {
  if (window->has_close_func) meta_window_delete(window, event->xkey.time);
}

static void handle_minimize(MetaDisplay *display, MetaScreen *screen,
                            MetaWindow *window, XEvent *event,
                            MetaKeyBinding *binding) {
  if (window->has_minimize_func) meta_window_minimize(window);
}

static void handle_begin_move(MetaDisplay *display, MetaScreen *screen,
                              MetaWindow *window, XEvent *event,
                              MetaKeyBinding *binding) {
...
static void handle_begin_resize(MetaDisplay *display, MetaScreen *screen,
                                MetaWindow *window, XEvent *event,
                                MetaKeyBinding *binding) {
  if (window->has_resize_func) {
    meta_window_begin_grab_op(window, META_GRAB_OP_KEYBOARD_RESIZING_UNKNOWN,
                              FALSE, event->xkey.time);
  }
}

static void handle_toggle_on_all_workspaces(MetaDisplay *display,
                                            MetaScreen *screen,
                                            MetaWindow *window, XEvent *event,
                                            MetaKeyBinding *binding) {
...
static void handle_move_to_workspace(MetaDisplay *display, MetaScreen *screen,
                                     MetaWindow *window, XEvent *event,
                                     MetaKeyBinding *binding) {
  gint which = binding->handler->data;
  MetaWorkspace *workspace;
  ...
  if (window->always_sticky) return;

  if (which < 0) {
    handle_workspace_switch_or_move(display, screen, window, event, binding,
                                    TRUE);
    return;
  }
...
static void handle_raise_or_lower(MetaDisplay *display, MetaScreen *screen,
                                  MetaWindow *window, XEvent *event,
                                  MetaKeyBinding *binding) {
...
static void handle_raise(MetaDisplay *display, MetaScreen *screen,
                         MetaWindow *window, XEvent *event,
                         MetaKeyBinding *binding) {
  meta_window_raise(window);
}

static void handle_lower(MetaDisplay *display, MetaScreen *screen,
                         MetaWindow *window, XEvent *event,
                         MetaKeyBinding *binding) {
  meta_window_lower(window);
}
...
static void handle_workspace_switch_or_move(MetaDisplay *display,
  ...
  if (meta_screen_get_n_workspaces(screen) == 1) return;
  ...
  if (meta_display_begin_grab_op(display, screen, is_move ? window : NULL,
                                 grab_op, FALSE, FALSE, 0, grab_mask,
                                 event->xkey.time, 0, 0)) {
    MetaWorkspace *next;
    ...
    if (is_move) {
      ...
      meta_workspace_activate_with_focus(next, window, event->xkey.time);
    } else
...
static void handle_set_spew_mark(MetaDisplay *display, MetaScreen *screen,
                                 MetaWindow *window, XEvent *event,
                                 MetaKeyBinding *binding) {
...
void meta_set_keybindings_disabled(gboolean setting) {
  ...
  meta_topic(META_DEBUG_KEYBINDINGS, "Keybindings %s\n",
             all_bindings_disabled ? "disabled" : "enabled");
}

static void handle_run_terminal(MetaDisplay *display, MetaScreen *screen,
                                MetaWindow *window, XEvent *event,
                                MetaKeyBinding *binding) {
  const char *command;
  GError *err;
...
static gboolean handle_rename_workspace_callback(GIOChannel *ioc,
                                                 GIOCondition cond,
                                                 gpointer data) {
  meta_topic(META_DEBUG_KEYBINDINGS,
             "handle_rename_workspace_callback: called.\n");
  gint *workspace_index = data;

  if (!already_displaying_rename_workspace) {
    meta_topic(META_DEBUG_KEYBINDINGS,
               "handle_rename_workspace_callback: done, "
  ...
  meta_topic(META_DEBUG_KEYBINDINGS,
             "handle_rename_workspace_callback: workspace_index=%d\n",
             *workspace_index);
  if (*workspace_index < 0 || *workspace_index > 36) {
    meta_warning(
        "handle_rename_workspace_callback: invalid workspace_index=%d\n",
        *workspace_index);
    g_free(workspace_index);
    already_displaying_rename_workspace = FALSE;
    return FALSE;
  ...
  ret = g_io_channel_read_chars(ioc, buf, RENAME_WORKSPACE_BUFSIZE, &buf_len,
                                NULL);

  if (buf_len <= 0 || ret != G_IO_STATUS_NORMAL) {
    meta_topic(
        META_DEBUG_KEYBINDINGS,
        "handle_rename_workspace_callback: error getting the new name.\n");
    g_free(workspace_index);
    already_displaying_rename_workspace = FALSE;
    return FALSE;
  }
  ...
  if (!g_utf8_validate(buf, -1, NULL)) {
    meta_topic(
        META_DEBUG_KEYBINDINGS,
        "handle_rename_workspace_callback: the string is not utf-8: %s\n", buf);
    g_free(workspace_index);
    already_displaying_rename_workspace = FALSE;
    return FALSE;
  }
  ...
  if (clean_buf_len <= 1) {
    meta_topic(META_DEBUG_KEYBINDINGS,
               "handle_rename_workspace_callback: invalid name.\n");
    g_free(workspace_index);
    already_displaying_rename_workspace = FALSE;
    return FALSE;
  }
...
static void handle_rename_workspace(MetaDisplay *display, MetaScreen *screen,
                                    MetaWindow *window, XEvent *event,
                                    MetaKeyBinding *binding) {
  ...
  if (already_displaying_rename_workspace) {
    meta_topic(META_DEBUG_KEYBINDINGS,
               "handle_rename_workspace: return, "
  ...
  *workspace_index = meta_workspace_index(screen->active_workspace);
  meta_topic(META_DEBUG_KEYBINDINGS,
             "handle_rename_workspace: workspace_index=%d\n", *workspace_index);

  entry_text = meta_prefs_get_workspace_name(*workspace_index);
  dialog_pid = meta_show_entry_dialog(
      window_content, workspace_index, entry_text, screen->screen_name, _("OK"),
      _("Cancel"), 0, handle_rename_workspace_callback);
