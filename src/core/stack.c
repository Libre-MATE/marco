...
#define WINDOW_HAS_TRANSIENT_TYPE(w)                                       \
  (w->type == META_WINDOW_DIALOG || w->type == META_WINDOW_MODAL_DIALOG || \
   w->type == META_WINDOW_TOOLBAR || w->type == META_WINDOW_MENU ||        \
   w->type == META_WINDOW_UTILITY)

#define WINDOW_TRANSIENT_FOR_WHOLE_GROUP(w)                             \
  ((w->xtransient_for == None || w->transient_parent_is_root_window) && \
...
void meta_stack_remove(MetaStack *stack, MetaWindow *window) {
   ...
   */
  meta_window_set_stack_position_no_sync(window, stack->n_positions - 1);
   ...
   */
  stack->removed =
      g_list_prepend(stack->removed, GUINT_TO_POINTER(window->xwindow));
  if (window->frame)
...
void meta_stack_raise(MetaStack *stack, MetaWindow *window) {
  ...
  if (max_stack_position == window->stack_position) return;
...
void meta_stack_lower(MetaStack *stack, MetaWindow *window) {
  ...
  if (min_stack_position == window->stack_position) return;
...
void meta_stack_freeze(MetaStack *stack) { stack->freeze_count += 1; }
...
static MetaStackLayer get_standalone_layer(MetaWindow *window) {
  ...
  switch (window->type) {
    ...
    default:
      meta_window_foreach_transient(window, is_focused_foreach,
                                    &focused_transient);
...
static void compute_layer(MetaWindow *window) {
  ...
  meta_topic(META_DEBUG_STACK,
             ...
             window->layer, window->type, window->has_focus);
}
 ...
 */
static void add_constraint(Constraint **constraints, MetaWindow *above,
                           MetaWindow *below) {
  Constraint *c;
...
static void create_constraints(Constraint **constraints, GList *windows) {
  ...
  while (tmp != NULL) {
    ...
    if (WINDOW_TRANSIENT_FOR_WHOLE_GROUP(w)) {
      ...
      parent = meta_display_lookup_x_window(w->display, w->xtransient_for);

      if (parent && WINDOW_IN_STACK(parent) && parent->screen == w->screen) {
        meta_topic(META_DEBUG_STACK,
...
static void graph_constraints(Constraint **constraints, int n_constraints) {
  ...
  while (i < n_constraints) {
    ...
    while (c != NULL) {
      ...
      while (n != NULL) {
        c->next_nodes = g_slist_prepend(c->next_nodes, n);
...
static void ensure_above(MetaWindow *above, MetaWindow *below) {
  if (WINDOW_HAS_TRANSIENT_TYPE(above) && above->layer < below->layer) {
    meta_topic(META_DEBUG_STACK,
  ...
  meta_topic(META_DEBUG_STACK, "%s above at %d > %s below at %d\n", above->desc,
             above->stack_position, below->desc, below->stack_position);
}
...
static void apply_constraints(Constraint **constraints, int n_constraints) {
  ...
  while (i < n_constraints) {
    ...
    c = constraints[i];
    while (c != NULL) {
      if (!c->has_prev) heads = g_slist_prepend(heads, c);
...
static void stack_do_window_additions(MetaStack *stack) {
  ...
  if (n_added > 0) {
    ...
    meta_topic(META_DEBUG_STACK, "Adding %d windows to sorted list\n", n_added);
...
static void stack_do_relayer(MetaStack *stack) {
  ...
  while (tmp != NULL) {
    ...
    if (w->layer != old_layer) {
      meta_topic(META_DEBUG_STACK, "Window %s moved from layer %u to %u\n",
...
static void stack_do_constrain(MetaStack *stack) {
  ...
  if (!stack->need_constrain) return;

  meta_topic(META_DEBUG_STACK, "Reapplying constraints\n");

  constraints = g_new0(Constraint *, stack->n_positions);
 ...
 */
static void stack_do_resort(MetaStack *stack) {
  if (!stack->need_resort) return;

  meta_topic(META_DEBUG_STACK, "Sorting stack list\n");

  stack->sorted =
      g_list_sort(stack->sorted, (GCompareFunc)compare_window_position);
...
static void raise_window_relative_to_managed_windows(MetaScreen *screen,
  ...
  while (i >= 0) {
    if (children[i] == xwindow) {
       ...
       * to be moved below.
       */
    } else if (meta_display_lookup_x_window(screen->display, children[i]) !=
               NULL) {
...
static void stack_sync_to_server(MetaStack *stack) {
  ...
  /* Bail out if frozen */
  if (stack->freeze_count > 0) return;
  ...
  while (tmp != NULL) {
    ...
    meta_topic(META_DEBUG_STACK, "%u:%d - %s ", w->layer, w->stack_position,
               w->desc);
  ...
  if (stack->last_root_children_stacked == NULL) {
     ...
     */
    meta_topic(META_DEBUG_STACK,
               "Don't know last stack state, restacking everything\n");
    ...
    while (oldp != old_end && newp != new_end) {
      if (*oldp == *newp) {
        /* Stacks are the same here, move on */
        ++oldp;
        last_window = *newp;
        ++newp;
      } else if (meta_display_lookup_x_window(stack->screen->display, *oldp) ==
                 NULL) {
        ...
        if (last_window == None) {
          meta_topic(
              META_DEBUG_STACK,
              "Using window 0x%lx as topmost (but leaving it in-place)\n",
              *newp);
          ...
          XConfigureWindow(stack->screen->display->xdisplay, *newp,
                           CWSibling | CWStackMode, &changes);
        }
    ...
    if (newp != new_end) {
       ...
       * with respect to it.
       */
      if (newp != new_stack) --newp;
      XRestackWindows(stack->screen->display->xdisplay, (Window *)newp,
                      new_end - newp);
    }
  }
  ...
  XChangeProperty(stack->screen->display->xdisplay, stack->screen->xroot,
                  stack->screen->display->atom__NET_CLIENT_LIST, XA_WINDOW, 32,
                  PropModeReplace, (unsigned char *)stack->windows->data,
                  stack->windows->len);
  XChangeProperty(stack->screen->display->xdisplay, stack->screen->xroot,
                  stack->screen->display->atom__NET_CLIENT_LIST_STACKING,
                  XA_WINDOW, 32, PropModeReplace,
                  (unsigned char *)stacked->data, stacked->len);
...
MetaWindow *meta_stack_get_above(MetaStack *stack, MetaWindow *window,
  ...
  if (link->prev == NULL) return NULL;

  above = link->prev->data;

  if (only_within_layer && above->layer != window->layer)
    return NULL;
  else
    return above;
}

MetaWindow *meta_stack_get_below(MetaStack *stack, MetaWindow *window,
                                 gboolean only_within_layer) {
  GList *link;
  MetaWindow *below;
  ...
  if (only_within_layer && below->layer != window->layer)
    return NULL;
  else
    return below;
}

static gboolean window_contains_point(MetaWindow *window, int root_x,
                                      int root_y) {
  MetaRectangle rect;
...
static MetaWindow *get_default_focus_window(MetaStack *stack,
  ...
  while (link) {
    ...
    if (window && window != not_this_one && (window->unmaps_pending == 0) &&
        !window->minimized && (window->input || window->take_focus) &&
        (workspace == NULL ||
         meta_window_located_on_workspace(window, workspace))) {
      if (topmost_dock == NULL && window->type == META_WINDOW_DOCK)
        topmost_dock = window;

      if (not_this_one != NULL) {
        if (transient_parent == NULL && not_this_one->xtransient_for != None &&
            not_this_one->xtransient_for == window->xwindow &&
            (!must_be_at_point ||
             window_contains_point(window, root_x, root_y)))
          transient_parent = window;

        if (topmost_in_group == NULL && not_this_one_group != NULL &&
      ...
      if (topmost_overall == NULL && window->type != META_WINDOW_DOCK &&
          (!must_be_at_point || window_contains_point(window, root_x, root_y)))
...
MetaWindow *meta_stack_get_default_focus_window_at_point(
    MetaStack *stack, MetaWorkspace *workspace, MetaWindow *not_this_one,
    int root_x, int root_y) {
  return get_default_focus_window(stack, workspace, not_this_one, TRUE, root_x,
                                  root_y);
}
...
MetaWindow *meta_stack_get_default_focus_window(MetaStack *stack,
                                                MetaWorkspace *workspace,
                                                MetaWindow *not_this_one) {
  return get_default_focus_window(stack, workspace, not_this_one, FALSE, 0, 0);
}
...
GList *meta_stack_list_windows(MetaStack *stack, MetaWorkspace *workspace) {
  ...
  while (link) {
    ...
    if (window && (workspace == NULL ||
                   meta_window_located_on_workspace(window, workspace))) {
      workspace_windows = g_list_prepend(workspace_windows, window);
    }
...
int meta_stack_windows_cmp(MetaStack *stack, MetaWindow *window_a,
                           MetaWindow *window_b) {
...
void meta_stack_set_positions(MetaStack *stack, GList *windows) {
  ...
  if (!lists_contain_same_windows(windows, stack->sorted)) {
    meta_warning(
        "This list of windows has somehow changed; not resetting "
...
void meta_window_set_stack_position_no_sync(MetaWindow *window, int position) {
  int low, high, delta;
  GList *tmp;
  ...
  while (tmp != NULL) {
    ...
    if (w->stack_position >= low && w->stack_position <= high)
  ...
  meta_topic(META_DEBUG_STACK, "Window %s had stack_position set to %d\n",
