...
static int set_wm_check_hint(MetaScreen *screen) {
  ...
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_SUPPORTING_WM_CHECK, XA_WINDOW, 32,
                  PropModeReplace, (guchar *)data, 1);

  return Success;
}

static void unset_wm_check_hint(MetaScreen *screen) {
  XDeleteProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_SUPPORTING_WM_CHECK);
}

static int set_supported_hint(MetaScreen *screen) {
  Atom atoms[] = {
...
#undef EWMH_ATOMS_ONLY
  ...
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_SUPPORTED, XA_ATOM, 32,
                  PropModeReplace, (guchar *)atoms, G_N_ELEMENTS(atoms));
...
#define N_VALS 6
  ...
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom_WM_ICON_SIZE, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)vals, N_VALS);
...
static void reload_xinerama_infos(MetaScreen *screen) {
  ...
  if (screen->xinerama_infos) g_free(screen->xinerama_infos);
  ...
  if (XineramaIsActive(display->xdisplay)) {
    ...
    meta_topic(META_DEBUG_XINERAMA, "Found %d Xinerama screens on display %s\n",
    ...
    if (n_infos > 0) {
      ...
      while (i < n_infos) {
        ...
        meta_topic(META_DEBUG_XINERAMA, "Xinerama %d is %d,%d %d x %d\n",
    ...
    meta_topic(META_DEBUG_XINERAMA,
               "No XFree86 Xinerama extension or XFree86 Xinerama inactive on "
               "display %s\n",
               display->name);
  }
#else
  ...
  if (screen->n_xinerama_infos == 0 &&
      XineramaGetState(screen->display->xdisplay, screen->number)) {
    ...
    n_monitors = 0;
    result = XineramaGetInfo(screen->display->xdisplay, screen->number,
                             monitors, hints, &n_monitors);
    ...
    meta_topic(META_DEBUG_XINERAMA,
               "No Solaris Xinerama extension or Solaris Xinerama inactive on "
               "display %s\n",
               display->name);
  }
#else
...
MetaScreen *meta_screen_new(MetaDisplay *display, int number,
  ...
  if (current_wm_sn_owner != None) {
    ...
    if (!replace_current_wm) {
      meta_warning(_("Screen %d on display \"%s\" already has a window "
    ...
    XChangeWindowAttributes(xdisplay, current_wm_sn_owner, CWEventMask, &attrs);
  ...
  XSetSelectionOwner(xdisplay, wm_sn_atom, new_wm_sn_owner, manager_timestamp);

  if (XGetSelectionOwner(xdisplay, wm_sn_atom) != new_wm_sn_owner) {
    meta_warning(_("Could not acquire window manager selection on screen %d "
                   "display \"%s\"\n"),
                 number, display->name);
  ...
  XSelectInput(xdisplay, xroot,
               SubstructureRedirectMask | SubstructureNotifyMask |
                   ColormapChangeMask | PropertyChangeMask | LeaveWindowMask |
  ...
  if (meta_error_trap_pop_with_return(display, FALSE) != Success) {
    meta_warning(
        _("Screen %d on display \"%s\" already has a window manager\n"), number,
        display->name);
  ...
  screen->wm_cm_selection_window =
      meta_create_offscreen_window(xdisplay, xroot, NoEventMask);
#endif
  ...
  screen->no_focus_window = meta_create_offscreen_window(
      display->xdisplay, screen->xroot,
  ...
  current_workspace = 0;
  if (meta_prop_get_cardinal(screen->display, screen->xroot,
  ...
  screen->ui = meta_ui_new(screen->display->xdisplay, screen->xscreen);
...
#ifdef HAVE_STARTUP_NOTIFICATION
  screen->sn_context =
      sn_monitor_context_new(screen->display->sn_display, screen->number,
                             meta_screen_sn_event, screen, NULL);
  ...
  /* Switch to the _NET_CURRENT_DESKTOP workspace */
  {
    ...
    space = meta_screen_get_workspace_by_index(screen, current_workspace);

    if (space != NULL) meta_workspace_activate(space, timestamp);
  }

  meta_verbose("Added screen %d ('%s') root 0x%lx\n", screen->number,
               screen->screen_name, screen->xroot);
...
void meta_screen_free(MetaScreen *screen, guint32 timestamp) {
  ...
  XDestroyWindow(screen->display->xdisplay, screen->wm_sn_selection_window);

  if (screen->work_area_idle != 0) g_source_remove(screen->work_area_idle);

  if (XGetGCValues(screen->display->xdisplay, screen->root_xor_gc, GCFont,
                   &gc_values)) {
    XUnloadFont(screen->display->xdisplay, gc_values.font);
  }

  XFreeGC(screen->display->xdisplay, screen->root_xor_gc);
  ...
  if (screen->tile_preview) meta_tile_preview_free(screen->tile_preview);
...
static GList *list_windows(MetaScreen *screen) {
  ...
  XQueryTree(screen->display->xdisplay, screen->xroot, &ignored1, &ignored2,
             &children, &n_children);
  ...
  for (i = 0; i < n_children; ++i) {
    ...
    XGetWindowAttributes(screen->display->xdisplay, children[i], &info->attrs);
...
#ifdef HAVE_COMPOSITE_EXTENSIONS
    test_window_owner =
        test_window_owner || info->xwindow == screen->wm_cm_selection_window;
#endif
...
void meta_screen_composite_all_windows(MetaScreen *screen) {
  ...
  for (list = windows; list != NULL; list = list->next) {
    ...
    meta_compositor_add_window(
        display->compositor,
        meta_display_lookup_x_window(display, info->xwindow), info->xwindow,
        &info->attrs);
  }
...
void meta_screen_foreach_window(MetaScreen *screen, MetaScreenWindowFunc func,
  ...
  winlist = NULL;
  g_hash_table_foreach(screen->display->window_ids, listify_func, &winlist);
  ...
  while (tmp != NULL) {
     ...
     */
    if (tmp->next == NULL || (tmp->next && tmp->next->data != tmp->data)) {
      MetaWindow *window = tmp->data;

      if (window->screen == screen) (*func)(screen, window, data);
    }
...
static void queue_resize(MetaScreen *screen, MetaWindow *window,
                         gpointer data) {
...
MetaWorkspace *meta_screen_get_workspace_by_index(MetaScreen *screen, int idx) {
  GList *tmp;
  int i;
...
static void set_number_of_spaces_hint(MetaScreen *screen, int n_spaces) {
  ...
  meta_error_trap_push(screen->display);
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_NUMBER_OF_DESKTOPS, XA_CARDINAL,
...
static void set_desktop_geometry_hint(MetaScreen *screen) {
  ...
  meta_error_trap_push(screen->display);
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_DESKTOP_GEOMETRY, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, 2);
  meta_error_trap_pop(screen->display, FALSE);
}
...
static void set_desktop_viewport_hint(MetaScreen *screen) {
  ...
  meta_error_trap_push(screen->display);
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_DESKTOP_VIEWPORT, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, 2);
  meta_error_trap_pop(screen->display, FALSE);
}
...
static void update_num_workspaces(MetaScreen *screen, guint32 timestamp) {
  ...
  while (tmp != NULL) {
    ...
    if (w == screen->active_workspace) need_change_space = TRUE;

    tmp = tmp->next;
  }

  if (need_change_space) meta_workspace_activate(last_remaining, timestamp);
...
void meta_screen_set_cursor(MetaScreen *screen, MetaCursor cursor) {
  ...
  if (cursor == screen->current_cursor) return;
...
void meta_screen_update_cursor(MetaScreen *screen) {
  ...
  xcursor =
      meta_display_create_x_cursor(screen->display, screen->current_cursor);
  XDefineCursor(screen->display->xdisplay, screen->xroot, xcursor);
  XFlush(screen->display->xdisplay);
  XFreeCursor(screen->display->xdisplay, xcursor);
}
...
static cairo_surface_t *get_window_surface(MetaWindow *window) {
  ...
  surface =
      meta_compositor_get_window_surface(window->display->compositor, window);
  ...
  if (width > height) {
    max_size = MIN(MAX_PREVIEW_SIZE, MAX_PREVIEW_SCREEN_FRACTION *
                                         ((double)current->rect.width) /
                                         ((double)max_columns));
...
void meta_screen_ensure_tab_popup(MetaScreen *screen, MetaTabList list_type,
  ...
  if (screen->tab_popup) return;

  tab_list = meta_display_get_tab_list(screen->display, list_type, screen,
  ...
  border = meta_prefs_show_tab_border()
               ? BORDER_OUTLINE_TAB | BORDER_OUTLINE_WINDOW
               : BORDER_OUTLINE_TAB;
  i = 0;
  tmp = tab_list;
  while (i < len) {
    ...
    if (meta_prefs_get_compositing_manager() &&
        !meta_prefs_get_compositing_fast_alt_tab()) {
...
#define ICON_OFFSET 6
        ...
        surface = cairo_surface_create_similar(
            win_surface, cairo_surface_get_content(win_surface), width, height);
        ...
        cairo_set_source_surface(cr, icon, width - icon_width - ICON_OFFSET,
    ...
    if (show_type == META_TAB_SHOW_INSTANTLY || !entries[i].hidden ||
...
#define OUTLINE_WIDTH 5
      ...
      entries[i].inner_rect.height =
          r.height - entries[i].inner_rect.y - OUTLINE_WIDTH;

      /* Left side */
      entries[i].inner_rect.x = OUTLINE_WIDTH;

      /* Right side */
      entries[i].inner_rect.width =
          r.width - entries[i].inner_rect.x - OUTLINE_WIDTH;
    }
  ...
  screen->tab_popup = meta_ui_tab_popup_new(
      entries, len, meta_prefs_get_alt_tab_max_columns(),
      meta_prefs_get_alt_tab_expand_to_fit_title(), border);

  for (i = 0; i < len; i++) {
    g_object_unref(entries[i].icon);
    if (entries[i].win_surface) cairo_surface_destroy(entries[i].win_surface);
  }
...
void meta_screen_ensure_workspace_popup(MetaScreen *screen) {
  ...
  meta_screen_calc_workspace_layout(screen, n_workspaces, current_workspace,
                                    &layout);
  ...
  while (i < len) {
    if (layout.grid[i] >= 0) {
      ...
      workspace = meta_screen_get_workspace_by_index(screen, layout.grid[i]);
  ...
  screen->tab_popup = meta_ui_tab_popup_new(entries, len, layout.cols,
...
static gboolean meta_screen_tile_preview_update_timeout(gpointer data) {
  MetaScreen *screen = data;
  ...
  if (!screen->tile_preview) screen->tile_preview = meta_tile_preview_new();

  if (window) {
    switch (window->tile_mode) {
      case META_TILE_MAXIMIZED:
        if (!META_WINDOW_MAXIMIZED(window)) needs_preview = TRUE;
        break;
      ...
      default:
        if (!META_WINDOW_SIDE_TILED(window)) needs_preview = TRUE;
        break;
    }
  }
...
void meta_screen_tile_preview_update(MetaScreen *screen, gboolean delay) {
  if (delay) {
    if (screen->tile_preview_timeout_id > 0) return;

    screen->tile_preview_timeout_id =
        g_timeout_add(TILE_PREVIEW_TIMEOUT_MS,
                      meta_screen_tile_preview_update_timeout, screen);
  } else {
...
void meta_screen_tile_preview_hide(MetaScreen *screen) {
  ...
  if (screen->tile_preview) meta_tile_preview_hide(screen->tile_preview);
}
...
MetaWindow *meta_screen_get_mouse_window(MetaScreen *screen,
  ...
  if (not_this_one)
    meta_topic(META_DEBUG_FOCUS, "Focusing mouse window excluding %s\n",
               not_this_one->desc);

  meta_error_trap_push(screen->display);
  XQueryPointer(screen->display->xdisplay, screen->xroot, &root_return,
                &child_return, &root_x_return, &root_y_return, &win_x_return,
                &win_y_return, &mask_return);
  meta_error_trap_pop(screen->display, TRUE);

  window = meta_stack_get_default_focus_window_at_point(
      screen->stack, screen->active_workspace, not_this_one, root_x_return,
      root_y_return);
...
const MetaXineramaScreenInfo *meta_screen_get_xinerama_for_rect(
  ...
  if (screen->n_xinerama_infos == 1) return &screen->xinerama_infos[0];
  ...
  for (i = 0; i < screen->n_xinerama_infos; i++) {
    MetaRectangle dest;
    if (meta_rectangle_intersect(&screen->xinerama_infos[i].rect, rect,
                                 &dest)) {
...
const MetaXineramaScreenInfo *meta_screen_get_xinerama_neighbor(
    MetaScreen *screen, int which_xinerama, MetaScreenDirection direction) {
...
void meta_screen_get_natural_xinerama_list(MetaScreen *screen,
  ...
  *xineramas_list = g_new(int, screen->n_xinerama_infos);
  ...
  while (!g_queue_is_empty(xinerama_queue)) {
    current = (const MetaXineramaScreenInfo *)g_queue_pop_head(xinerama_queue);

    (*xineramas_list)[cur++] = current->number;

    /* enqueue each of the directions */
    tmp = meta_screen_get_xinerama_neighbor(screen, current->number,
                                            META_SCREEN_LEFT);
    if (tmp && !visited[tmp->number]) {
      g_queue_push_tail(xinerama_queue, (MetaXineramaScreenInfo *)tmp);
...
const MetaXineramaScreenInfo *meta_screen_get_current_xinerama(
  ...
  if (screen->display->xinerama_cache_invalidated) {
    ...
    XQueryPointer(screen->display->xdisplay, screen->xroot, &root_return,
                  &child_return, &pointer_position.x, &pointer_position.y,
...
void meta_screen_update_workspace_layout(MetaScreen *screen) {
  ...
  meta_verbose(
      "Workspace layout rows = %d cols = %d orientation = %d starting corner = "
      "%u\n",
      screen->rows_of_workspaces, screen->columns_of_workspaces,
      screen->vertical_workspaces, screen->starting_corner);
}
...
static void set_workspace_names(MetaScreen *screen) {
  ...
  while (i < n_spaces) {
    ...
    if (name)
      g_string_append_len(flattened, name, strlen(name) + 1);
    else
  ...
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  ...
                  screen->display->atom_UTF8_STRING, 8, PropModeReplace,
...
void meta_screen_update_workspace_names(MetaScreen *screen) {
  ...
  names = NULL;
  n_names = 0;
  if (!meta_prop_get_utf8_list(screen->display, screen->xroot,
                               screen->display->atom__NET_DESKTOP_NAMES, &names,
                               &n_names)) {
  ...
  while (i < n_names) {
    meta_topic(META_DEBUG_PREFS,
               "Setting workspace %d name to \"%s\" due to _NET_DESKTOP_NAMES "
               "change\n",
...
Window meta_create_offscreen_window(Display *xdisplay, Window parent,
  ...
  return XCreateWindow(xdisplay, parent, -100, -100, 1, 1, 0, CopyFromParent,
                       CopyFromParent, (Visual *)CopyFromParent,
                       CWOverrideRedirect | CWEventMask, &attrs);
}

static void set_workspace_work_area_hint(MetaWorkspace *workspace,
                                         MetaScreen *screen) {
  unsigned long *data;
  unsigned long *tmp;
  int i;
  ...
  workarea_name =
      g_strdup_printf("_GTK_WORKAREAS_D%d", meta_workspace_index(workspace));
  ...
  XChangeProperty(screen->display->xdisplay, screen->xroot, workarea_atom,
                  XA_CARDINAL, 32, PropModeReplace, (guchar *)data,
                  screen->n_xinerama_infos * 4);
  meta_error_trap_pop(screen->display, TRUE);
...
static void set_work_area_hint(MetaScreen *screen) {
  ...
  meta_error_trap_push(screen->display);
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_WORKAREA, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, num_workspaces * 4);
  g_free(data);
  meta_error_trap_pop(screen->display, FALSE);
}

static gboolean set_work_area_idle_func(MetaScreen *screen) {
  meta_topic(META_DEBUG_WORKAREA, "Running work area idle function\n");
...
void meta_screen_queue_workarea_recalc(MetaScreen *screen) {
  ...
  if (screen->work_area_idle == 0) {
    meta_topic(META_DEBUG_WORKAREA, "Adding work area hint idle function\n");
    screen->work_area_idle =
        g_idle_add_full(META_PRIORITY_WORK_AREA_HINT,
                        (GSourceFunc)set_work_area_idle_func, screen, NULL);
  }
}
...
void meta_screen_calc_workspace_layout(MetaScreen *screen, int num_workspaces,
  ...
  if (rows <= 0 && cols <= 0) cols = num_workspaces;
  ...
  if (meta_is_verbose()) {
    ...
    while (r < layout->rows) {
      ...
      c = 0;
      while (c < layout->cols) {
        if (r == layout->current_row && c == layout->current_col)
...
static void meta_screen_resize_func(MetaScreen *screen, MetaWindow *window,
                                    void *user_data) {
  if (window->struts) {
...
void meta_screen_update_showing_desktop_hint(MetaScreen *screen) {
  ...
  meta_error_trap_push(screen->display);
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_SHOWING_DESKTOP, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, 1);
  meta_error_trap_pop(screen->display, FALSE);
}
...
void meta_screen_show_desktop(MetaScreen *screen, guint32 timestamp) {
  ...
  if (screen->active_workspace->showing_desktop) return;
  ...
  while (windows != NULL) {
    ...
    if (w->screen == screen && w->type == META_WINDOW_DESKTOP &&
...
static void update_startup_feedback(MetaScreen *screen) {
  if (screen->startup_sequences != NULL) {
    ...
    meta_topic(META_DEBUG_STARTUP, "Setting default cursor\n");
    meta_screen_set_cursor(screen, META_CURSOR_DEFAULT);
  }
}

static void add_sequence(MetaScreen *screen, SnStartupSequence *sequence) {
  meta_topic(META_DEBUG_STARTUP, "Adding sequence %s\n",
             sn_startup_sequence_get_id(sequence));
  sn_startup_sequence_ref(sequence);
  screen->startup_sequences =
      g_slist_prepend(screen->startup_sequences, sequence);
  ...
  if (screen->startup_sequence_timeout == 0)
    screen->startup_sequence_timeout =
        g_timeout_add(1000, startup_sequence_timeout, screen);

  update_startup_feedback(screen);
}

static void remove_sequence(MetaScreen *screen, SnStartupSequence *sequence) {
  meta_topic(META_DEBUG_STARTUP, "Removing sequence %s\n",
             sn_startup_sequence_get_id(sequence));

  screen->startup_sequences =
      g_slist_remove(screen->startup_sequences, sequence);
...
static void collect_timed_out_foreach(void *element, void *data) {
  ...
  meta_topic(META_DEBUG_STARTUP, "Sequence used %g seconds vs. %g max: %s\n",
...
static gboolean startup_sequence_timeout(void *data) {
  ...
  g_slist_foreach(screen->startup_sequences, collect_timed_out_foreach, &ctod);

  tmp = ctod.list;
  while (tmp != NULL) {
    SnStartupSequence *sequence = tmp->data;

    meta_topic(META_DEBUG_STARTUP, "Timed out sequence %s\n",
               sn_startup_sequence_get_id(sequence));
...
static void meta_screen_sn_event(SnMonitorEvent *event, void *user_data) {
  ...
  switch (sn_monitor_event_get_type(event)) {
    case SN_MONITOR_EVENT_INITIATED: {
      ...
      meta_topic(
          META_DEBUG_STARTUP, "Received startup initiated for %s wmclass %s\n",
          sn_startup_sequence_get_id(sequence), wmclass ? wmclass : "(unset)");
      add_sequence(screen, sequence);
    } break;

    case SN_MONITOR_EVENT_COMPLETED: {
      meta_topic(META_DEBUG_STARTUP, "Received startup completed for %s\n",
                 sn_startup_sequence_get_id(sequence));
      remove_sequence(screen, sn_monitor_event_get_startup_sequence(event));
    } break;

    case SN_MONITOR_EVENT_CHANGED:
      meta_topic(META_DEBUG_STARTUP, "Received startup changed for %s\n",
                 sn_startup_sequence_get_id(sequence));
      break;

    case SN_MONITOR_EVENT_CANCELED:
      meta_topic(META_DEBUG_STARTUP, "Received startup canceled for %s\n",
                 sn_startup_sequence_get_id(sequence));
      break;
  }
}
...
gboolean meta_screen_apply_startup_properties(MetaScreen *screen,
  ...
  meta_topic(META_DEBUG_STARTUP, "Applying startup props to %s id \"%s\"\n",
             window->desc, startup_id ? startup_id : "(none)");
  ...
  if (startup_id == NULL) {
    ...
    while (tmp != NULL) {
      ...
      if (wmclass != NULL &&
          ((window->res_class && strcmp(wmclass, window->res_class) == 0) ||
           (window->res_name && strcmp(wmclass, window->res_name) == 0))) {
        sequence = tmp->data;
        ...
        meta_topic(META_DEBUG_STARTUP,
                   ...
                   sn_startup_sequence_get_id(sequence), window->desc);
  ...
  if (startup_id == NULL) return FALSE;
...
int meta_screen_get_screen_number(MetaScreen *screen) { return screen->number; }

MetaDisplay *meta_screen_get_display(MetaScreen *screen) {
  return screen->display;
}

Window meta_screen_get_xroot(MetaScreen *screen) { return screen->xroot; }
...
void meta_screen_unset_cm_selection(MetaScreen *screen) {
  XSetSelectionOwner(screen->display->xdisplay, screen->wm_cm_atom, None,
                     screen->wm_cm_timestamp);
}

gboolean meta_screen_is_cm_selected(MetaScreen *screen) {
  Window owner =
      XGetSelectionOwner(screen->display->xdisplay, screen->wm_cm_atom);
  return owner != None;
}
