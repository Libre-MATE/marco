...
static void ensure_logfile(void) {
  if (logfile == NULL && g_getenv("MARCO_USE_LOGFILE")) {
    ...
    tmpl = g_strdup_printf("marco-%d-debug-log-XXXXXX", (int)getpid());

    err = NULL;
    fd = g_file_open_tmp(tmpl, &filename, &err);
...
gboolean meta_is_verbose(void) { return is_verbose; }
...
#else
  if (setting) ensure_logfile();
#endif

  is_verbose = setting;
}

gboolean meta_is_debugging(void) { return is_debugging; }
...
gboolean meta_get_replace_current_wm(void) { return replace_current; }
...
void meta_debug_spew_real(const char *format, ...) {
  ...
  if (no_prefix == 0) utf8_fputs(_("Window manager: "), out);
  utf8_fputs(str, out);
...
void meta_verbose_real(const char *format, ...) {
  ...
  if (no_prefix == 0) utf8_fputs("Window manager: ", out);
  utf8_fputs(str, out);
...
void meta_bug(const char *format, ...) {
  ...
  if (no_prefix == 0) utf8_fputs(_("Bug in window manager: "), out);
  utf8_fputs(str, out);
...
void meta_warning(const char *format, ...) {
  ...
  if (no_prefix == 0) utf8_fputs(_("Window manager warning: "), out);
  utf8_fputs(str, out);
...
void meta_fatal(const char *format, ...) {
  ...
  if (no_prefix == 0) utf8_fputs(_("Window manager error: "), out);
  utf8_fputs(str, out);
...
GPid meta_show_dialog(const char *type, const char *message,
                      const char *timeout, const char *display,
                      const char *ok_text, const char *cancel_text,
                      const int transient_for, GSList *columns,
                      GSList *entries) {
  GError *error = NULL;
  GSList *tmp;
  int i = 0;
  ...
  const char **argvl =
      g_malloc(sizeof(char *) *
               (17 + g_slist_length(columns) * 2 + g_slist_length(entries)));
  ...
  g_spawn_async("/", (gchar **)argvl, /* ugh */
                NULL, G_SPAWN_SEARCH_PATH | G_SPAWN_DO_NOT_REAP_CHILD, NULL,
                NULL, &child_pid, &error);

  if (transient_for) unsetenv("WINDOWID");
...
GPid meta_show_entry_dialog(const char *message, gint *active_workspace_id,
                            const char *entry_text, const char *display,
                            const char *ok_text, const char *cancel_text,
                            const int transient_for,
                            const GIOFunc stdio_func_cb) {
  meta_topic(META_DEBUG_KEYBINDINGS,
             "meta_show_entry_dialog: called. active_workspace_id=%d "
             "message=%s entry_text=%s\n",
  ...
  if (error) {
    ...
    g_io_add_watch(ioc, G_IO_IN | G_IO_PRI | G_IO_ERR | G_IO_HUP | G_IO_NVAL,
                   stdio_func_cb, active_workspace_id);
