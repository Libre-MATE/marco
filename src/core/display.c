...
static void remove_pending_pings_for_window(MetaDisplay *display,
  ...
  for (tmp = display->pending_pings; tmp; tmp = tmp->next) {
    ...
    if (ping_data->xwindow == xwindow) dead = g_slist_prepend(dead, ping_data);
  }
...
static void sn_error_trap_push(SnDisplay *sn_display, Display *xdisplay) {
  ...
  if (display != NULL) meta_error_trap_push(display);
}

static void sn_error_trap_pop(SnDisplay *sn_display, Display *xdisplay) {
  MetaDisplay *display;
  display = meta_display_for_x_display(xdisplay);
  if (display != NULL) meta_error_trap_pop(display, FALSE);
}
#endif
...
static void enable_compositor(MetaDisplay *display,
  ...
  if (!META_DISPLAY_HAS_COMPOSITE(display) ||
      !META_DISPLAY_HAS_DAMAGE(display) || !META_DISPLAY_HAS_XFIXES(display) ||
    ...
    meta_warning(_("Missing %s extension required for compositing"),
                 ...
                 : !META_DISPLAY_HAS_XFIXES(display)  ? "xfixes"
                                                      : "render");
    return;
  }

  if (!display->compositor) display->compositor = meta_compositor_new(display);

  if (!display->compositor) return;

  for (list = display->screens; list != NULL; list = list->next) {
    MetaScreen *screen = list->data;

    meta_compositor_manage_screen(screen->display->compositor, screen);

    if (composite_windows) meta_screen_composite_all_windows(screen);
  }
}
...
#undef item
  ...
  if (meta_is_syncing()) XSynchronize(xdisplay, True);
  ...
  the_display->static_gravity_works =
      g_getenv("MARCO_USE_STATIC_GRAVITY") != NULL;
...
#undef item
...
#ifdef HAVE_STARTUP_NOTIFICATION
  the_display->sn_display = sn_display_new(
      the_display->xdisplay, sn_error_trap_push, sn_error_trap_pop);
#endif
  ...
  the_display->window_ids =
      g_hash_table_new(meta_unsigned_long_hash, meta_unsigned_long_equal);
...
#ifdef HAVE_XSYNC
  {
    ...
    if (!XSyncQueryExtension(the_display->xdisplay,
        ...
        !XSyncInitialize(the_display->xdisplay, &major, &minor)) {
      the_display->xsync_error_base = 0;
      the_display->xsync_event_base = 0;
    } else
    ...
    meta_verbose(
        "Attempted to init Xsync, found version %d.%d error base %d event base "
        "%d\n",
...
#ifdef HAVE_SHAPE
  {
    ...
    meta_verbose("Attempted to init Shape, found error base %d event base %d\n",
                 the_display->shape_error_base, the_display->shape_event_base);
  }
#else  /* HAVE_SHAPE */
...
#ifdef HAVE_RENDER
  {
    ...
    meta_verbose(
        "Attempted to init Render, found error base %d event base %d\n",
        the_display->render_error_base, the_display->render_event_base);
  }
#else  /* HAVE_RENDER */
...
#ifdef HAVE_COMPOSITE_EXTENSIONS
  {
    ...
    meta_verbose(
        ...
        "extn ver %d %d\n",
        the_display->composite_error_base, the_display->composite_event_base,
        composite_major_version, composite_minor_version);
    ...
    meta_verbose(
        ...
        the_display->damage_error_base, the_display->damage_event_base);
    ...
    meta_verbose(
        ...
        the_display->xfixes_error_base, the_display->xfixes_event_base);
  }
   ...
   */
  {
     ...
     * 354213 for details.
     */
    the_display->leader_window = meta_create_offscreen_window(
        the_display->xdisplay, DefaultRootWindow(the_display->xdisplay),
        PropertyChangeMask);
    ...
    meta_prop_set_utf8_string_hint(the_display, the_display->leader_window,
                                   the_display->atom__MARCO_VERSION, VERSION);

    data[0] = the_display->leader_window;
    XChangeProperty(the_display->xdisplay, the_display->leader_window,
                    the_display->atom__NET_SUPPORTING_WM_CHECK, XA_WINDOW, 32,
                    PropModeReplace, (guchar *)data, 1);

    XWindowEvent(the_display->xdisplay, the_display->leader_window,
     ...
     */
    XSelectInput(the_display->xdisplay, the_display->leader_window,
                 NoEventMask);
  }
   ...
   */
  the_display->timestamp_pinging_window = meta_create_offscreen_window(
      the_display->xdisplay, DefaultRootWindow(the_display->xdisplay),
      PropertyChangeMask);
  ...
  while (i < ScreenCount(xdisplay)) {
    ...
    if (screen) screens = g_slist_prepend(screens, screen);
    ++i;
  }
...

  {
    ...
    if (focus == None || focus == PointerRoot)
      ...
      meta_display_focus_the_no_focus_window(
          the_display, the_display->screens->data, timestamp);
    else {
      MetaWindow *window;
      window = meta_display_lookup_x_window(the_display, focus);
      if (window)
        meta_display_set_input_focus_window(the_display, window, FALSE,
                                            timestamp);
      else
        /* Just focus the no_focus_window on the first screen */
        meta_display_focus_the_no_focus_window(
...
GSList *meta_display_list_windows(MetaDisplay *display) {
  ...
  winlist = NULL;
  g_hash_table_foreach(display->window_ids, listify_func, &winlist);
  ...
  while (tmp != NULL) {
    ...
    if (next && next->data == tmp->data) {
      /* Delete tmp from list */

      if (prev) prev->next = next;

      if (tmp == winlist) winlist = next;
...
void meta_display_close(MetaDisplay *display, guint32 timestamp) {
  ...
  meta_ui_remove_event_func(display->xdisplay, event_callback, display);
  ...
  if (display->compositor) meta_compositor_destroy(display->compositor);
...
MetaScreen *meta_display_screen_for_root(MetaDisplay *display, Window xroot) {
  ...
  while (tmp != NULL) {
    ...
    if (xroot == screen->xroot) return screen;
...
MetaScreen *meta_display_screen_for_xwindow(MetaDisplay *display,
   ...
   * on success.
   */
  if (result == 0 || attr.screen == NULL) return NULL;
...
MetaScreen *meta_display_screen_for_x_screen(MetaDisplay *display,
  ...
  while (tmp != NULL) {
    ...
    if (xscreen == screen->xscreen) return screen;
...
MetaDisplay *meta_display_for_x_display(Display *xdisplay) {
  if (the_display->xdisplay == xdisplay) return the_display;
 ...
 *          during startup.
 */
MetaDisplay *meta_get_display(void) { return the_display; }
...
guint32 meta_display_get_current_time_roundtrip(MetaDisplay *display) {
  ...
  if (timestamp == CurrentTime) {
    ...
    XWindowEvent(display->xdisplay, display->timestamp_pinging_window,
                 PropertyChangeMask, &property_event);
...
static void add_ignored_serial(MetaDisplay *display, unsigned long serial) {
  ...
  if (display->ignored_serials[N_IGNORED_SERIALS - 1] == serial) return;
...
static gboolean serial_is_ignored(MetaDisplay *display, unsigned long serial) {
  ...
  i = 0;
  while (i < N_IGNORED_SERIALS) {
    if (display->ignored_serials[i] == serial) return TRUE;
    ++i;
  }
  return FALSE;
}
...
static gboolean window_raise_with_delay_callback(void *data) {
  ...
  window =
      meta_display_lookup_x_window(auto_raise->display, auto_raise->xwindow);
  ...
  if (meta_stack_get_top(window->screen->stack) != window) {
    ...
    same_screen = XQueryPointer(window->display->xdisplay, window->xwindow,
...
void meta_display_queue_autoraise_callback(MetaDisplay *display,
  ...
  meta_topic(META_DEBUG_FOCUS,
             ...
             window->desc, meta_prefs_get_auto_raise_delay());
  ...
  display->autoraise_timeout_id = g_timeout_add_full(
      G_PRIORITY_DEFAULT, meta_prefs_get_auto_raise_delay(),
      window_raise_with_delay_callback, auto_raise_data, g_free);
...
static gboolean maybe_send_event_to_gtk(MetaDisplay *display, XEvent *xevent) {
  ...
  switch (xevent->type) {
    ...
    case ButtonRelease:
      ...
      if (xevent->type == ButtonPress) {
        ...
        if (xevent->xbutton.button == display->button_click_number &&
            ...
            xevent->xbutton.time <
                display->button_click_time + double_click_time &&
            ABS(xevent->xbutton.x - display->button_click_x) <=
                double_click_distance &&
            ABS(xevent->xbutton.y - display->button_click_y) <=
                double_click_distance) {
    ...
    case LeaveNotify:
      gdk_event = gdk_event_new(xevent->type == EnterNotify ? GDK_ENTER_NOTIFY
                                                            : GDK_LEAVE_NOTIFY);
...
static gboolean mouse_event_is_in_tab_popup(MetaDisplay *display,
                                            MetaScreen *screen,
                                            Window event_window, int event_x,
                                            int event_y, int *popup_x,
                                            int *popup_y) {
  if (screen && screen->tab_popup) {
    int x, y;
    Window child1, child2;
    gboolean ok1 =
        XTranslateCoordinates(display->xdisplay, event_window, event_window,
                              event_x, event_y, &x, &y, &child1);
    ...
    if (ok1 && popup_widget != NULL) {
      ...
      gboolean ok2 =
          XTranslateCoordinates(display->xdisplay, event_window, popup_xid,
                                event_x, event_y, popup_x, popup_y, &child2);
...
static gboolean event_callback(XEvent *event, gpointer data) {
...
#ifdef WITH_VERBOSE_MODE
  if (dump_events) meta_spew_event(display, event);
#endif
  ...
  if (META_DISPLAY_HAS_XSYNC(display) &&
    ...
    if (display->grab_op != META_GRAB_OP_NONE && display->grab_window != NULL &&
    ...
    if (window && !frame_was_receiver) {
      ...
      if (sev->kind == ShapeBounding) {
        if (sev->shaped && !window->has_shape) {
          ...
          meta_topic(META_DEBUG_SHAPES, "Window %s now has a shape\n",
                     window->desc);
        } else if (!sev->shaped && window->has_shape) {
          window->has_shape = FALSE;
          meta_topic(META_DEBUG_SHAPES, "Window %s no longer has a shape\n",
                     window->desc);
        } else {
          meta_topic(META_DEBUG_SHAPES, "Window %s shape changed\n",
                     window->desc);
        }
      ...
      meta_topic(META_DEBUG_SHAPES,
                 "ShapeNotify not on a client window (window %s "
                 "frame_was_receiver = %d)\n",
                 window ? window->desc : "(none)", frame_was_receiver);
    }
  }
#endif /* HAVE_SHAPE */
  ...
  if (window && ((event->type == KeyPress) || (event->type == ButtonPress))) {
    if (CurrentTime == display->current_time) {
       ...
       */
      meta_warning(
          "Event has no timestamp! You may be using a broken "
  ...
  switch (event->type) {
    ...
    case KeyRelease:
      ...
      break;
    case ButtonPress:
      /* Use window under pointer when processing synthetic events from another
       * client */
      if (window == NULL && event->xbutton.send_event) {
        int x, y, root_x, root_y;
        Window root, child;
        guint mask;
        XQueryPointer(display->xdisplay, event->xany.window, &root, &child,
                      &root_x, &root_y, &x, &y, &mask);
        window = meta_display_lookup_x_window(display, child);
      }
      if ((window && grab_op_is_mouse(display->grab_op) &&
        ...
        if (grab_op_is_keyboard(display->grab_op)) {
          ...
          int popup_x, popup_y;
          is_in_tab_popup = mouse_event_is_in_tab_popup(
              display, screen, event->xany.window, event->xbutton.x,
              event->xbutton.y, &popup_x, &popup_y);
        ...
        if (!is_in_tab_popup) {
          ...
          meta_display_end_grab_op(display, event->xbutton.time);
        }
        ...
        if (g_getenv("MARCO_DEBUG_BUTTON_GRABS")) grab_mask |= ControlMask;
        ...
        if (unmodified || event->xbutton.button == 1) {
          ...
          if (!frame_was_receiver) {
            if (meta_prefs_get_raise_on_click())
              ...
              meta_topic(META_DEBUG_FOCUS,
                         "Not raising window on click due to "
                         "don't-raise-on-click option\n");
           ...
           * the click-to-focus
           */
          if (!unmodified) begin_move = TRUE;
        } else if (!unmodified && event->xbutton.button ==
          ...
          if (window->has_resize_func) {
            ...
            west =
                event->xbutton.x_root < (root_x + 1 * window->rect.width / 3);
            east =
                event->xbutton.x_root > (root_x + 2 * window->rect.width / 3);
            north =
                event->xbutton.y_root < (root_y + 1 * window->rect.height / 3);
            south =
                event->xbutton.y_root > (root_y + 2 * window->rect.height / 3);
            ...
            if (op != META_GRAB_OP_NONE)
              meta_display_begin_grab_op(
                  display, window->screen, window, op, TRUE, FALSE,
                  event->xbutton.button, 0, event->xbutton.time,
                  event->xbutton.x_root, event->xbutton.y_root);
          }
        } else if (event->xbutton.button ==
                   meta_prefs_get_mouse_button_menu()) {
          if (meta_prefs_get_raise_on_click()) meta_window_raise(window);
          meta_window_show_menu(window, event->xbutton.x_root,
                                event->xbutton.y_root, event->xbutton.button,
                                event->xbutton.time);
        }
        ...
        if (!frame_was_receiver && unmodified) {
          ...
          if (meta_prefs_get_focus_mode() == META_FOCUS_MODE_CLICK &&
              meta_prefs_get_application_based() && !window->has_focus &&
              window->type != META_WINDOW_DOCK &&
              window->type != META_WINDOW_DESKTOP &&
              (display->focus_window == NULL ||
               !meta_window_same_application(window, display->focus_window)))
          ...
          XAllowEvents(display->xdisplay, mode, event->xbutton.time);
        }

        if (begin_move && window->has_move_func) {
          meta_display_begin_grab_op(
              display, window->screen, window, META_GRAB_OP_MOVING, TRUE, FALSE,
              event->xbutton.button, 0, event->xbutton.time,
              event->xbutton.x_root, event->xbutton.y_root);
        }
      }
      break;
    case ButtonRelease:
      if (display->grab_window == window && grab_op_is_mouse(display->grab_op))
    ...
    case MotionNotify:
      if (display->grab_window == window && grab_op_is_mouse(display->grab_op))
        meta_window_handle_mouse_grab_op_event(window, event);
      else if (grab_op_is_keyboard(display->grab_op) &&
               display->tab_popup_mouse_pressed) {
        MetaScreen *screen =
            meta_display_screen_for_root(display, event->xany.window);
        int popup_x, popup_y;
        gboolean is_in_tab_popup = mouse_event_is_in_tab_popup(
            display, screen, event->xany.window, event->xbutton.x,
            event->xbutton.y, &popup_x, &popup_y);
        if (is_in_tab_popup)
          meta_ui_tab_popup_mouse_press(screen->tab_popup, popup_x, popup_y);
    ...
    case EnterNotify:
      ...
      if (window && !serial_is_ignored(display, event->xany.serial) &&
        ...
        switch (meta_prefs_get_focus_mode()) {
          ...
          case META_FOCUS_MODE_MOUSE:
            ...
            if (window->type == META_WINDOW_DESKTOP &&
              ...
              meta_display_focus_the_no_focus_window(display, window->screen,
                                                     event->xcrossing.time);
            }
            break;
        ...
        if (window->type == META_WINDOW_DOCK) meta_window_raise(window);
      }
      break;
    case LeaveNotify:
      if (display->grab_window == window && grab_op_is_mouse(display->grab_op))
        ...
        if (window->type == META_WINDOW_DOCK &&
            event->xcrossing.mode != NotifyGrab &&
            event->xcrossing.mode != NotifyUngrab && !window->has_focus)
          meta_window_lower(window);
      }
      break;
    case FocusIn:
    case FocusOut:
      ...
      } else if (meta_display_xwindow_is_a_no_focus_window(
                     display, event->xany.window)) {
        meta_topic(META_DEBUG_FOCUS,
    ...
    case DestroyNotify:
      if (window) {
        ...
        if (frame_was_receiver) {
          meta_warning(
              "Unexpected destruction of frame 0x%lx, not sure if this should "
              "silently fail or be considered a bug\n",
      ...
      if (window) {
        ...
        if (!frame_was_receiver) {
          if (window->unmaps_pending == 0) {
            meta_topic(META_DEBUG_WINDOW_STATE, "Window %s withdrawn\n",
                       window->desc);
         ...
         */
        if (window) meta_window_notify_focus(window, event);
      }
      break;
    case MapNotify:
      break;
      ...
      /* if frame was receiver it's some malicious send event or something */
        ...
        if (window->minimized) {
          ...
          if (window->workspace != window->screen->active_workspace) {
            meta_verbose(
                "Changing workspace due to MapRequest mapped = %d minimized = "
                "%d\n",
    ...
    case ConfigureNotify:
      /* Handle screen resize */
      {
        ...
        screen =
            meta_display_screen_for_root(display, event->xconfigure.window);
        ...
        if (screen != NULL) {
          ...
          meta_screen_resize(screen, event->xconfigure.width,
    ...
    case ConfigureRequest:
      ...
      if (window == NULL) {
        ...
        if (!frame_was_receiver) meta_window_configure_request(window, event);
      }
      break;
    case GravityNotify:
      break;
    ...
    case PropertyNotify: {
      ...
      if (group != NULL) meta_group_property_notify(group, event);

      screen = NULL;
      if (window == NULL &&
          group ==
              NULL) /* window/group != NULL means it wasn't a root window */
        screen = meta_display_screen_for_root(display, event->xproperty.window);
      ...
      if (screen != NULL) {
         ...
         */
        if (event->xproperty.atom == display->atom__MARCO_SENTINEL) {
    ...
    case ClientMessage:
      if (window) {
        if (!frame_was_receiver) meta_window_client_message(window, event);
      } else {
        MetaScreen *screen;

        screen = meta_display_screen_for_root(display, event->xclient.window);
        ...
        if (screen) {
          if (event->xclient.message_type ==
            ...
            if (time == 0 && workspace) {
              meta_warning(
                  "Received a NET_CURRENT_DESKTOP message "
...
static void meta_spew_event(MetaDisplay *display, XEvent *event) {
  ...
  switch (event->type) {
    ...
    case ButtonPress:
      ...
      extra = g_strdup_printf(
          "button %u state 0x%x x %d y %d root 0x%lx same_screen %d",
          event->xbutton.button, event->xbutton.state, event->xbutton.x,
          event->xbutton.y, event->xbutton.root, event->xbutton.same_screen);
      break;
    case ButtonRelease:
      name = "ButtonRelease";
      extra = g_strdup_printf(
    ...
    case MotionNotify:
      name = "MotionNotify";
      extra = g_strdup_printf("win: 0x%lx x: %d y: %d", event->xmotion.window,
                              event->xmotion.x, event->xmotion.y);
      break;
    case EnterNotify:
      name = "EnterNotify";
      extra = g_strdup_printf(
          "win: 0x%lx root: 0x%lx subwindow: 0x%lx mode: %s detail: %s focus: "
          "%d x: %d y: %d",
          ...
          event->xcrossing.focus, event->xcrossing.x, event->xcrossing.y);
      break;
    case LeaveNotify:
      name = "LeaveNotify";
      extra = g_strdup_printf(
    ...
    case MapNotify:
      name = "MapNotify";
      extra = g_strdup_printf(
          "event: 0x%lx window: 0x%lx override_redirect: %d", event->xmap.event,
          event->xmap.window, event->xmap.override_redirect);
      break;
    case MapRequest:
      name = "MapRequest";
      extra =
          g_strdup_printf("window: 0x%lx parent: 0x%lx\n",
                          event->xmaprequest.window, event->xmaprequest.parent);
      break;
    case ReparentNotify:
      name = "ReparentNotify";
    ...
    case ConfigureNotify:
      ...
      extra = g_strdup_printf(
          ...
          event->xconfigure.height, event->xconfigure.above,
          event->xconfigure.override_redirect);
      break;
    case ConfigureRequest:
      name = "ConfigureRequest";
      extra = g_strdup_printf(
          "parent: 0x%lx window: 0x%lx x: %d %sy: %d %sw: %d %sh: %d %sborder: "
          "%d %sabove: %lx %sstackmode: %s %s",
          event->xconfigurerequest.parent, event->xconfigurerequest.window,
          event->xconfigurerequest.x,
          event->xconfigurerequest.value_mask & CWX ? "" : "(unset) ",
          event->xconfigurerequest.y,
          event->xconfigurerequest.value_mask & CWY ? "" : "(unset) ",
          event->xconfigurerequest.width,
          event->xconfigurerequest.value_mask & CWWidth ? "" : "(unset) ",
          event->xconfigurerequest.height,
          event->xconfigurerequest.value_mask & CWHeight ? "" : "(unset) ",
          event->xconfigurerequest.border_width,
          event->xconfigurerequest.value_mask & CWBorderWidth ? "" : "(unset)",
          event->xconfigurerequest.above,
          event->xconfigurerequest.value_mask & CWSibling ? "" : "(unset)",
          stack_mode_to_string(event->xconfigurerequest.detail),
          event->xconfigurerequest.value_mask & CWStackMode ? "" : "(unset)");
      break;
    case GravityNotify:
      name = "GravityNotify";
      break;
    case ResizeRequest:
      name = "ResizeRequest";
      extra =
          g_strdup_printf("width = %d height = %d", event->xresizerequest.width,
    ...
    case PropertyNotify: {
      ...
      meta_error_trap_push(display);
      str = XGetAtomName(display->xdisplay, event->xproperty.atom);
      meta_error_trap_pop(display, TRUE);
    ...
    case ClientMessage: {
      ...
      name = "ClientMessage";
      meta_error_trap_push(display);
      str = XGetAtomName(display->xdisplay, event->xclient.message_type);
      meta_error_trap_pop(display, TRUE);
      extra =
          g_strdup_printf("type: %s format: %d\n", str ? str : "(unknown atom)",
    ...
    default:
      ...
      if (META_DISPLAY_HAS_XSYNC(display) &&
        ...
        extra = g_strdup_printf(
            ...
            " alarm_value: %" G_GINT64_FORMAT " time: %u alarm state: %s",
            aevent->alarm, (gint64)sync_value_to_64(&aevent->counter_value),
            (gint64)sync_value_to_64(&aevent->alarm_value),
            (unsigned int)aevent->time, alarm_state_to_string(aevent->state));
      } else
#endif /* HAVE_XSYNC */
#ifdef HAVE_SHAPE
          if (META_DISPLAY_HAS_SHAPE(display) &&
        ...
        extra = g_strdup_printf(
            ...
            sev->kind == ShapeBounding
                ? "ShapeBounding"
                : (sev->kind == ShapeClip ? "ShapeClip" : "(unknown)"),
            sev->x, sev->y, sev->width, sev->height, sev->shaped);
      } else
#endif /* HAVE_SHAPE */
      {
        name = "(Unknown event)";
  ...
  meta_topic(META_DEBUG_EVENTS, "%s on %s%s %s %sserial %lu\n", name, winname,
             extra ? ":" : "", extra ? extra : "",
             event->xany.send_event ? "SEND " : "", event->xany.serial);
...
void meta_display_register_x_window(MetaDisplay *display, Window *xwindowp,
                                    MetaWindow *window) {
...
static Cursor xcursor_for_op(MetaDisplay *display, MetaGrabOp op) {
  ...
  if (cursor == META_CURSOR_DEFAULT) return None;
  return meta_display_create_x_cursor(display, cursor);
}

void meta_display_set_grab_op_cursor(MetaDisplay *display, MetaScreen *screen,
                                     MetaGrabOp op, gboolean change_pointer,
                                     Window grab_xwindow, guint32 timestamp) {
  Cursor cursor;
...
#define GRAB_MASK                                                              \
  ...
  if (change_pointer) {
    meta_error_trap_push(display);
    XChangeActivePointerGrab(display->xdisplay, GRAB_MASK, cursor, timestamp);
    ...
    if (meta_error_trap_pop_with_return(display, FALSE) != Success) {
      ...
      if (display->grab_have_pointer) display->grab_have_pointer = FALSE;
    }
  } else {
    g_assert(screen != NULL);

    meta_error_trap_push(display);
    if (XGrabPointer(display->xdisplay, grab_xwindow, False, GRAB_MASK,
                     GrabModeAsync, GrabModeAsync, screen->xroot, cursor,
                     timestamp) == GrabSuccess) {
      display->grab_have_pointer = TRUE;
      meta_topic(META_DEBUG_WINDOW_OPS,
                 "XGrabPointer() returned GrabSuccess time %u\n", timestamp);
    } else {
      meta_topic(META_DEBUG_WINDOW_OPS, "XGrabPointer() failed time %u\n",
                 timestamp);
    }
    meta_error_trap_pop(display, TRUE);
  }
...
#undef GRAB_MASK
  ...
  if (cursor != None) XFreeCursor(display->xdisplay, cursor);
}

gboolean meta_display_begin_grab_op(MetaDisplay *display, MetaScreen *screen,
                                    MetaWindow *window, MetaGrabOp op,
                                    gboolean pointer_already_grabbed,
                                    gboolean frame_action, int button,
                                    gulong modmask, guint32 timestamp,
                                    int root_x, int root_y) {
  Window grab_xwindow;

  if (grab_op_is_mouse(op) && meta_grab_op_is_moving(op)) {
    if (display->compositor) {
      meta_compositor_begin_move(display->compositor, window, &window->rect,
                                 root_x, root_y);
    }
  }

  meta_topic(META_DEBUG_WINDOW_OPS,
             "Doing grab op %u on window %s button %d pointer already grabbed: "
             "%d pointer pos %d,%d\n",
             op, window ? window->desc : "none", button,
             pointer_already_grabbed, root_x, root_y);

  if (display->grab_op != META_GRAB_OP_NONE) {
    if (window)
      meta_warning(
          "Attempt to perform window operation %u on window %s when operation "
          "%u on %s already in effect\n",
          op, window->desc, display->grab_op,
          display->grab_window ? display->grab_window->desc : "none");
    return FALSE;
  }

  if (window && (meta_grab_op_is_moving(op) || meta_grab_op_is_resizing(op))) {
    if (meta_prefs_get_raise_on_click())
      meta_window_raise(window);
    else {
  ...
  if (pointer_already_grabbed) display->grab_have_pointer = TRUE;
  ...
  if (display->grab_window) {
    ...
    if (!display->grab_wireframe_active &&
      ...
      display->grab_sync_request_alarm =
          XSyncCreateAlarm(display->xdisplay,
      ...
      meta_topic(META_DEBUG_RESIZING, "Created update alarm 0x%lx\n",
                 display->grab_sync_request_alarm);
    }
#endif
  }
  ...
  if (GRAB_OP_IS_WINDOW_SWITCH(display->grab_op)) {
    meta_topic(META_DEBUG_WINDOW_OPS,
               ...
               display->grab_old_window_stacking);
    display->grab_old_window_stacking = meta_stack_get_positions(screen->stack);
  }

  /* Do this last, after everything is set up. */
  switch (op) {
    case META_GRAB_OP_KEYBOARD_TABBING_NORMAL:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_NORMAL,
                                   META_TAB_SHOW_ICON);
      break;
    case META_GRAB_OP_KEYBOARD_TABBING_NORMAL_ALL_WORKSPACES:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_NORMAL_ALL_WORKSPACES,
                                   META_TAB_SHOW_ICON);
      break;
    case META_GRAB_OP_KEYBOARD_ESCAPING_NORMAL:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_NORMAL,
    ...
    case META_GRAB_OP_KEYBOARD_TABBING_DOCK:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_DOCKS,
                                   META_TAB_SHOW_ICON);
      break;
    case META_GRAB_OP_KEYBOARD_ESCAPING_DOCK:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_DOCKS,
      ...
      break;
    case META_GRAB_OP_KEYBOARD_TABBING_GROUP:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_GROUP,
                                   META_TAB_SHOW_ICON);
      break;
    case META_GRAB_OP_KEYBOARD_ESCAPING_GROUP:
      meta_screen_ensure_tab_popup(screen, META_TAB_LIST_GROUP,
...
void meta_display_end_grab_op(MetaDisplay *display, guint32 timestamp) {
  meta_topic(META_DEBUG_WINDOW_OPS, "Ending grab op %u at time %u\n",
             display->grab_op, timestamp);

  if (display->grab_op == META_GRAB_OP_NONE) return;

  if (display->grab_window != NULL) display->grab_window->shaken_loose = FALSE;

  /*if(display->grab_window != NULL && display->grab_window->tile_mode ==
    META_TILE_MAXIMIZED)
    {
      display->grab_window->tile_mode = META_TILE_NONE;
    }*/

  if (display->grab_window != NULL && !meta_prefs_get_raise_on_click() &&
      (meta_grab_op_is_moving(display->grab_op) ||
       meta_grab_op_is_resizing(display->grab_op))) {
  ...
  if (display->grab_wireframe_active) {
    ...
    if (!display->grab_was_cancelled) {
      if (meta_grab_op_is_moving(display->grab_op))
        meta_window_move(display->grab_window, TRUE,
                         display->grab_wireframe_rect.x,
                         display->grab_wireframe_rect.y);
      if (meta_grab_op_is_resizing(display->grab_op))
        meta_window_resize_with_gravity(
            display->grab_window, TRUE, display->grab_wireframe_rect.width,
  ...
  if (display->compositor && display->grab_window &&
      grab_op_is_mouse(display->grab_op) &&
      meta_grab_op_is_moving(display->grab_op)) {
  ...
  if (display->grab_have_pointer) {
    meta_topic(META_DEBUG_WINDOW_OPS, "Ungrabbing pointer with timestamp %u\n",
               timestamp);
    XUngrabPointer(display->xdisplay, timestamp);
  }

  if (display->grab_have_keyboard) {
    meta_topic(META_DEBUG_WINDOW_OPS, "Ungrabbing all keys timestamp %u\n",
               timestamp);
    if (display->grab_window)
  ...
  if (display->grab_sync_request_alarm != None) {
    XSyncDestroyAlarm(display->xdisplay, display->grab_sync_request_alarm);
    display->grab_sync_request_alarm = None;
  }
#endif /* HAVE_XSYNC */
...
static void meta_change_button_grab(MetaDisplay *display, Window xwindow,
                                    gboolean grab, gboolean sync, int button,
                                    int modmask) {
  unsigned int ignored_mask;

  meta_verbose("%s 0x%lx sync = %d button = %d modmask 0x%x\n",
               grab ? "Grabbing" : "Ungrabbing", xwindow, sync, button,
               modmask);
  ...
  while (ignored_mask <= display->ignored_modifier_mask) {
    ...
    if (meta_is_debugging()) meta_error_trap_push(display);

    /* GrabModeSync means freeze until XAllowEvents */

    if (grab)
      XGrabButton(
          display->xdisplay, button, modmask | ignored_mask, xwindow, False,
          ButtonPressMask | ButtonReleaseMask | PointerMotionMask |
              PointerMotionHintMask,
          sync ? GrabModeSync : GrabModeAsync, GrabModeAsync, False, None);
    else
      XUngrabButton(display->xdisplay, button, modmask | ignored_mask, xwindow);
    ...
    if (meta_is_debugging()) {
      ...
      if (result != Success)
        meta_verbose(
            "Failed to %s button %d with mask 0x%x for window 0x%lx error code "
            "%d\n",
            grab ? "grab" : "ungrab", button, modmask | ignored_mask, xwindow,
            result);
    }
...
void meta_display_grab_window_buttons(MetaDisplay *display, Window xwindow) {
  ...
  if (display->window_grab_modifiers != 0) {
    ...
    int i;
    for (i = 1; i < 4; i++) {
      meta_change_button_grab(display, xwindow, TRUE, FALSE, i,
       ...
       */
      if (debug)
        meta_change_button_grab(display, xwindow, TRUE, FALSE, i, ControlMask);
    }
     ...
     */
    meta_change_button_grab(display, xwindow, TRUE, FALSE, 1,
                            display->window_grab_modifiers | ShiftMask);
  }
}
...
void meta_display_ungrab_window_buttons(MetaDisplay *display, Window xwindow) {
  ...
  if (display->window_grab_modifiers == 0) return;

  debug = g_getenv("MARCO_DEBUG_BUTTON_GRABS") != NULL;
  i = 1;
  while (i < 4) {
    meta_change_button_grab(display, xwindow, FALSE, FALSE, i,
                            display->window_grab_modifiers);

    if (debug)
      meta_change_button_grab(display, xwindow, FALSE, FALSE, i, ControlMask);
...
void meta_display_grab_focus_window_button(MetaDisplay *display,
...

  {
    ...
    while (i < MAX_FOCUS_BUTTON) {
      meta_change_button_grab(display, window->xwindow, TRUE, TRUE, i, 0);
...
void meta_display_ungrab_focus_window_button(MetaDisplay *display,
  ...
  if (!window->have_focus_click_grab) return;

  {
    int i = 1;
    while (i < MAX_FOCUS_BUTTON) {
      meta_change_button_grab(display, window->xwindow, FALSE, FALSE, i, 0);
...
void meta_display_update_active_window_hint(MetaDisplay *display) {
  ...
  while (tmp != NULL) {
    ...
    XChangeProperty(display->xdisplay, screen->xroot,
                    display->atom__NET_ACTIVE_WINDOW, XA_WINDOW, 32,
                    PropModeReplace, (guchar *)data, 1);
 ...
 * pointless.
 */
gboolean meta_is_syncing(void) { return is_syncing; }
...
static gboolean meta_display_ping_timeout(gpointer data) {
  ...
  meta_topic(META_DEBUG_PING, "Ping %u on window %lx timed out\n",
  ...
  ping_data->display->pending_pings =
      g_slist_remove(ping_data->display->pending_pings, ping_data);
  ping_data_free(ping_data);
 ...
 * \ingroup pings
 */
void meta_display_ping_window(MetaDisplay *display, MetaWindow *window,
                              guint32 timestamp,
                              MetaWindowPingFunc ping_reply_func,
                              MetaWindowPingFunc ping_timeout_func,
                              gpointer user_data) {
  ...
  ping_data->ping_timeout_id =
      g_timeout_add(PING_TIMEOUT_DELAY, meta_display_ping_timeout, ping_data);
...
static void process_request_frame_extents(MetaDisplay *display, XEvent *event) {
  ...
  hints_set = meta_prop_get_motif_hints(display, xwindow,
                                        display->atom__MOTIF_WM_HINTS, &hints);
  if ((hints_set && hints->decorations) || !hints_set) {
    MetaFrameBorders borders;
    MetaScreen *screen;

    screen = meta_display_screen_for_xwindow(display, event->xclient.window);
    if (screen == NULL) {
      meta_warning(
          "Received request to set _NET_FRAME_EXTENTS "
    ...
    meta_ui_theme_get_frame_borders(screen->ui, META_FRAME_TYPE_NORMAL, 0,
                                    &borders);
  ...
  meta_error_trap_push(display);
  XChangeProperty(display->xdisplay, xwindow, display->atom__NET_FRAME_EXTENTS,
...
static void process_pong_message(MetaDisplay *display, XEvent *event) {
  ...
  for (tmp = display->pending_pings; tmp; tmp = tmp->next) {
    ...
    if (timestamp == ping_data->timestamp) {
      meta_topic(META_DEBUG_PING, "Matching ping found for pong %u\n",
                 ping_data->timestamp);
...
gboolean meta_display_window_has_pending_pings(MetaDisplay *display,
  ...
  for (tmp = display->pending_pings; tmp; tmp = tmp->next) {
    ...
    if (ping_data->xwindow == window->xwindow) return TRUE;
  }
...
#define IN_TAB_CHAIN(w, t)                                                \
  (((t) == META_TAB_LIST_NORMAL && META_WINDOW_IN_NORMAL_TAB_CHAIN(w)) || \
   ((t) == META_TAB_LIST_NORMAL_ALL_WORKSPACES &&                         \
    META_WINDOW_IN_NORMAL_TAB_CHAIN(w)) ||                                \
   ((t) == META_TAB_LIST_DOCKS && META_WINDOW_IN_DOCK_TAB_CHAIN(w)) ||    \
   ((t) == META_TAB_LIST_GROUP &&                                         \
    META_WINDOW_IN_GROUP_TAB_CHAIN(w, get_focussed_group(w->display))))

static MetaWindow *find_tab_forward(MetaDisplay *display, MetaTabList type,
                                    MetaScreen *screen,
                                    MetaWorkspace *workspace, GList *start,
                                    gboolean skip_first) {
  GList *tmp;
  ...
  tmp = start;
  if (skip_first) tmp = tmp->next;

  while (tmp != NULL) {
    MetaWindow *window = tmp->data;

    if (window->screen == screen && IN_TAB_CHAIN(window, type)) return window;
  ...
  while (tmp != start && tmp != NULL) {
    ...
    if (IN_TAB_CHAIN(window, type)) return window;
...
static MetaWindow *find_tab_backward(MetaDisplay *display, MetaTabList type,
                                     MetaScreen *screen,
                                     MetaWorkspace *workspace, GList *start,
                                     gboolean skip_last) {
  GList *tmp;
...
GList *meta_display_get_tab_list(MetaDisplay *display, MetaTabList type,
  ...
  for (l = workspace_list; l != NULL; l = l->next) {
    ...
    while (tmp != NULL) {
      ...
      if (!window->minimized && window->screen == screen &&
...
MetaWindow *meta_display_get_tab_next(MetaDisplay *display, MetaTabList type,
                                      MetaScreen *screen,
                                      MetaWorkspace *workspace,
                                      MetaWindow *window, gboolean backward) {
  gboolean skip;
  GList *tab_list;
  MetaWindow *ret;
  tab_list = meta_display_get_tab_list(display, type, screen, workspace);
  ...
  if (window != NULL) {
    ...
    if (backward)
      ret = find_tab_backward(display, type, screen, workspace,
                              g_list_find(tab_list, window), TRUE);
    else
    ...
    if (backward)
      ret = find_tab_backward(display, type, screen, workspace, tab_list, skip);
    else
      ret = find_tab_forward(display, type, screen, workspace, tab_list, skip);
  }
...
MetaWindow *meta_display_get_tab_current(MetaDisplay *display, MetaTabList type,
                                         MetaScreen *screen,
                                         MetaWorkspace *workspace) {
  MetaWindow *window;
  ...
  if (window != NULL && window->screen == screen &&
      IN_TAB_CHAIN(window, type) &&
      (workspace == NULL ||
...
static MetaScreen *find_screen_for_selection(MetaDisplay *display, Window owner,
                                             Atom selection) {
  GSList *tmp;
...
static gboolean convert_property(MetaDisplay *display, MetaScreen *screen,
                                 Window w, Atom target, Atom property) {
#define N_TARGETS 4
  ...
  if (target == display->atom_TARGETS)
    XChangeProperty(display->xdisplay, w, property, XA_ATOM, 32,
                    PropModeReplace, (unsigned char *)conversion_targets,
                    N_TARGETS);
  else if (target == display->atom_TIMESTAMP)
    XChangeProperty(display->xdisplay, w, property, XA_INTEGER, 32,
                    PropModeReplace, (unsigned char *)&screen->wm_sn_timestamp,
                    1);
  ...
  if (meta_error_trap_pop_with_return(display, FALSE) != Success) return FALSE;
...
static void process_selection_request(MetaDisplay *display, XEvent *event) {
  ...
  screen = find_screen_for_selection(display, event->xselectionrequest.owner,
  ...
  if (screen == NULL) {
    ...
    meta_error_trap_push(display);
    str = XGetAtomName(display->xdisplay, event->xselectionrequest.selection);
    meta_error_trap_pop(display, TRUE);

    meta_verbose(
        "Selection request with selection %s window 0x%lx not a WM_Sn "
        "selection we recognize\n",
  ...
  if (event->xselectionrequest.target == display->atom_MULTIPLE) {
    if (event->xselectionrequest.property != None) {
      ...
      if (XGetWindowProperty(display->xdisplay,
                             ...
                             display->atom_ATOM_PAIR, &type, &format, &num,
                             &rest, &data) != Success) {
      ...
      if (meta_error_trap_pop_with_return(display, TRUE) == Success) {
        ...
        while (i < (int)num) {
          if (!convert_property(display, screen,
                                event->xselectionrequest.requestor, adata[i],
                                adata[i + 1]))
            adata[i + 1] = None;
          i += 2;
        }
        ...
        meta_error_trap_push(display);
        XChangeProperty(display->xdisplay, event->xselectionrequest.requestor,
                        event->xselectionrequest.property,
                        display->atom_ATOM_PAIR, 32, PropModeReplace, data,
                        num);
        meta_error_trap_pop(display, FALSE);
        meta_XFree(data);
      }
    ...
    if (convert_property(display, screen, event->xselectionrequest.requestor,
                         event->xselectionrequest.target,
                         event->xselectionrequest.property))
      reply.property = event->xselectionrequest.property;
  }

  XSendEvent(display->xdisplay, event->xselectionrequest.requestor, False, 0L,
             (XEvent *)&reply);
...
static void process_selection_clear(MetaDisplay *display, XEvent *event) {
  ...
  screen = find_screen_for_selection(display, event->xselectionclear.window,
  ...
  if (screen != NULL) {
    ...
    meta_display_unmanage_screen(display, screen, event->xselectionclear.time);
...

  {
    ...
    meta_error_trap_push(display);
    str = XGetAtomName(display->xdisplay, event->xselectionclear.selection);
    meta_error_trap_pop(display, TRUE);

    meta_verbose(
        "Selection clear with selection %s window 0x%lx not a WM_Sn selection "
        "we recognize\n",
...
void meta_display_unmanage_screen(MetaDisplay *display, MetaScreen *screen,
                                  guint32 timestamp) {
  ...
  if (display->screens == NULL) meta_display_close(display, timestamp);
}
...
void meta_display_devirtualize_modifiers(MetaDisplay *display,
  ...
  *mask = 0;
  ...
  if (modifiers & META_VIRTUAL_SHIFT_MASK) *mask |= ShiftMask;
  if (modifiers & META_VIRTUAL_CONTROL_MASK) *mask |= ControlMask;
  if (modifiers & META_VIRTUAL_ALT_MASK) *mask |= Mod1Mask;
  if (modifiers & META_VIRTUAL_META_MASK) *mask |= display->meta_mask;
  if (modifiers & META_VIRTUAL_HYPER_MASK) *mask |= display->hyper_mask;
  if (modifiers & META_VIRTUAL_SUPER_MASK) *mask |= display->super_mask;
  if (modifiers & META_VIRTUAL_MOD2_MASK) *mask |= Mod2Mask;
  if (modifiers & META_VIRTUAL_MOD3_MASK) *mask |= Mod3Mask;
  if (modifiers & META_VIRTUAL_MOD4_MASK) *mask |= Mod4Mask;
  if (modifiers & META_VIRTUAL_MOD5_MASK) *mask |= Mod5Mask;
}
...

{
  ...
  meta_display_devirtualize_modifiers(display, virtual_mods, &mods);
...
static void prefs_changed_callback(MetaPreference pref, void *data) {
   ...
   */
  if (pref == META_PREF_MOUSE_BUTTON_MODS || pref == META_PREF_FOCUS_MODE) {
    GSList *windows;
    GSList *tmp;
...
void meta_display_increment_focus_sentinel(MetaDisplay *display) {
  ...
  XChangeProperty(display->xdisplay,
                  ...
                  display->atom__MARCO_SENTINEL, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, 1);
...
void meta_display_decrement_focus_sentinel(MetaDisplay *display) {
  ...
  if (display->sentinel_counter < 0) display->sentinel_counter = 0;
}
...
static void sanity_check_timestamps(MetaDisplay *display, guint32 timestamp) {
...
static gboolean timestamp_too_old(MetaDisplay *display, MetaWindow *window,
                                  guint32 *timestamp) {
  ...
  if (*timestamp == CurrentTime) {
    meta_warning(
        "Got a request to focus %s with a timestamp of 0.  This "
        "shouldn't happen!\n",
    ...
    *timestamp = meta_display_get_current_time_roundtrip(display);
    ...
    if (XSERVER_TIME_IS_BEFORE(*timestamp, display->last_user_time)) {
      meta_topic(META_DEBUG_FOCUS,
                 ...
                 display->last_user_time, display->last_focus_time);
      return TRUE;
    } else {
      meta_topic(META_DEBUG_FOCUS,
                 ...
                 window ? window->desc : "the no_focus_window",
                 display->last_user_time, *timestamp, display->last_focus_time);
...
void meta_display_set_input_focus_window(MetaDisplay *display,
                                         ...
                                         guint32 timestamp) {
  if (timestamp_too_old(display, window, &timestamp)) return;
...
void meta_display_focus_the_no_focus_window(MetaDisplay *display,
                                            MetaScreen *screen,
                                            guint32 timestamp) {
  if (timestamp_too_old(display, NULL, &timestamp)) return;

  XSetInputFocus(display->xdisplay, screen->no_focus_window,
                 RevertToPointerRoot, timestamp);
  display->expected_focus_window = NULL;
  display->last_focus_time = timestamp;
