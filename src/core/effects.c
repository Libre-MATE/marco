...
static MetaEffect *create_effect(MetaEffectType type, MetaWindow *window,
                                 MetaEffectFinished finished,
                                 gpointer finished_data);

static void draw_box_animation(MetaScreen *screen, MetaRectangle *initial_rect,
...
void meta_effect_run_close(MetaWindow *window, MetaEffectFinished finished,
  ...
  effect = create_effect(META_EFFECT_CLOSE, window, finished, data);
...
#ifdef HAVE_SHAPE
static void update_wireframe_window(MetaDisplay *display, Window xwindow,
                                    const MetaRectangle *rect) {
  XMoveResizeWindow(display->xdisplay, xwindow, rect->x, rect->y, rect->width,
                    rect->height);

#define OUTLINE_WIDTH 3

  if (rect->width > OUTLINE_WIDTH * 2 && rect->height > OUTLINE_WIDTH * 2) {
    XRectangle xrect;
    ...
    XShapeCombineRegion(display->xdisplay, xwindow, ShapeBounding, 0, 0,
                        outer_xregion, ShapeSet);

    XDestroyRegion(outer_xregion);
    XDestroyRegion(inner_xregion);
  } else {
    /* Unset the shape */
    XShapeCombineMask(display->xdisplay, xwindow, ShapeBounding, 0, 0, None,
                      ShapeSet);
  }
}
#endif

static gboolean effects_draw_box_animation_timeout(
  ...
  if (!context->first_time) {
    ...
    XDrawRectangle(context->screen->display->xdisplay, context->screen->xroot,
                   context->gc, context->last_rect.x, context->last_rect.y,
  ...
  if (elapsed > context->millisecs_duration) {
    ...
    XFreeGC(context->screen->display->xdisplay, context->gc);
#endif /* !HAVE_SHAPE */
  ...
  draw_rect.x +=
      (int)(fraction * (double)(context->end_rect.x - context->start_rect.x));
  draw_rect.y +=
      (int)(fraction * (double)(context->end_rect.y - context->start_rect.y));
  draw_rect.width += (int)(fraction * (double)(context->end_rect.width -
                                               context->start_rect.width));
  draw_rect.height += (int)(fraction * (double)(context->end_rect.height -
                                                context->start_rect.height));

  /* don't confuse X or gdk-pixbuf with bogus rectangles */
  if (draw_rect.width < 1) draw_rect.width = 1;
  if (draw_rect.height < 1) draw_rect.height = 1;

#ifdef HAVE_SHAPE
  update_wireframe_window(context->screen->display, context->wireframe_xwindow,
                          &draw_rect);
#else
  ...
  /* Draw the rectangle */
  XDrawRectangle(context->screen->display->xdisplay, context->screen->xroot,
                 context->gc, draw_rect.x, draw_rect.y, draw_rect.width,
                 draw_rect.height);
...
void draw_box_animation(MetaScreen *screen, MetaRectangle *initial_rect,
                        MetaRectangle *destination_rect,
                        double seconds_duration) {
  BoxAnimationContext *context;
  ...
  attrs.background_pixel =
      BlackPixel(screen->display->xdisplay, screen->number);

  context->wireframe_xwindow =
      XCreateWindow(screen->display->xdisplay, screen->xroot, initial_rect->x,
                    initial_rect->y, initial_rect->width, initial_rect->height,
                    0, CopyFromParent, CopyFromParent, (Visual *)CopyFromParent,
                    CWOverrideRedirect | CWBackPixel, &attrs);

  update_wireframe_window(screen->display, context->wireframe_xwindow,
                          initial_rect);

  XMapWindow(screen->display->xdisplay, context->wireframe_xwindow);
  ...
  context->gc = XCreateGC(screen->display->xdisplay, screen->xroot,
                          GCSubwindowMode | GCFunction, &gc_values);
   ...
   */
  g_timeout_add(15, (GSourceFunc)effects_draw_box_animation_timeout, context);
...
void meta_effects_begin_wireframe(MetaScreen *screen, const MetaRectangle *rect,
  ...
  meta_effects_update_wireframe(screen, NULL, -1, -1, rect, width, height);
}

static void draw_xor_rect(MetaScreen *screen, const MetaRectangle *rect,
                          int width, int height) {
...
#define LINE_WIDTH META_WIREFRAME_XOR_LINE_WIDTH
  ...
  XDrawRectangle(screen->display->xdisplay, screen->xroot, screen->root_xor_gc,
                 shrunk_rect.x, shrunk_rect.y, shrunk_rect.width,
                 shrunk_rect.height);
  ...
  if ((width >= 0) && (height >= 0)) {
    ...
    if (XGetGCValues(screen->display->xdisplay, screen->root_xor_gc, GCFont,
      ...
      font_struct = XQueryFont(screen->display->xdisplay, gc_values.font);
      ...
      if (font_struct != NULL) {
        ...
        text_height =
            font_struct->max_bounds.descent + font_struct->max_bounds.ascent;
        ...
        if ((box_width < shrunk_rect.width) &&
          ...
          XFillRectangle(screen->display->xdisplay, screen->xroot,
                         screen->root_xor_gc, box_x, box_y, box_width,
                         box_height);
          XDrawString(screen->display->xdisplay, screen->xroot,
                      screen->root_xor_gc, box_x + LINE_WIDTH,
                      box_y + LINE_WIDTH + font_struct->max_bounds.ascent, text,
                      text_length);
        }
        ...
        if ((box_width + LINE_WIDTH) >= (shrunk_rect.width / 3)) return;

        if ((box_height + LINE_WIDTH) >= (shrunk_rect.height / 3)) return;
      }
    }
  }
  ...
  XDrawSegments(screen->display->xdisplay, screen->xroot, screen->root_xor_gc,
                segments, G_N_ELEMENTS(segments));
}
...
void meta_effects_update_wireframe(MetaScreen *screen,
                                   ...
                                   int new_height) {
  if (old_rect) draw_xor_rect(screen, old_rect, old_width, old_height);

  if (new_rect) draw_xor_rect(screen, new_rect, new_width, new_height);
...
void meta_effects_end_wireframe(MetaScreen *screen,
                                const MetaRectangle *old_rect, int old_width,
                                int old_height) {
  meta_effects_update_wireframe(screen, old_rect, old_width, old_height, NULL,
                                -1, -1);
...
static void run_default_effect_handler(MetaEffect *effect) {
  switch (effect->type) {
    case META_EFFECT_MINIMIZE:
      draw_box_animation(
          effect->window->screen, &(effect->u.minimize.window_rect),
          &(effect->u.minimize.icon_rect), META_MINIMIZE_ANIMATION_LENGTH);
      break;
...
static void run_handler(MetaEffect *effect) {
  if (meta_prefs_get_mate_animations()) run_default_effect_handler(effect);
