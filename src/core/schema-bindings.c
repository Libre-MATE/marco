...
static void single_stanza(gboolean is_window, const char *name,
                          const char *default_value, gboolean can_reverse,
...
static void single_stanza(gboolean is_window, const char *name,
  ...
  escaped_default_value = default_value == NULL
                              ? "disabled"
                              : g_markup_escape_text(default_value, -1);

  fprintf(target_file, "    <schema>\n");
  fprintf(target_file,
          "      <key>/schemas/apps/marco/%s_keybindings/%s</key>\n",
          keybinding_type, name);
  fprintf(target_file,
          "      <applyto>/apps/marco/%s_keybindings/%s</applyto>\n",
          keybinding_type, name);
  ...
  fprintf(target_file, "        <long>%s</long>\n",
          can_reverse ? about_reversible_keybindings : about_keybindings);
  fprintf(target_file, "      </locale>\n");
  fprintf(target_file, "    </schema>\n\n");

  g_free(escaped_description);

  if (default_value != NULL) g_free(escaped_default_value);
}
...
static void produce_bindings() {
  ...
  while (fgets(buffer, sizeof(buffer), source_file)) {
    if (strstr(buffer, "<!-- GENERATED -->")) break;
...
#define keybind(name, handler, param, flags, stroke, description) \
  single_stanza(flags &BINDING_PER_WINDOW, #name, stroke,         \
                flags &BINDING_REVERSES, description);
...
int main(int argc, char **argv) {
   ...
   */
  about_keybindings = g_markup_escape_text(
      _("The format looks like \"<Control>a\" or \"<Shift><Alt>F1\".\n\n"
  ...
  about_reversible_keybindings = g_markup_escape_text(
      _("The format looks like \"<Control>a\" or \"<Shift><Alt>F1\".\n\n"
        "The parser is fairly liberal and allows "
        "lower or upper case, and also abbreviations such as \"<Ctl>\" and "
        "\"<Ctrl>\". If you set the option to the special string "
