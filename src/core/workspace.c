...
static void focus_ancestor_or_top_window(MetaWorkspace *workspace,
                                         ...
                                         guint32 timestamp);
static void free_this(gpointer candidate, gpointer dummy);
static void workspace_free_struts(MetaWorkspace *workspace);

static void maybe_add_to_list(MetaScreen *screen, MetaWindow *window,
                              gpointer data) {
  GList **mru_list = data;

  if (window->on_all_workspaces) *mru_list = g_list_prepend(*mru_list, window);
}
...
static void free_this(gpointer candidate, gpointer dummy) { g_free(candidate); }
 ...
 */
static void workspace_free_struts(MetaWorkspace *workspace) {
  if (workspace->all_struts == NULL) return;
...
void meta_workspace_add_window(MetaWorkspace *workspace, MetaWindow *window) {
  ...
  if (window->struts) {
    meta_topic(META_DEBUG_WORKAREA,
               "Invalidating work area of workspace %d since we're adding "
               "window %s to it\n",
...
void meta_workspace_remove_window(MetaWorkspace *workspace,
  ...
  if (window->struts) {
    meta_topic(META_DEBUG_WORKAREA,
               "Invalidating work area of workspace %d since we're removing "
               "window %s from it\n",
               meta_workspace_index(workspace), window->desc);
    meta_workspace_invalidate_work_area(workspace);
  }
...
static void workspace_switch_sound(MetaWorkspace *from, MetaWorkspace *to) {
  ...
  ca_context_play(ca_gtk_context_get(), 1, CA_PROP_EVENT_ID, e,
...
void meta_workspace_activate_with_focus(MetaWorkspace *workspace,
  ...
  if (workspace->screen->active_workspace == workspace) return;
  ...
  if (move_window) /* Removes window from other spaces */
...
int meta_workspace_index(MetaWorkspace *workspace) {
  ...
  if (ret < 0) meta_bug("Workspace does not exist to index!\n");
...
static void set_active_space_hint(MetaScreen *screen) {
  ...
  XChangeProperty(screen->display->xdisplay, screen->xroot,
                  screen->display->atom__NET_CURRENT_DESKTOP, XA_CARDINAL, 32,
...
void meta_workspace_invalidate_work_area(MetaWorkspace *workspace) {
  ...
  meta_topic(META_DEBUG_WORKAREA, "Invalidating work area for workspace %d\n",
             meta_workspace_index(workspace));
...
static void ensure_work_areas_validated(MetaWorkspace *workspace) {
  ...
  if (!workspace->work_areas_invalid) return;
  ...
  for (tmp = windows; tmp != NULL; tmp = tmp->next) {
    ...
    for (s_iter = win->struts; s_iter != NULL; s_iter = s_iter->next) {
      ...
      *cpy = *((MetaStrut *)s_iter->data);
      workspace->all_struts = g_slist_prepend(workspace->all_struts, cpy);
    }
  }
  g_list_free(windows);
  ...
  workspace->xinerama_region =
      g_new(GList *, workspace->screen->n_xinerama_infos);
  for (i = 0; i < workspace->screen->n_xinerama_infos; i++) {
    workspace->xinerama_region[i] =
        meta_rectangle_get_minimal_spanning_set_for_region(
            &workspace->screen->xinerama_infos[i].rect, workspace->all_struts,
            FALSE);
  }
  workspace->screen_region = meta_rectangle_get_minimal_spanning_set_for_region(
      &workspace->screen->rect, workspace->all_struts, TRUE);
...
#define MIN_SANE_AREA 100
  ...
  meta_topic(META_DEBUG_WORKAREA,
             ...
             meta_workspace_index(workspace), workspace->work_area_screen.x,
             workspace->work_area_screen.y, workspace->work_area_screen.width,
             workspace->work_area_screen.height);

  /* Now find the work areas for each xinerama */
  g_free(workspace->work_area_xinerama);
  workspace->work_area_xinerama =
      g_new(MetaRectangle, workspace->screen->n_xinerama_infos);
  ...
  for (i = 0; i < workspace->screen->n_xinerama_infos; i++) {
    ...
    meta_topic(META_DEBUG_WORKAREA,
               ...
               meta_workspace_index(workspace), i,
  ...
  g_assert(workspace->screen_edges == NULL);
  g_assert(workspace->xinerama_edges == NULL);
  workspace->screen_edges = meta_rectangle_find_onscreen_edges(
      &workspace->screen->rect, workspace->all_struts);
  tmp = NULL;
  for (i = 0; i < workspace->screen->n_xinerama_infos; i++)
    tmp = g_list_prepend(tmp, &workspace->screen->xinerama_infos[i].rect);
  workspace->xinerama_edges = meta_rectangle_find_nonintersected_xinerama_edges(
      &workspace->screen->rect, tmp, workspace->all_struts);
  g_list_free(tmp);
...
MetaWorkspace *meta_workspace_get_neighbor(MetaWorkspace *workspace,
  ...
  /* LEFT */
  if (layout.current_col < 0) switch (wrap) {
      case META_WRAP_NONE:
        layout.current_col = 0;
        break;
      case META_WRAP_CLASSIC:
        layout.current_row =
            layout.current_row > 0 ? layout.current_row - 1 : layout.rows - 1;
        /* fall through */
      case META_WRAP_TOROIDAL:
        layout.current_col = layout.cols - 1;
    }
  /* RIGHT */
  if (layout.current_col >= layout.cols) switch (wrap) {
      case META_WRAP_NONE:
        layout.current_col = layout.cols - 1;
        break;
      case META_WRAP_CLASSIC:
        layout.current_row =
            layout.current_row < layout.rows - 1 ? layout.current_row + 1 : 0;
        /* fall through */
      case META_WRAP_TOROIDAL:
        layout.current_col = 0;
    }
  /* UP */
  if (layout.current_row < 0) switch (wrap) {
      case META_WRAP_NONE:
        layout.current_row = 0;
        break;
      case META_WRAP_CLASSIC:
        layout.current_col =
            layout.current_col > 0 ? layout.current_col - 1 : layout.cols - 1;
        /* fall through */
      case META_WRAP_TOROIDAL:
        layout.current_row = layout.rows - 1;
    }
  /* DOWN */
  if (layout.current_row >= layout.rows) switch (wrap) {
      case META_WRAP_NONE:
        layout.current_row = layout.rows - 1;
        break;
      case META_WRAP_CLASSIC:
        layout.current_col =
            layout.current_col < layout.cols - 1 ? layout.current_col + 1 : 0;
        /* fall through */
      case META_WRAP_TOROIDAL:
        layout.current_row = 0;
    }

  /* If we have an uneven arrangement of workspaces, (layout.cols - n,
     layout.rows - 1) may be an invalid workspace e.g. we have 7 workspaces on a
     3x3 pane */
  if (wrap != META_WRAP_NONE &&
      (layout.current_row * layout.cols + layout.current_col >= num_workspaces))
    switch (direction) {
      case META_MOTION_LEFT:
        layout.current_col =
            num_workspaces - (layout.current_row * layout.cols + 1);
        break;
      case META_MOTION_RIGHT:
        layout.current_col = 0;
        if (wrap == META_WRAP_CLASSIC) layout.current_row = 0;
        break;
      case META_MOTION_UP:
        layout.current_row -= 1;
        break;
  ...
  if (i < 0) i = current_space;

  if (i >= num_workspaces)
    meta_bug(
        "calc_workspace_layout left an invalid (too-high) workspace number %d "
        "in the grid\n",
        i);

  meta_verbose("Neighbor workspace is %d at row %d col %d\n", i,
               layout.current_row, layout.current_col);
...
void meta_workspace_focus_default_window(MetaWorkspace *workspace,
  ...
  if (timestamp == CurrentTime) {
    meta_warning(
        "CurrentTime used to choose focus window; "
    ...
    if (window && window->type != META_WINDOW_DOCK &&
      ...
      if (timestamp == CurrentTime) {
        ...
        meta_topic(META_DEBUG_FOCUS,
                   "Not focusing mouse window %s because EnterNotify events "
                   "should handle that\n",
                   window->desc);
      } else {
        meta_topic(META_DEBUG_FOCUS, "Focusing mouse window %s\n",
                   window->desc);
...
static void focus_ancestor_or_top_window(MetaWorkspace *workspace,
  ...
  if (not_this_one)
    meta_topic(META_DEBUG_FOCUS, "Focusing MRU window excluding %s\n",
               not_this_one->desc);
  else
    meta_topic(META_DEBUG_FOCUS, "Focusing MRU window\n");
  ...
  if (not_this_one) {
    ...
    if (ancestor != NULL) {
      meta_topic(META_DEBUG_FOCUS, "Focusing %s, ancestor of %s\n",
  ...
  if (window) {
    meta_topic(META_DEBUG_FOCUS, "Focusing workspace MRU window %s\n",
               window->desc);
