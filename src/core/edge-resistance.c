...
#define WINDOW_EDGES_RELEVANT(window, display)           \
  meta_window_should_be_showing(window) &&               \
      window->screen == display->grab_screen &&window != \
          display->grab_window &&                        \
 ...
 */
static int find_index_of_edge_near_position(const GArray *edges, int position,
  ...
  if (want_interval_min) {
    ...
    if (compare < position) return edges->len;
...
static int find_nearest_position(const GArray *edges, int position,
                                 int old_position,
                                 const MetaRectangle *new_rect,
                                 gboolean horizontal, gboolean only_forward) {
  ...
  for (i = mid + 1; i < (int)edges->len; i++) {
    ...
    if (edges_align && (!only_forward || !points_on_same_side(position, compare,
                                                              old_position))) {
      int dist = ABS(compare - position);
      if (dist < best_dist) {
        best = compare;
...
static gboolean movement_towards_edge(MetaSide side, int increment) {
  switch (side) {
    case META_SIDE_LEFT:
    case META_SIDE_TOP:
      return increment < 0;
...
static int apply_edge_resistance(MetaWindow *window, int old_pos, int new_pos,
                                 const MetaRectangle *old_rect,
                                 const MetaRectangle *new_rect, GArray *edges,
                                 ResistanceDataForAnEdge *resistance_data,
                                 GSourceFunc timeout_func, gboolean xdir,
  ...
  i = begin;
  while ((increasing && i <= end) || (!increasing && i >= end)) {
    gboolean edges_align;
    ...
    } else /* mouse op */
    {
      ...
      if (movement_towards_edge(edge->side_type, increment)) {
        ...
        if (!resistance_data->timeout_setup && timeout_length_ms != 0) {
          resistance_data->timeout_id = g_timeout_add(
              timeout_length_ms, edge_resistance_timeout, resistance_data);
        ...
        if (!resistance_data->timeout_over && timeout_length_ms != 0)
          return compare;
      }
      ...
      if (ABS(compare - new_pos) < threshold) return compare;
    }
...
static int apply_edge_snapping(int old_pos, int new_pos,
                               const MetaRectangle *new_rect, GArray *edges,
                               gboolean xdir, gboolean keyboard_op) {
  int snap_to;

  if (old_pos == new_pos) return new_pos;

  snap_to = find_nearest_position(edges, new_pos, old_pos, new_rect, xdir,
                                  keyboard_op);
   ...
   * anyway.
   */
  if (!keyboard_op && ABS(snap_to - old_pos) >= 8 && ABS(new_pos - old_pos) < 8)
    return old_pos;
  else
 ...
 */
static gboolean apply_edge_resistance_to_each_side(
    MetaDisplay *display, MetaWindow *window, const MetaRectangle *old_outer,
    MetaRectangle *new_outer, GSourceFunc timeout_func, gboolean auto_snap,
  ...
  if (auto_snap) {
    ...
    new_left = apply_edge_snapping(BOX_LEFT(*old_outer), BOX_LEFT(*new_outer),
                                   new_outer, edge_data->left_edges, TRUE,
                                   keyboard_op);

    new_right = apply_edge_snapping(BOX_RIGHT(*old_outer),
                                    BOX_RIGHT(*new_outer), new_outer,
                                    edge_data->right_edges, TRUE, keyboard_op);

    new_top =
        apply_edge_snapping(BOX_TOP(*old_outer), BOX_TOP(*new_outer), new_outer,
                            edge_data->top_edges, FALSE, keyboard_op);

    new_bottom = apply_edge_snapping(
        BOX_BOTTOM(*old_outer), BOX_BOTTOM(*new_outer), new_outer,
        edge_data->bottom_edges, FALSE, keyboard_op);
  } else {
    ...
    if (!is_resize || window->size_hints.width_inc == 1) {
      ...
      new_left = apply_edge_resistance(
          window, BOX_LEFT(*old_outer), BOX_LEFT(*new_outer), old_outer,
          new_outer, edge_data->left_edges, &edge_data->left_data, timeout_func,
          TRUE, keyboard_op);
      new_right = apply_edge_resistance(
          window, BOX_RIGHT(*old_outer), BOX_RIGHT(*new_outer), old_outer,
          new_outer, edge_data->right_edges, &edge_data->right_data,
          timeout_func, TRUE, keyboard_op);
    } else {
    ...
    if (!is_resize || window->size_hints.height_inc == 1) {
      new_top = apply_edge_resistance(
          window, BOX_TOP(*old_outer), BOX_TOP(*new_outer), old_outer,
          new_outer, edge_data->top_edges, &edge_data->top_data, timeout_func,
          FALSE, keyboard_op);
      new_bottom = apply_edge_resistance(
          window, BOX_BOTTOM(*old_outer), BOX_BOTTOM(*new_outer), old_outer,
          new_outer, edge_data->bottom_edges, &edge_data->bottom_data,
          timeout_func, FALSE, keyboard_op);
    } else {
      new_top = new_outer->y;
  ...
  modified_rect =
      meta_rect(new_left, new_top, new_right - new_left, new_bottom - new_top);
...
void meta_display_cleanup_edges(MetaDisplay *display) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    for (j = 0; j < tmp->len; j++) {
      ...
      if (edge->edge_type == META_EDGE_WINDOW && edge->side_type == side) {
...
static void cache_edges(MetaDisplay *display, GList *window_edges,
                        GList *xinerama_edges, GList *screen_edges) {
  MetaEdgeResistanceData *edge_data;
  GList *tmp;
  ...
  if (meta_is_verbose()) {
    int max_edges =
        MAX(MAX(g_list_length(window_edges), g_list_length(xinerama_edges)),
  ...
  edge_data->left_edges =
      g_array_sized_new(FALSE, FALSE, sizeof(MetaEdge *), num_left + num_right);
  edge_data->right_edges =
      g_array_sized_new(FALSE, FALSE, sizeof(MetaEdge *), num_left + num_right);
  edge_data->top_edges =
      g_array_sized_new(FALSE, FALSE, sizeof(MetaEdge *), num_top + num_bottom);
...
static void compute_resistance_and_snapping_edges(MetaDisplay *display) {
   ...
   */
  stacked_windows = meta_stack_list_windows(
      display->grab_screen->stack, display->grab_screen->active_workspace);
  ...
  while (cur_window_iter != NULL) {
    ...
    if (WINDOW_EDGES_RELEVANT(cur_window, display) &&
       ...
       */
      meta_rectangle_intersect(&cur_rect, &display->grab_screen->rect,
                               &reduced);
      ...
      new_edges = meta_rectangle_remove_intersections_with_boxes_from_edges(
          new_edges, rem_windows);
 ...
 * the window.
 */
void meta_window_edge_resistance_for_move(MetaWindow *window, int old_x,
  ...
  is_resize = FALSE;
  if (apply_edge_resistance_to_each_side(window->display, window, &old_outer,
                                         &new_outer, timeout_func, snap,
                                         is_keyboard_op, is_resize)) {
    ...
    *new_x =
        old_x + smaller_x_change + (BOX_LEFT(*reference) - BOX_LEFT(old_outer));
    *new_y =
        old_y + smaller_y_change + (BOX_TOP(*reference) - BOX_TOP(old_outer));

    meta_topic(META_DEBUG_EDGE_RESISTANCE,
               "outer x & y move-to coordinate changed from %d,%d to %d,%d\n",
               proposed_outer.x, proposed_outer.y,
               old_outer.x + (*new_x - old_x), old_outer.y + (*new_y - old_y));
  }
}
 ...
 */
void meta_window_edge_resistance_for_resize(MetaWindow *window, int old_width,
                                            int old_height, int *new_width,
                                            int *new_height, int gravity,
                                            GSourceFunc timeout_func,
                                            gboolean snap,
                                            gboolean is_keyboard_op) {
  ...
  meta_rectangle_resize_with_gravity(&old_outer, &new_outer, gravity,
  ...
  if (apply_edge_resistance_to_each_side(window->display, window, &old_outer,
    ...
    *new_height = old_height + (new_outer.height - old_outer.height);
    ...
    meta_topic(META_DEBUG_EDGE_RESISTANCE,
               ...
               proposed_outer_width, proposed_outer_height, new_outer.width,
               new_outer.height);
  }
}
