...
char *meta_rectangle_to_string(const MetaRectangle *rect, char *output) {
   ...
   */
  g_snprintf(output, RECT_LENGTH, "%d,%d +%d,%d", rect->x, rect->y, rect->width,
             rect->height);
...
char *meta_rectangle_region_to_string(GList *region,
  ...
  if (region == NULL) g_snprintf(output, 10, "(EMPTY)");

  while (tmp) {
    MetaRectangle *rect = tmp->data;
    g_snprintf(rect_string, RECT_LENGTH, "[%d,%d +%d,%d]", rect->x, rect->y,
               rect->width, rect->height);
    cur = g_stpcpy(cur, rect_string);
    tmp = tmp->next;
    if (tmp) cur = g_stpcpy(cur, separator_string);
  }
...
char *meta_rectangle_edge_to_string(const MetaEdge *edge, char *output) {
   ...
   */
  g_snprintf(output, EDGE_LENGTH, "[%d,%d +%d,%d], %2d, %2d", edge->rect.x,
             edge->rect.y, edge->rect.width, edge->rect.height, edge->side_type,
             edge->edge_type);
...
char *meta_rectangle_edge_list_to_string(GList *edge_list,
  ...
  if (edge_list == NULL) g_snprintf(output, 10, "(EMPTY)");

  while (tmp) {
    MetaEdge *edge = tmp->data;
    MetaRectangle *rect = &edge->rect;
    g_snprintf(rect_string, EDGE_LENGTH, "([%d,%d +%d,%d], %2d, %2d)", rect->x,
               rect->y, rect->width, rect->height, edge->side_type,
               edge->edge_type);
    cur = g_stpcpy(cur, rect_string);
    tmp = tmp->next;
    if (tmp) cur = g_stpcpy(cur, separator_string);
...
gboolean meta_rectangle_contains_rect(const MetaRectangle *outer_rect,
                                      const MetaRectangle *inner_rect) {
  return inner_rect->x >= outer_rect->x && inner_rect->y >= outer_rect->y &&
...
static GList *merge_spanning_rects_in_region(GList *region) {
  ...
  if (region == NULL) {
    meta_warning(
        "Region to merge was empty!  Either you have a some "
  ...
  *temp_rect = *basic_rect;
  ...
  for (strut_iter = all_struts; strut_iter; strut_iter = strut_iter->next) {
    ...
    if (skip_middle_struts && strut->edge == META_EDGE_XINERAMA) {
      if ((strut->side == META_SIDE_LEFT && strut_rect->x != basic_rect->x) ||
          (strut->side == META_SIDE_RIGHT &&
           strut_rect->x + strut_rect->width != basic_rect->width) ||
          (strut->side == META_SIDE_TOP && strut_rect->y != basic_rect->y) ||
...
GList *meta_rectangle_expand_region(GList *region, const int left_expand,
  ...
  return meta_rectangle_expand_region_conditionally(
      region, left_expand, right_expand, top_expand, bottom_expand, 0, 0);
}
...
void meta_rectangle_expand_to_avoiding_struts(MetaRectangle *rect,
  ...
  for (strut_iter = all_struts; strut_iter; strut_iter = strut_iter->next) {
    ...
    if (!meta_rectangle_overlap(&strut->rect, rect)) continue;
...
void meta_rectangle_clamp_to_fit_into_region(const GList *spanning_rects,
  ...
  if (best_rect == NULL) {
    ...
    if (!(fixed_directions & FIXED_DIRECTION_X)) rect->width = min_size->width;
    if (!(fixed_directions & FIXED_DIRECTION_Y))
...
void meta_rectangle_shove_into_region(const GList *spanning_rects,
  ...
  for (temp = spanning_rects; temp; temp = temp->next) {
    ...
    dist_to_compare = 0;
    if (compare_rect->x > rect->x) dist_to_compare += compare_rect->x - rect->x;
    if (compare_rect->x + compare_rect->width < rect->x + rect->width)
      dist_to_compare +=
          (rect->x + rect->width) - (compare_rect->x + compare_rect->width);
    if (compare_rect->y > rect->y) dist_to_compare += compare_rect->y - rect->y;
    if (compare_rect->y + compare_rect->height < rect->y + rect->height)
      dist_to_compare +=
          (rect->y + rect->height) - (compare_rect->y + compare_rect->height);
  ...
  if (best_rect == NULL)
    ...
    if (!(fixed_directions & FIXED_DIRECTION_X)) {
      ...
      if (best_rect->x > rect->x) rect->x = best_rect->x;
    ...
    if (!(fixed_directions & FIXED_DIRECTION_Y)) {
      ...
      if (best_rect->y > rect->y) rect->y = best_rect->y;
...
void meta_rectangle_find_linepoint_closest_to_point(double x1, double y1,
  ...
  *valx = (py * diffx * diffy + px * diffx * diffx + y2 * x1 * diffy -
           y1 * x2 * diffy) /
          den;
  *valy = (px * diffx * diffy + py * diffy * diffy + x2 * y1 * diffx -
           x1 * y2 * diffx) /
          den;
}
 ...
 */
static GList *get_disjoint_strut_rect_list_in_region(
    const GSList *old_struts, const MetaRectangle *region) {
  GList *strut_rects;
  GList *tmp;
...
gint meta_rectangle_edge_cmp(gconstpointer a, gconstpointer b) {
  ...
  if (a_compare == b_compare) return meta_rectangle_edge_cmp_ignore_type(a, b);
 ...
 * TOP<->BOTTOM).
 */
static GList *add_edges(GList *cur_edges, const MetaRectangle *rect,
 ...
 */
static GList *split_edge(GList *cur_list, const MetaEdge *old_edge,
  ...
  switch (old_edge->side_type) {
    ...
    case META_SIDE_RIGHT:
      ...
      if (BOX_TOP(old_edge->rect) < BOX_TOP(remove->rect)) {
        temp_edge = g_new(MetaEdge, 1);
        *temp_edge = *old_edge;
        temp_edge->rect.height =
            BOX_TOP(remove->rect) - BOX_TOP(old_edge->rect);
      ...
      if (BOX_BOTTOM(old_edge->rect) > BOX_BOTTOM(remove->rect)) {
        ...
        *temp_edge = *old_edge;
        ...
        temp_edge->rect.height =
            BOX_BOTTOM(old_edge->rect) - BOX_BOTTOM(remove->rect);
        cur_list = g_list_prepend(cur_list, temp_edge);
      }
      break;
    case META_SIDE_TOP:
    case META_SIDE_BOTTOM:
      ...
      if (BOX_LEFT(old_edge->rect) < BOX_LEFT(remove->rect)) {
        temp_edge = g_new(MetaEdge, 1);
        *temp_edge = *old_edge;
        temp_edge->rect.width =
            BOX_LEFT(remove->rect) - BOX_LEFT(old_edge->rect);
        cur_list = g_list_prepend(cur_list, temp_edge);
      }
      if (BOX_RIGHT(old_edge->rect) > BOX_RIGHT(remove->rect)) {
        ...
        *temp_edge = *old_edge;
        ...
        temp_edge->rect.width =
            BOX_RIGHT(old_edge->rect) - BOX_RIGHT(remove->rect);
        cur_list = g_list_prepend(cur_list, temp_edge);
      }
      break;
    default:
 ...
 */
static void fix_up_edges(MetaRectangle *rect, MetaEdge *edge,
...
GList *meta_rectangle_remove_intersections_with_boxes_from_edges(
  ...
  while (rect_iter) {
    ...
    while (edge_iter) {
      ...
      if (!edge_iter_advanced) edge_iter = edge_iter->next;
    }
...
GList *meta_rectangle_find_onscreen_edges(const MetaRectangle *basic_rect,
  ...
  while (strut_rect_iter) {
    ...
    while (edge_iter) {
      ...
      fix_up_edges(strut_rect, cur_edge, &new_strut_edges, &splits_of_cur_edge,
                   &edge_needs_removal);
...
GList *meta_rectangle_find_nonintersected_xinerama_edges(
  ...
  while (cur) {
    ...
    if (BOX_LEFT(*cur_rect) != BOX_LEFT(*screen_rect)) {
      ...
      new_edge->rect = meta_rect(BOX_LEFT(*cur_rect), BOX_TOP(*cur_rect), 0,
                                 cur_rect->height);
    ...
    if (BOX_RIGHT(*cur_rect) != BOX_RIGHT(*screen_rect)) {
      new_edge = g_new(MetaEdge, 1);
      new_edge->rect = meta_rect(BOX_RIGHT(*cur_rect), BOX_TOP(*cur_rect), 0,
                                 cur_rect->height);
    ...
    if (BOX_TOP(*cur_rect) != BOX_TOP(*screen_rect)) {
      new_edge = g_new(MetaEdge, 1);
      new_edge->rect = meta_rect(BOX_LEFT(*cur_rect), BOX_TOP(*cur_rect),
                                 cur_rect->width, 0);
    ...
    if (BOX_BOTTOM(*cur_rect) != BOX_BOTTOM(*screen_rect)) {
      new_edge = g_new(MetaEdge, 1);
      new_edge->rect = meta_rect(BOX_LEFT(*cur_rect), BOX_BOTTOM(*cur_rect),
                                 cur_rect->width, 0);
  ...
  for (; all_struts; all_struts = all_struts->next)
    temp_rects =
        g_slist_prepend(temp_rects, &((MetaStrut *)all_struts->data)->rect);
