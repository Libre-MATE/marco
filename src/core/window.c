...
static void update_resize(MetaWindow *window, gboolean snap, int x, int y,
                          gboolean force);

static MetaTileMode calculate_tiling_mode(int x, int y, MetaWindow *window,
...
static void maybe_leave_show_desktop_mode(MetaWindow *window) {
  ...
  if (!window->screen->active_workspace->showing_desktop) return;
  ...
  is_desktop_or_dock_foreach(window, &is_desktop_or_dock);

  meta_window_foreach_ancestor(window, is_desktop_or_dock_foreach,
                               &is_desktop_or_dock);

  if (!is_desktop_or_dock) {
    meta_screen_minimize_all_on_active_workspace_except(window->screen, window);
...
MetaWindow *meta_window_new(MetaDisplay *display, Window xwindow,
  ...
  if (XGetWindowAttributes(display->xdisplay, xwindow, &attrs)) {
    ...
    window =
        meta_window_new_with_attrs(display, xwindow, must_be_viewable, &attrs);
  } else {
...
#define N_INITIAL_PROPS 20
  ...
  meta_verbose("must_be_viewable = %d attrs->map_state = %d (%s)\n",
               must_be_viewable, attrs->map_state,
               (attrs->map_state == IsUnmapped)     ? "IsUnmapped"
  ...
  if (must_be_viewable && attrs->map_state != IsViewable) {
    ...
    if (!(meta_prop_get_cardinal_with_atom_type(
              display, xwindow, display->atom_WM_STATE, display->atom_WM_STATE,
              &state) &&
  ...
  event_mask = PropertyChangeMask | EnterWindowMask | LeaveWindowMask |
  ...
  if (attrs->win_gravity != NorthWestGravity) {
    ...
    XChangeWindowAttributes(display->xdisplay, xwindow, CWWinGravity,
                            &set_attrs);
  }
  ...
  if (!window->net_wm_user_time_set) {
    ...
    if (window->xtransient_for)
      parent =
          meta_display_lookup_x_window(window->display, window->xtransient_for);
  ...
  if (window->decorated) meta_window_ensure_frame(window);
  ...
  if (window->initial_workspace_set) {
    if (window->initial_workspace == (int)0xFFFFFFFF) {
      meta_topic(META_DEBUG_PLACEMENT, "Window %s is initially on all spaces\n",
                 window->desc);
      ...
      meta_topic(META_DEBUG_PLACEMENT, "Window %s is initially on space %d\n",
                 window->desc, window->initial_workspace);

      space = meta_screen_get_workspace_by_index(window->screen,
                                                 window->initial_workspace);

      if (space) meta_workspace_add_window(space, window);
    }
  }

  if (window->workspace == NULL && window->xtransient_for != None) {
    ...
    if (parent && parent->workspace) {
      ...
      if (parent->on_all_workspaces) window->on_all_workspaces = TRUE;
  ...
  if (window->workspace == NULL) {
    meta_topic(META_DEBUG_PLACEMENT, "Putting window %s on active workspace\n",
               window->desc);
  ...
  meta_window_move_resize_internal(
      window, flags, window->size_hints.win_gravity, window->size_hints.x,
      window->size_hints.y, window->size_hints.width,
   ...
   */
  meta_window_foreach_transient(window, queue_calc_showing_func, NULL);
...
/* This function should only be called from the end of
 * meta_window_new_with_attrs () */
static void meta_window_apply_session_info(MetaWindow *window,
                                           const MetaWindowSessionInfo *info) {
  if (info->stack_position_set) {
    meta_topic(META_DEBUG_SM, "Restoring stack position %d for window %s\n",
  ...
  if (info->minimized_set) {
    meta_topic(META_DEBUG_SM, "Restoring minimized state %d for window %s\n",
  ...
  if (info->maximized_set) {
    meta_topic(META_DEBUG_SM, "Restoring maximized state %d for window %s\n",
    ...
    if (window->has_maximize_func && info->maximized) {
      ...
      if (info->saved_rect_set) {
        meta_topic(
            META_DEBUG_SM, "Restoring saved rect %d,%d %dx%d for window %s\n",
            info->saved_rect.x, info->saved_rect.y, info->saved_rect.width,
            info->saved_rect.height, window->desc);
  ...
  if (info->on_all_workspaces_set) {
    ...
    meta_topic(META_DEBUG_SM, "Restoring sticky state %d for window %s\n",
  ...
  if (info->workspace_indices) {
    ...
    while (tmp != NULL) {
      ...
      if (space) spaces = g_slist_prepend(spaces, space);
    ...
    if (spaces) {
      ...
      meta_topic(META_DEBUG_SM, "Restoring saved window %s to workspace %d\n",
                 window->desc, meta_workspace_index(spaces->data));
  ...
  if (info->geometry_set) {
    ...
    meta_topic(META_DEBUG_SM, "Restoring pos %d,%d size %d x %d for %s\n", x, y,
               w, h, window->desc);

    flags =
        META_DO_GRAVITY_ADJUST | META_IS_MOVE_ACTION | META_IS_RESIZE_ACTION;
    meta_window_move_resize_internal(
        window, flags, window->size_hints.win_gravity, x, y, w, h);
  }
}
...
void meta_window_free(MetaWindow *window, guint32 timestamp) {
  ...
  if (window->fullscreen) {
    ...
    if (group) meta_group_update_layers(group);
    meta_stack_thaw(window->screen->stack);
  }
  ...
  meta_window_unqueue(window, META_QUEUE_CALC_SHOWING | META_QUEUE_MOVE_RESIZE |
  ...
  if (window->border_width != 0)
    XSetWindowBorderWidth(window->display->xdisplay, window->xwindow,
                          window->border_width);
  ...
  XSelectInput(window->display->xdisplay, window->xwindow, NoEventMask);

  /* Stop getting events for the window's _NET_WM_USER_TIME_WINDOW too */
  if (window->user_time_window != None) {
    meta_display_unregister_x_window(window->display, window->user_time_window);
    XSelectInput(window->display->xdisplay, window->user_time_window,
                 NoEventMask);
    window->user_time_window = None;
  }
...
static void set_wm_state(MetaWindow *window, int state) {
  ...
  XChangeProperty(window->display->xdisplay, window->xwindow,
                  ...
                  window->display->atom_WM_STATE, 32, PropModeReplace,
                  (guchar *)data, 2);
  meta_error_trap_pop(window->display, FALSE);
}
...
static void set_net_wm_state(MetaWindow *window) {
  ...
  meta_error_trap_push(window->display);
  XChangeProperty(window->display->xdisplay, window->xwindow,
                  window->display->atom__NET_WM_STATE, XA_ATOM, 32,
                  PropModeReplace, (guchar *)data, i);
  meta_error_trap_pop(window->display, FALSE);
...
gboolean meta_window_showing_on_its_workspace(MetaWindow *window) {
  ...
  if (window->minimized) showing = FALSE;
  ...
  if (showing && workspace_of_window && workspace_of_window->showing_desktop &&
      !is_desktop_or_dock) {
    meta_verbose(
  ...
  if (showing) {
    if (ancestor_is_minimized(window)) showing = FALSE;
  }
...
gboolean meta_window_should_be_showing(MetaWindow *window) {
  ...
  on_workspace = meta_window_located_on_workspace(
...
static gboolean idle_calc_showing(gpointer data) {
  ...
  meta_topic(META_DEBUG_WINDOW_STATE, "Clearing the calc_showing queue\n");
...
static void meta_window_unqueue(MetaWindow *window, guint queuebits) {
  ...
  for (queuenum = 0; queuenum < NUMBER_OF_QUEUES; queuenum++) {
    ...
    {
      meta_topic(META_DEBUG_WINDOW_STATE, "Removing %s from the %s queue\n",
                 window->desc, meta_window_queue_names[queuenum]);
...
void meta_window_queue(MetaWindow *window, guint queuebits) {
  ...
  for (queuenum = 0; queuenum < NUMBER_OF_QUEUES; queuenum++) {
    if (queuebits & 1 << queuenum) {
       ...
       * it on a queue.
       */
      if (window->unmanaging) break;

      /* If the window already claims to be in that queue, there's no
       * point putting it in the queue.
       */
      if (window->is_in_queues & 1 << queuenum) break;

      meta_topic(META_DEBUG_WINDOW_STATE, "Putting %s in the %s queue\n",
                 window->desc, meta_window_queue_names[queuenum]);
      ...
      if (queue_idle[queuenum] == 0)
        queue_idle[queuenum] =
            g_idle_add_full(window_queue_idle_priority[queuenum],
                            window_queue_idle_handler[queuenum],
                            GUINT_TO_POINTER(queuenum), NULL);

      /* And now we actually put it on the queue. */
      queue_pending[queuenum] =
          g_slist_prepend(queue_pending[queuenum], window);
    }
  }
}
...
static gboolean intervening_user_event_occurred(MetaWindow *window) {
  ...
  meta_topic(META_DEBUG_STARTUP,
             ...
             "  initial_timestamp    : %u\n",
             window->net_wm_user_time_set, window->net_wm_user_time,
             window->initial_timestamp_set, window->initial_timestamp);
  if (focus_window != NULL) {
    meta_topic(META_DEBUG_STARTUP,
               ...
               focus_window->desc, focus_window->net_wm_user_time_set,
  ...
  if ((window->net_wm_user_time_set && (window->net_wm_user_time == 0)) ||
    ...
    meta_topic(META_DEBUG_STARTUP, "window %s explicitly requested no focus\n",
               window->desc);
    return TRUE;
  }

  if (!window->net_wm_user_time_set && !window->initial_timestamp_set) {
    meta_topic(META_DEBUG_STARTUP, "no information about window %s found\n",
               window->desc);
    return FALSE;
  }

  if (focus_window != NULL && !focus_window->net_wm_user_time_set) {
    meta_topic(META_DEBUG_STARTUP,
  ...
  compare = 0;
  if (window->net_wm_user_time_set && window->initial_timestamp_set)
    compare = XSERVER_TIME_IS_BEFORE(window->net_wm_user_time,
  ...
  if ((focus_window != NULL) &&
    ...
    meta_topic(META_DEBUG_STARTUP,
               ...
               window->desc, compare, focus_window->net_wm_user_time);
    return TRUE;
  } else {
    meta_topic(META_DEBUG_STARTUP, "new window %s with no intervening events\n",
               window->desc);
    return FALSE;
  }
}
...
gboolean __window_is_terminal(MetaWindow *window) {
  if (window == NULL || window->res_class == NULL) return FALSE;
  ...
  if (strcmp(window->res_class, "Mate-terminal") == 0) return TRUE;
  /* xterm, rxvt, aterm */
 ...
 */
static void window_state_on_map(MetaWindow *window, gboolean *takes_focus,
...
static gboolean windows_overlap(const MetaWindow *w1, const MetaWindow *w2) {
  if (w1->minimized || w2->minimized) return FALSE;
...
void meta_window_show(MetaWindow *window) {
  ...
  meta_topic(META_DEBUG_WINDOW_STATE,
             ...
             window->desc, takes_focus_on_map ? "does" : "does not",
  ...
  if (focus_window != NULL && window->showing_for_first_time &&
      ((!place_on_top_on_map && !takes_focus_on_map) || will_be_covered)) {
    if (!meta_window_is_ancestor_of_transient(focus_window, window)) {
      needs_stacking_adjustment = TRUE;
      if (!window->placed) window->denied_focus_and_not_transient = TRUE;
    }
  }
  ...
  if (needs_stacking_adjustment) {
     ...
     */
    if (!will_be_covered &&
        (overlap || (meta_prefs_get_focus_mode() == META_FOCUS_MODE_CLICK &&
     ...
     */
    if (overlap || will_be_covered) window->wm_state_demands_attention = TRUE;
  }

  /* Shaded means the frame is mapped but the window is not */

  if (window->frame && !window->frame->mapped) {
    meta_topic(META_DEBUG_WINDOW_STATE, "Frame actually needs map\n");
  ...
  if (window->shaded) {
    if (window->mapped) {
      meta_topic(META_DEBUG_WINDOW_STATE, "%s actually needs unmap (shaded)\n",
                 window->desc);
      meta_topic(META_DEBUG_WINDOW_STATE,
    ...
    if (!window->mapped) {
      meta_topic(META_DEBUG_WINDOW_STATE, "%s actually needs map\n",
                 window->desc);
      window->mapped = TRUE;
      meta_error_trap_push(window->display);
      XMapWindow(window->display->xdisplay, window->xwindow);
      ...
      if (window->was_minimized) {
        ...
        if (meta_window_get_icon_geometry(window, &icon_rect)) {
          ...
          meta_effect_run_unminimize(window, &window_rect, &icon_rect, NULL,
                                     NULL);
        }
      }
    }
  ...
  if (window->showing_for_first_time) {
    ...
    if (takes_focus_on_map) {
      ...
      if (window->move_after_placement) {
        meta_window_begin_grab_op(window, META_GRAB_OP_KEYBOARD_MOVING, FALSE,
                                  timestamp);
        window->move_after_placement = FALSE;
      }
    } else {
...
static void meta_window_hide(MetaWindow *window) {
  ...
  meta_topic(META_DEBUG_WINDOW_STATE, "Hiding window %s\n", window->desc);
  ...
  if (window->mapped) {
    meta_topic(META_DEBUG_WINDOW_STATE, "%s actually needs unmap\n",
               window->desc);
    meta_topic(META_DEBUG_WINDOW_STATE,
...
static void meta_window_save_rect(MetaWindow *window) {
  if (!(META_WINDOW_MAXIMIZED(window) || META_WINDOW_SIDE_TILED(window) ||
    ...
    if (!window->maximized_horizontally) {
      ...
      if (window->frame) window->saved_rect.x += window->frame->rect.x;
    }
    if (!window->maximized_vertically) {
      window->saved_rect.y = window->rect.y;
      window->saved_rect.height = window->rect.height;
      if (window->frame) window->saved_rect.y += window->frame->rect.y;
    }
  }
}
...
void meta_window_maximize_internal(MetaWindow *window,
  ...
  meta_topic(META_DEBUG_WINDOW_OPS, "Maximizing %s%s\n", window->desc,
             maximize_horizontally && maximize_vertically ? ""
             : maximize_horizontally                      ? " horizontally"
             : maximize_vertically                        ? " vertically"
                                                          : "BUGGGGG");
   ...
   * size.
   */
  if (window->frame) window->frame->need_reapply_frame_shape = TRUE;
  ...
  if ((maximize_horizontally && !window->maximized_horizontally) ||
    ...
    if (!window->placed) {
      ...
      window->maximize_vertically_after_placement =
          window->maximize_vertically_after_placement || maximize_vertically;
      return;
    }
...
static void unmaximize_window_before_freeing(MetaWindow *window) {
  meta_topic(META_DEBUG_WINDOW_OPS, "Unmaximizing %s just before freeing\n",
             window->desc);
  ...
  } else if (window->screen->closing) /* See bug #358042 */
  {
     ...
     * before closing it. */
    meta_window_move_resize(window, FALSE, window->saved_rect.x,
                            window->saved_rect.y, window->saved_rect.width,
...
void meta_window_tile(MetaWindow *window) {
  ...
  if (window->tile_mode == META_TILE_NONE) return;
...
static gboolean meta_window_can_tile_maximized(MetaWindow *window) {
...
gboolean meta_window_can_tile(MetaWindow *window) {
  ...
  if (!meta_window_can_tile_maximized(window) || window->shaded) return FALSE;
...
void meta_window_unmaximize(MetaWindow *window, MetaMaximizeFlags directions) {
  ...
  if ((unmaximize_horizontally && window->maximized_horizontally) ||
    ...
    meta_topic(META_DEBUG_WINDOW_OPS, "Unmaximizing %s%s\n", window->desc,
               unmaximize_horizontally && unmaximize_vertically ? ""
               : unmaximize_horizontally ? " horizontally"
               : unmaximize_vertically   ? " vertically"
                                         : "BUGGGGG");
    ...
    meta_window_move_resize(window, FALSE, target_rect.x, target_rect.y,
...
void meta_window_move_to_monitor(MetaWindow *window,
  ...
  if (META_WINDOW_TILED(window)) {
    ...
    meta_window_transform_to_monitor(&window->saved_rect, &from_monitor->rect,
                                     &to_monitor->rect);
    meta_window_transform_to_monitor(&window->user_rect, &from_monitor->rect,
                                     &to_monitor->rect);
  ...
  meta_window_transform_to_monitor(&target_rect, &from_monitor->rect,
                                   &to_monitor->rect);

  meta_window_move_resize(window, TRUE, target_rect.x, target_rect.y,
                          target_rect.width, target_rect.height);
}
...
void meta_window_make_fullscreen_internal(MetaWindow *window) {
  if (!window->fullscreen) {
    meta_topic(META_DEBUG_WINDOW_OPS, "Fullscreening %s\n", window->desc);
...
void meta_window_unmake_fullscreen(MetaWindow *window) {
  if (window->fullscreen) {
    ...
    meta_topic(META_DEBUG_WINDOW_OPS, "Unfullscreening %s\n", window->desc);
...
void meta_window_shade(MetaWindow *window, guint32 timestamp) {
  meta_topic(META_DEBUG_WINDOW_OPS, "Shading %s\n", window->desc);
  if (!window->shaded) {
    window->shaded = TRUE;
     ...
     */
    meta_topic(META_DEBUG_FOCUS, "Re-focusing window %s after shading it\n",
               window->desc);
    meta_window_focus(window, timestamp);
...
void meta_window_unshade(MetaWindow *window, guint32 timestamp) {
  meta_topic(META_DEBUG_WINDOW_OPS, "Unshading %s\n", window->desc);
  if (window->shaded) {
    window->shaded = FALSE;
    meta_window_queue(window, META_QUEUE_MOVE_RESIZE | META_QUEUE_CALC_SHOWING);
...
static void window_activate(MetaWindow *window, guint32 timestamp,
  ...
  if (workspace == NULL) workspace = window->screen->active_workspace;
  ...
  if (window->shaded) meta_window_unshade(window, timestamp);
  ...
  meta_topic(META_DEBUG_FOCUS, "Focusing window %s due to activation\n",
             window->desc);
  meta_window_focus(window, timestamp);
}
...
void meta_window_activate_with_workspace(MetaWindow *window, guint32 timestamp,
                                         MetaWorkspace *workspace) {
  /* We're not really a pager, but the behavior we want is the same as if
   * we were such.  If we change the pager behavior later, we could revisit
   * this and just add extra flags to window_activate.
 ...
 */
static void adjust_for_gravity(MetaWindow *window, MetaFrameBorders *borders,
...
static void send_sync_request(MetaWindow *window) {
   ...
   */
  XSendEvent(window->display->xdisplay, window->xwindow, False, 0,
             (XEvent *)&ev);
...
static gboolean move_attached_dialog(MetaWindow *window, void *data) {
...
static void meta_window_move_resize_internal(MetaWindow *window,
  ...
  meta_topic(META_DEBUG_GEOMETRY,
             ...
             is_user_action ? " (user move/resize)" : "", old_rect.x,
             old_rect.y, old_rect.width, old_rect.height);

  if (have_window_frame) meta_frame_calc_borders(window->frame, &borders);
  ...
  if ((flags & (META_IS_MOVE_ACTION | META_IS_RESIZE_ACTION)) ==
    ...
    meta_rectangle_resize_with_gravity(&old_rect, &new_rect, gravity,
    ...
    adjust_for_gravity(window, have_window_frame ? &borders : NULL,
                       /* configure request coords assume
                        * the border width existed
                        */
                       is_configure_request, gravity, &new_rect);
  ...
  meta_window_constrain(window, have_window_frame ? &borders : NULL, flags,
                        gravity, &old_rect, &new_rect);
  ...
  if (w != window->rect.width || h != window->rect.height)
  ...
  if (have_window_frame) {
    ...
    meta_topic(META_DEBUG_GEOMETRY, "Calculated frame size %dx%d\n",
               window->frame->rect.width, window->frame->rect.height);
  }
  ...
  if (have_window_frame) {
    ...
    if (need_resize_frame && need_move_frame &&
      ...
      if (need_move_frame) need_move_client = TRUE;
    ...
    if (root_x_nw != window->rect.x || root_y_nw != window->rect.y)
      need_move_client = TRUE;
  ...
  if (is_configure_request &&
      !(need_move_client || need_move_frame || need_resize_client ||
        need_resize_frame || window->border_width != 0))
  ...
  if (use_static_gravity) meta_window_set_gravity(window, StaticGravity);

  if (configure_frame_first && have_window_frame)
    frame_shape_changed = meta_frame_sync_to_window(
        window->frame, gravity, need_move_frame, need_resize_frame);
  ...
  if (need_move_client) mask |= (CWX | CWY);
  if (need_resize_client) mask |= (CWWidth | CWHeight);
  ...
  if (mask != 0) {
    {
      ...
      meta_topic(META_DEBUG_GEOMETRY,
                 "Syncing new client geometry %d,%d %dx%d, border: %s pos: %s "
                 "size: %s\n",
    ...
    XConfigureWindow(window->display->xdisplay, window->xwindow, mask, &values);
  ...
  if (use_static_gravity) meta_window_set_gravity(window, NorthWestGravity);

  if (need_configure_notify) send_configure_notify(window);
  ...
  if (need_move_frame || need_resize_frame || need_move_client ||
      need_resize_client) {
    int newx, newy;
    meta_window_get_position(window, &newx, &newy);
    meta_topic(META_DEBUG_GEOMETRY,
               "New size/position %d,%d %dx%d (user %d,%d %dx%d)\n", newx, newy,
               window->rect.width, window->rect.height, window->user_rect.x,
               window->user_rect.y, window->user_rect.width,
...
void meta_window_resize(MetaWindow *window, gboolean user_op, int w, int h) {
  ...
  meta_window_move_resize_internal(window, flags, NorthWestGravity, x, y, w, h);
}
...
void meta_window_move(MetaWindow *window, gboolean user_op, int root_x_nw,
  ...
  meta_window_move_resize_internal(window, flags, NorthWestGravity, root_x_nw,
                                   root_y_nw, window->rect.width,
                                   window->rect.height);
}

void meta_window_move_resize(MetaWindow *window, gboolean user_op,
                             int root_x_nw, int root_y_nw, int w, int h) {
  MetaMoveResizeFlags flags = (user_op ? META_IS_USER_ACTION : 0) |
...
static void meta_window_move_resize_now(MetaWindow *window) {
   ...
   */
  meta_window_move_resize(window, FALSE, window->user_rect.x,
                          window->user_rect.y, window->user_rect.width,
                          window->user_rect.height);
}
...
void meta_window_get_position(MetaWindow *window, int *x, int *y) {
  if (window->frame) {
    if (x) *x = window->frame->rect.x + window->frame->child_x;
    if (y) *y = window->frame->rect.y + window->frame->child_y;
  } else {
    if (x) *x = window->rect.x;
    if (y) *y = window->rect.y;
  }
}
...
void meta_window_get_gravity_position(MetaWindow *window, int gravity,
...
void meta_window_get_geometry(MetaWindow *window, int *x, int *y, int *width,
                              int *height) {
  meta_window_get_gravity_position(window, window->size_hints.win_gravity, x,
                                   y);
...
void meta_window_get_xor_rect(MetaWindow *window,
  ...
  if (window->frame) {
    ...
    xor_rect->width = grab_wireframe_rect->width + window->frame->child_x +
                      window->frame->right_width;
 ...
 */
static void meta_window_get_wireframe_geometry(MetaWindow *window, int *width,
                                               int *height) {
  if (!window->display->grab_wireframe_active) return;
...
void meta_window_begin_wireframe(MetaWindow *window) {
  ...
  meta_effects_begin_wireframe(window->screen, &new_xor, display_width,
                               display_height);
...
void meta_window_update_wireframe(MetaWindow *window, int x, int y, int width,
  ...
  meta_effects_update_wireframe(
      window->screen, &window->display->grab_wireframe_last_xor_rect,
      window->display->grab_wireframe_last_display_width,
      window->display->grab_wireframe_last_display_height, &new_xor,
...
void meta_window_end_wireframe(MetaWindow *window) {
  meta_effects_end_wireframe(
      window->display->grab_window->screen,
...
const char *meta_window_get_startup_id(MetaWindow *window) {
  if (window->startup_id == NULL) {
    ...
    if (group != NULL) return meta_group_get_startup_id(group);
  }

  return window->startup_id;
}

static MetaWindow *get_modal_transient(MetaWindow *window) {
  GSList *windows;
  GSList *tmp;
  ...
  if (window == modal_transient) modal_transient = NULL;
...
void meta_window_focus(MetaWindow *window, guint32 timestamp) {
  ...
  if (window->display->grab_window &&
    ...
    meta_topic(META_DEBUG_FOCUS,
               "Current focus window %s has global keygrab, not focusing "
               "window %s after all\n",
  ...
  if (modal_transient != NULL && !modal_transient->unmanaging) {
    meta_topic(META_DEBUG_FOCUS,
   ...
   */
  if (window->frame &&
      (window->shaded || !(window->input || window->take_focus))) {
    if (window->frame) {
      meta_topic(META_DEBUG_FOCUS, "Focusing frame of %s\n", window->desc);
      meta_display_set_input_focus_window(window->display, window, TRUE,
                                          timestamp);
    }
  } else {
    if (window->input) {
      meta_topic(META_DEBUG_FOCUS,
                 ...
                 window->desc);
      meta_display_set_input_focus_window(window->display, window, FALSE,
                                          timestamp);
    }
    ...
    if (window->take_focus) {
      meta_topic(META_DEBUG_FOCUS,
                 ...
                 window->desc);
      meta_window_send_icccm_message(
          window, window->display->atom_WM_TAKE_FOCUS, timestamp);
...
static void meta_window_change_workspace_without_transients(
   ...
   */
  if (window->on_all_workspaces) meta_window_unstick(window);
...
void meta_window_change_workspace(MetaWindow *window,
  ...
  meta_window_foreach_transient(window, change_workspace_foreach, workspace);
  meta_window_foreach_ancestor(window, change_workspace_foreach, workspace);
}
...
void meta_window_stick(MetaWindow *window) {
  gboolean stick = TRUE;
  window_stick_impl(window);
  meta_window_foreach_transient(window, stick_foreach_func, &stick);
}
...
static void update_net_frame_extents(MetaWindow *window) {
  ...
  meta_error_trap_push(window->display);
  XChangeProperty(window->display->xdisplay, window->xwindow,
                  window->display->atom__NET_FRAME_EXTENTS, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, 4);
  meta_error_trap_pop(window->display, FALSE);
}
...
void meta_window_set_current_workspace_hint(MetaWindow *window) {
  ...
  meta_error_trap_push(window->display);
  XChangeProperty(window->display->xdisplay, window->xwindow,
                  window->display->atom__NET_WM_DESKTOP, XA_CARDINAL, 32,
                  PropModeReplace, (guchar *)data, 1);
  meta_error_trap_pop(window->display, FALSE);
}
...
void meta_window_raise(MetaWindow *window) {
  ...
  meta_topic(META_DEBUG_WINDOW_OPS, "Raising window %s, ancestor of %s\n",
   ...
   */
  if (window != ancestor) meta_stack_raise(window->screen->stack, window);
}

void meta_window_lower(MetaWindow *window) {
  meta_topic(META_DEBUG_WINDOW_OPS, "Lowering window %s\n", window->desc);
...
void meta_window_move_resize_request(MetaWindow *window, guint value_mask,
                                     int gravity, int new_x, int new_y,
   ...
   * generated.
   */
  meta_window_get_gravity_position(window, gravity, &x, &y);
  ...
  if (meta_prefs_get_disable_workarounds()) {
    if (window->type == META_WINDOW_DIALOG ||
      ...
      ; /* No position change for these */
              ...
              */
             ((window->size_hints.flags & USPosition) && !window->placed))
      allow_position_change = TRUE;
  } else {
    allow_position_change = TRUE;
  }

  if (in_grab_op) allow_position_change = FALSE;

  if (allow_position_change) {
    if (value_mask & CWX) x = new_x;
    if (value_mask & CWY) y = new_y;
    ...
    meta_topic(META_DEBUG_GEOMETRY,
               "Not allowing position change for window %s PPosition 0x%lx "
               "USPosition 0x%lx type %u\n",
               window->desc, window->size_hints.flags & PPosition,
               window->size_hints.flags & USPosition, window->type);
  }

  width = window->rect.width;
  height = window->rect.height;
  if (!in_grab_op) {
    if (value_mask & CWWidth) width = new_width;

    if (value_mask & CWHeight) height = new_height;
  }
  ...
  if (value_mask & (CWX | CWY)) flags |= META_IS_MOVE_ACTION;
  if (value_mask & (CWWidth | CWHeight)) flags |= META_IS_RESIZE_ACTION;
  ...
  if (flags & (META_IS_MOVE_ACTION | META_IS_RESIZE_ACTION)) {
    ...
    if (meta_prefs_get_force_fullscreen() &&
        ...
        window->has_fullscreen_func && !window->fullscreen) {
      g_warning(
    ...
    meta_window_move_resize_internal(window, flags, gravity, x, y, width,
                                     height);
  }
...
gboolean meta_window_configure_request(MetaWindow *window, XEvent *event) {
  ...
  meta_window_move_resize_request(
      window, event->xconfigurerequest.value_mask,
      window->size_hints.win_gravity, event->xconfigurerequest.x,
      event->xconfigurerequest.y, event->xconfigurerequest.width,
  ...
  if (event->xconfigurerequest.value_mask & CWStackMode) {
    ...
    if (meta_prefs_get_disable_workarounds() ||
      ...
      meta_topic(META_DEBUG_STACK,
                 ...
                 "user_time %u).\n",
                 window->desc, window->net_wm_user_time, active_window->desc,
                 active_window->net_wm_user_time);
...
gboolean meta_window_client_message(MetaWindow *window, XEvent *event) {
  ...
  if (event->xclient.message_type == display->atom__NET_CLOSE_WINDOW) {
    guint32 timestamp;
    ...
    return TRUE;
  } else if (event->xclient.message_type == display->atom__NET_WM_DESKTOP) {
    int space;
    MetaWorkspace *workspace;
    ...
    workspace = meta_screen_get_workspace_by_index(window->screen, space);
    ...
    return TRUE;
  } else if (event->xclient.message_type == display->atom__NET_WM_STATE) {
    gulong action;
    Atom first;
    Atom second;
    ...
    if (first == display->atom__NET_WM_STATE_MAXIMIZED_HORZ ||
      ...
      max = (action == _NET_WM_STATE_ADD || (action == _NET_WM_STATE_TOGGLE &&
                                             !window->maximized_horizontally));
    ...
    if (first == display->atom__NET_WM_STATE_MAXIMIZED_VERT ||
      ...
      max = (action == _NET_WM_STATE_ADD ||
             (action == _NET_WM_STATE_TOGGLE && !window->maximized_vertically));
      if (max && window->has_maximize_func) {
        if (meta_prefs_get_raise_on_click()) meta_window_raise(window);
    ...
    return TRUE;
  } else if (event->xclient.message_type == display->atom_WM_CHANGE_STATE) {
    meta_verbose("WM_CHANGE_STATE client message, state: %ld\n",
                 event->xclient.data.l[0]);
    if (event->xclient.data.l[0] == IconicState) meta_window_minimize(window);

    return TRUE;
  } else if (event->xclient.message_type == display->atom__NET_WM_MOVERESIZE) {
    int x_root;
    int y_root;
    int action;
    MetaGrabOp op;
    ...
    meta_topic(META_DEBUG_WINDOW_OPS,
               ...
               "button %d\n",
               window->desc, x_root, y_root, action, button);
    ...
    if (action == _NET_WM_MOVERESIZE_CANCEL) {
      ...
      if (button == 0) {
        ...
        mask = 0;
        meta_error_trap_push(window->display);
        XQueryPointer(window->display->xdisplay, window->xwindow, &root, &child,
                      &query_root_x, &query_root_y, &x, &y, &mask);
      ...
      if (button != 0) {
        ...
        meta_display_begin_grab_op(window->display, window->screen, window, op,
                                   FALSE, frame_action, button, 0, timestamp,
                                   x_root, y_root);
      }
    }
    ...
    if (gravity == 0) gravity = window->size_hints.win_gravity;
  ...
  } else if (event->xclient.message_type == display->atom__NET_ACTIVE_WINDOW) {
    MetaClientType source_indication;
    guint32 timestamp;
    ...
    meta_window_show_menu(window, x_root, y_root, button, timestamp);
  }

  return FALSE;
}

static void meta_window_appears_focused_changed(MetaWindow *window) {
  set_net_wm_state(window);

  if (window->frame) meta_frame_queue_draw(window->frame);
}

static void check_ancestor_focus_appearance(MetaWindow *window) {
  MetaWindow *parent = meta_window_get_transient_for(window);

  if (!meta_prefs_get_attach_modal_dialogs()) return;

  if (window->type != META_WINDOW_MODAL_DIALOG || !parent || parent == window)
    return;

  if (parent->frame) meta_frame_queue_draw(parent->frame);
...
gboolean meta_window_notify_focus(MetaWindow *window, XEvent *event) {
  ...
  meta_topic(META_DEBUG_FOCUS,
             ...
             event->xany.window == window->xwindow ? "client window"
  ...
  if (event->type == FocusIn) {
    if (window != window->display->focus_window) {
      ...
      if (window->screen->active_workspace &&
        ...
        window->screen->active_workspace->mru_list = g_list_remove_link(
            window->screen->active_workspace->mru_list, link);
        g_list_free(link);

        window->screen->active_workspace->mru_list =
            g_list_prepend(window->screen->active_workspace->mru_list, window);
      }
    ...
    if (window == window->display->focus_window) {
      meta_topic(META_DEBUG_FOCUS,
                 "%s is now the previous focus window due to being focused out "
                 "or unmapped\n",
                 window->desc);

      meta_topic(META_DEBUG_FOCUS, "* Focus --> NULL (was %s)\n", window->desc);
...
static gboolean process_property_notify(MetaWindow *window,
  ...
  {
    char *property_name = XGetAtomName(window->display->xdisplay, event->atom);
...
static void send_configure_notify(MetaWindow *window) {
  ...
  event.xconfigure.border_width =
      window->border_width;      /* requested not actual */
  ...
  meta_topic(
      ...
      window->desc, event.xconfigure.x, event.xconfigure.y,
      event.xconfigure.width, event.xconfigure.height);

  meta_error_trap_push(window->display);
  XSendEvent(window->display->xdisplay, window->xwindow, False,
...
gboolean meta_window_get_icon_geometry(MetaWindow *window,
  ...
  if (meta_prop_get_cardinal_list(window->display, window->xwindow,
...
static Window read_client_leader(MetaDisplay *display, Window xwindow) {
  ...
  meta_prop_get_window(display, xwindow, display->atom_WM_CLIENT_LEADER,
                       &retval);
...
static gboolean find_client_leader_func(MetaWindow *ancestor, void *data) {
  ...
  d->leader = read_client_leader(ancestor->display, ancestor->xwindow);
...
static void update_sm_hints(MetaWindow *window) {
  ...
  if (leader == None) {
    ClientLeaderData d;
    d.leader = None;
    meta_window_foreach_ancestor(window, find_client_leader_func, &d);
    leader = d.leader;
  }
  ...
  if (leader != None) {
    ...
    if (meta_prop_get_latin1_string(window->display, leader,
                                    window->display->atom_SM_CLIENT_ID, &str)) {
    ...
    if (!meta_prefs_get_disable_workarounds()) {
      ...
      if (meta_prop_get_latin1_string(window->display, window->xwindow,
        ...
        if (window->sm_client_id == NULL) /* first time through */
          meta_warning(
              _("Window %s sets SM_CLIENT_ID on itself, instead of on the "
                "WM_CLIENT_LEADER window as specified in the ICCCM.\n"),
              window->desc);
...
void meta_window_update_role(MetaWindow *window) {
  ...
  if (window->role) g_free(window->role);
  window->role = NULL;

  if (meta_prop_get_latin1_string(window->display, window->xwindow,
                                  window->display->atom_WM_WINDOW_ROLE, &str)) {
    window->role = g_strdup(str);
    meta_XFree(str);
  }

  meta_verbose("Updated role of %s to '%s'\n", window->desc,
               window->role ? window->role : "null");
}
...
void meta_window_update_net_wm_type(MetaWindow *window) {
  ...
  meta_prop_get_atom_list(window->display, window->xwindow,
                          window->display->atom__NET_WM_WINDOW_TYPE, &atoms,
                          &n_atoms);
...
void meta_window_update_icon_now(MetaWindow *window) {
  ...
  if (meta_read_icons(window->screen, window->xwindow, window->res_name,
                      &window->icon_cache, window->wm_hints_pixmap,
                      window->wm_hints_mask, &icon, icon_size, /* width  */
                      icon_size,                               /* height */
                      &mini_icon, META_MINI_ICON_WIDTH,
                      META_MINI_ICON_HEIGHT)) {
    if (window->icon) g_object_unref(G_OBJECT(window->icon));

    if (window->mini_icon) g_object_unref(G_OBJECT(window->mini_icon));
...
void meta_window_update_struts(MetaWindow *window) {
  ...
  if (meta_prop_get_cardinal_list(window->display, window->xwindow,
    ...
    if (nitems != 4) {
      ...
      for (i = 0; i < window->screen->n_xinerama_infos; i++) {
        ...
        if (!meta_rectangle_contains_rect(&monitor, &temp->rect)) continue;

        if (temp->rect.height > temp->rect.width) {
          if (temp->rect.x == monitor.x) {
            side = META_SIDE_LEFT;
            valid = TRUE;
          } else if (temp->rect.x + temp->rect.width ==
          ...
          if (temp->rect.y == monitor.y) {
            side = META_SIDE_TOP;
            valid = TRUE;
          } else if (temp->rect.y + temp->rect.height ==
  ...
  if (!new_struts &&
      meta_prop_get_cardinal_list(window->display, window->xwindow,
                                  window->display->atom__NET_WM_STRUT_PARTIAL,
                                  &struts, &nitems)) {
    if (nitems != 12)
      meta_verbose(
          "_NET_WM_STRUT_PARTIAL on %s has %d values instead "
          "of 12\n",
          window->desc, nitems);
    else {
      ...
      meta_verbose(
          "_NET_WM_STRUT_PARTIAL struts %lu %lu %lu %lu for "
          "window %s\n",
          struts[0], struts[1], struts[2], struts[3], window->desc);
    }
    meta_XFree(struts);
  } else {
  ...
  if (!new_struts &&
      meta_prop_get_cardinal_list(window->display, window->xwindow,
                                  window->display->atom__NET_WM_STRUT, &struts,
                                  &nitems)) {
    if (nitems != 4)
  ...
  if (changed) {
    ...
    meta_topic(META_DEBUG_WORKAREA, "Struts on %s were unchanged\n",
               window->desc);
  }
}
...
static void recalc_window_type(MetaWindow *window) {
  ...
  if (window->type_atom != None) {
    ...
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_DOCK)
      window->type = META_WINDOW_DOCK;
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_TOOLBAR)
      window->type = META_WINDOW_TOOLBAR;
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_MENU)
      window->type = META_WINDOW_MENU;
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_DIALOG)
      window->type = META_WINDOW_DIALOG;
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_NORMAL)
      window->type = META_WINDOW_NORMAL;
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_UTILITY)
      window->type = META_WINDOW_UTILITY;
    else if (window->type_atom ==
             window->display->atom__NET_WM_WINDOW_TYPE_SPLASH)
      window->type = META_WINDOW_SPLASHSCREEN;
    else
      meta_bug(
          "Set a type atom for %s that wasn't handled in recalc_window_type\n",
          window->desc);
  } else if (window->xtransient_for != None) {
    window->type = META_WINDOW_DIALOG;
  } else {
  ...
  if (window->type == META_WINDOW_DIALOG && window->wm_state_modal)
    window->type = META_WINDOW_MODAL_DIALOG;

  meta_verbose("Calculated type %u for %s, old type %u\n", window->type,
               window->desc, old_type);
...
#define MAX_N_ACTIONS 12
  ...
  i = 0;
  if (window->has_move_func && !window->minimized) {
    data[i] = window->display->atom__NET_WM_ACTION_MOVE;
    ++i;
  }
  if (window->has_resize_func && !window->minimized && !window->shaded &&
      !window->tiled &&
      !(window->maximized_horizontally && window->maximized_vertically)) {
    data[i] = window->display->atom__NET_WM_ACTION_RESIZE;
    ++i;
  }
  if (window->has_fullscreen_func && !window->minimized && !window->shaded) {
  ...
  if (window->has_shade_func && !window->minimized) {
   ...
   */
  if (window->has_maximize_func && !window->minimized && !window->shaded) {
  ...
  meta_error_trap_push(window->display);
  XChangeProperty(window->display->xdisplay, window->xwindow,
                  window->display->atom__NET_WM_ALLOWED_ACTIONS, XA_ATOM, 32,
                  PropModeReplace, (guchar *)data, i);
  meta_error_trap_pop(window->display, FALSE);
#undef MAX_N_ACTIONS
}
...
static void recalc_window_features(MetaWindow *window) {
  ...
  if (window->size_hints.min_width == window->size_hints.max_width &&
    ...
    meta_warning(_("Window %s sets an MWM hint indicating it isn't resizable, "
                   "but sets min size %d x %d and max size %d x %d; this "
                   "doesn't make much sense.\n"),
                 window->desc, window->size_hints.min_width,
                 window->size_hints.min_height, window->size_hints.max_width,
  ...
  if (window->type == META_WINDOW_TOOLBAR) window->decorated = FALSE;
  ...
  meta_topic(META_DEBUG_WINDOW_OPS,
             "Window %s fullscreen = %d not resizable, maximizable = %d "
             "fullscreenable = %d min size %dx%d max size %dx%d\n",
             window->desc, window->fullscreen, window->has_maximize_func,
             window->has_fullscreen_func, window->size_hints.min_width,
             window->size_hints.min_height, window->size_hints.max_width,
             window->size_hints.max_height);

  /* no shading if not decorated */
  if (!window->decorated || window->border_only) window->has_shade_func = FALSE;

  window->skip_taskbar = FALSE;
  window->skip_pager = FALSE;

  if (window->wm_state_skip_taskbar) window->skip_taskbar = TRUE;

  if (window->wm_state_skip_pager) window->skip_pager = TRUE;
  ...
  meta_topic(META_DEBUG_WINDOW_OPS,
             "Window %s decorated = %d border_only = %d has_close = %d "
             "has_minimize = %d has_maximize = %d has_move = %d has_shade = %d "
             "skip_taskbar = %d skip_pager = %d\n",
             window->desc, window->decorated, window->border_only,
             window->has_close_func, window->has_minimize_func,
             window->has_maximize_func, window->has_move_func,
             window->has_shade_func, window->skip_taskbar, window->skip_pager);
   ...
   */
  if (window->constructing || old_has_close_func != window->has_close_func ||
      old_has_minimize_func != window->has_minimize_func ||
      old_has_move_func != window->has_move_func ||
      old_has_resize_func != window->has_resize_func ||
      old_has_shade_func != window->has_shade_func ||
...
static void menu_callback(MetaWindowMenu *menu, Display *xdisplay,
                          Window client_xwindow, guint32 timestamp,
                          MetaMenuOp op, int workspace_index, gpointer data) {
  MetaDisplay *display;
  MetaWindow *window;
  MetaWorkspace *workspace;
    ...
    switch (op) {
      ...
      case META_MENU_OP_UNMAXIMIZE:
        meta_window_unmaximize(
            window, META_MAXIMIZE_HORIZONTAL | META_MAXIMIZE_VERTICAL);
        break;
      ...
      case META_MENU_OP_MOVE_LEFT:
        workspace = meta_workspace_get_neighbor(
            window->screen->active_workspace, META_MOTION_LEFT);
        break;

      case META_MENU_OP_MOVE_RIGHT:
        workspace = meta_workspace_get_neighbor(
            window->screen->active_workspace, META_MOTION_RIGHT);
        break;

      case META_MENU_OP_MOVE_UP:
        workspace = meta_workspace_get_neighbor(
            window->screen->active_workspace, META_MOTION_UP);
        break;

      case META_MENU_OP_MOVE_DOWN:
        workspace = meta_workspace_get_neighbor(
            window->screen->active_workspace, META_MOTION_DOWN);
        break;

      case META_MENU_OP_WORKSPACES:
        workspace =
            meta_screen_get_workspace_by_index(window->screen, workspace_index);
        break;
      ...
      case META_MENU_OP_MOVE:
        meta_window_begin_grab_op(window, META_GRAB_OP_KEYBOARD_MOVING, TRUE,
                                  timestamp);
        break;

      case META_MENU_OP_RESIZE:
        meta_window_begin_grab_op(
            window, META_GRAB_OP_KEYBOARD_RESIZING_UNKNOWN, TRUE, timestamp);
        break;
...
void meta_window_show_menu(MetaWindow *window, int root_x, int root_y,
  ...
  if (!meta_window_titlebar_is_onscreen(window) &&
      window->type != META_WINDOW_DOCK && window->type != META_WINDOW_DESKTOP)
    ops |= META_MENU_OP_RECOVER;

  n_workspaces = meta_screen_get_n_workspaces(window->screen);

  if (n_workspaces > 1) ops |= META_MENU_OP_WORKSPACES;

  meta_screen_calc_workspace_layout(
      window->screen, n_workspaces,
      meta_workspace_index(window->screen->active_workspace), &layout);
  ...
  if (!window->on_all_workspaces) {
    ...
    if ((layout.current_col < layout.cols - 1) &&
        (layout.current_row * layout.cols + (layout.current_col + 1) <
         n_workspaces))
      ops |= ltr ? META_MENU_OP_MOVE_RIGHT : META_MENU_OP_MOVE_LEFT;
    if (layout.current_row > 0) ops |= META_MENU_OP_MOVE_UP;
  ...
  if (!window->has_minimize_func) insensitive |= META_MENU_OP_MINIMIZE;
  ...
  if (!META_WINDOW_ALLOWS_MOVE(window)) insensitive |= META_MENU_OP_MOVE;

  if (!META_WINDOW_ALLOWS_RESIZE(window)) insensitive |= META_MENU_OP_RESIZE;

  if (window->always_sticky)
    insensitive |=
        META_MENU_OP_STICK | META_MENU_OP_UNSTICK | META_MENU_OP_WORKSPACES;
   ...
   */
  if ((ops & ~insensitive) == 0) return;

  menu = meta_ui_window_menu_new(
      window->screen->ui, window->xwindow, ops, insensitive,
      meta_window_get_net_wm_desktop(window),
      meta_screen_get_n_workspaces(window->screen), menu_callback, NULL);
...
void meta_window_shove_titlebar_onscreen(MetaWindow *window) {
  ...
  meta_rectangle_expand_region(onscreen_region, horiz_amount, horiz_amount, 0,
                               vert_amount);
  meta_rectangle_shove_into_region(onscreen_region, FIXED_DIRECTION_X,
                                   &outer_rect);
  meta_rectangle_expand_region(onscreen_region, -horiz_amount, -horiz_amount, 0,
                               -vert_amount);
  ...
  meta_window_move_resize(window, FALSE, newx, newy, window->rect.width,
                          window->rect.height);
}
...
gboolean meta_window_titlebar_is_onscreen(MetaWindow *window) {
  ...
  while (onscreen_region) {
    ...
    if (overlap.height > MIN(titlebar_rect.height, min_height_needed) &&
        overlap.width >
            MIN((int)(min_width_percent * (double)titlebar_rect.width),
                min_width_absolute)) {
      is_onscreen = TRUE;
      break;
    }
...
static double time_diff(const gint64 first, const gint64 second) {
...
static gboolean check_moveresize_frequency(MetaWindow *window,
  ...
  if (!window->disable_sync &&
    ...
    if (window->sync_request_time != 0) {
      ...
      if (elapsed < 1000.0) {
         ...
         */
        if (remaining) *remaining = 1000.0 - elapsed + 100;
...
#endif /* HAVE_XSYNC */
  {
    ...
    elapsed =
        time_diff(current_time, window->display->grab_last_moveresize_time);
    ...
    if (elapsed >= 0.0 && elapsed < ms_between_resizes) {
      ...
      if (remaining) *remaining = (ms_between_resizes - elapsed);

      return FALSE;
    }

    meta_topic(META_DEBUG_RESIZING,
               " Checked moveresize freq, allowing move/resize now (%g of %g "
               "seconds elapsed)\n",
...
static gboolean update_move_timeout(gpointer data) {
  ...
  update_move(window, window->display->grab_last_user_action_was_snap != FALSE,
...
static void update_move(MetaWindow *window, gboolean snap, int x, int y) {
  ...
  meta_verbose("x,y = %d,%d anchor ptr %d,%d anchor pos %d,%d dx,dy %d,%d\n", x,
               y, display->grab_anchor_root_x, display->grab_anchor_root_y,
               display->grab_anchor_window_pos.x,
               display->grab_anchor_window_pos.y, dx, dy);
...
#define DRAG_THRESHOLD_TO_SHAKE_THRESHOLD_FACTOR 2
  ...
  if (snap) {
  ...
  } else if (meta_prefs_get_allow_tiling() && !META_WINDOW_MAXIMIZED(window) &&
     ...
     */
    monitor = meta_screen_get_current_xinerama(window->screen);
    meta_window_get_work_area_for_xinerama(window, monitor->number, &work_area);

    /* Check if the cursor is in a position which triggers tiling
     * and set tile_mode accordingly.
     */
    MetaTileMode old_tile_mode = window->tile_mode;
    window->tile_mode = calculate_tiling_mode(x, y, window, monitor, work_area,
                                              shake_threshold);
  ...
  if ((META_WINDOW_MAXIMIZED(window) && ABS(dy) >= shake_threshold) ||
    ...
    prop = ((double)(x - display->grab_initial_window_pos.x)) /
   ...
   */
    ...
    for (monitor = 0; monitor < window->screen->n_xinerama_infos; monitor++) {
      ...
      if (x >= work_area.x + shake_threshold &&
          ...
          y >= work_area.y && y < (work_area.y + shake_threshold)) {
  ...
  meta_window_edge_resistance_for_move(window, old.x, old.y, &new_x, &new_y,
                                       update_move_timeout, snap, FALSE);

  if (display->compositor) {
    int root_x =
        new_x - display->grab_anchor_window_pos.x + display->grab_anchor_root_x;
    int root_y =
        new_y - display->grab_anchor_window_pos.y + display->grab_anchor_root_y;
...
static MetaTileMode calculate_tiling_mode(int x, int y, MetaWindow *window,
                                          ...
                                          MetaRectangle work_area,
                                          int shake_threshold) {
  if (meta_window_can_tile(window) && x >= monitor->rect.x &&
  ...
  } else if (meta_window_can_tile_maximized(window) && y >= monitor->rect.y &&
             y <= work_area.y && meta_prefs_get_allow_top_tiling())
...
static gboolean update_resize_timeout(gpointer data) {
  ...
  update_resize(window,
                ...
                window->display->grab_latest_motion_x,
                window->display->grab_latest_motion_y, TRUE);
  return FALSE;
}
...
static void update_resize(MetaWindow *window, gboolean snap, int x, int y,
  ...
  /* Do any edge resistance/snapping */
  meta_window_edge_resistance_for_resize(window, old.width, old.height, &new_w,
                                         &new_h, gravity, update_resize_timeout,
                                         snap, FALSE);

  if (window->display->grab_wireframe_active) {
    if ((new_x + new_w <= new_x) || (new_y + new_h <= new_y)) return;
...
static Bool find_last_time_predicate(Display *display, XEvent *xevent,
                                     XPointer arg) {
...
static gboolean check_use_this_motion_notify(MetaWindow *window,
  ...
  if (window->display->grab_motion_notify_time != 0) {
    ...
    if (window->display->grab_motion_notify_time <= event->xmotion.time) {
      meta_topic(META_DEBUG_RESIZING,
  ...
  XCheckIfEvent(window->display->xdisplay, &useless, find_last_time_predicate,
                (XPointer)&esd);
...
static void update_tile_mode(MetaWindow *window) {
  switch (window->tile_mode) {
    case META_TILE_LEFT:
    case META_TILE_RIGHT:
      if (!META_WINDOW_SIDE_TILED(window)) window->tile_mode = META_TILE_NONE;
      break;
  }
}
  ...
  switch (event->type) {
    case ButtonRelease:
      meta_display_check_threshold_reached(
          window->display, event->xbutton.x_root, event->xbutton.y_root);
      ...
      if (!window->display->grab_last_user_action_was_snap) {
        if (meta_grab_op_is_moving(window->display->grab_op)) {
          ...
          if (event->xbutton.root == window->screen->xroot)
            update_resize(window, event->xbutton.state & ShiftMask,
                          event->xbutton.x_root, event->xbutton.y_root, TRUE);
          if (window->display->compositor)
            meta_compositor_set_updates(window->display->compositor, window,
                                        TRUE);
    ...
    case MotionNotify:
      meta_display_check_threshold_reached(
          window->display, event->xmotion.x_root, event->xmotion.y_root);
      if (meta_grab_op_is_moving(window->display->grab_op)) {
        if (event->xmotion.root == window->screen->xroot) {
          if (check_use_this_motion_notify(window, event))
            update_move(window, event->xmotion.state & ShiftMask,
                        event->xmotion.x_root, event->xmotion.y_root);
        }
      } else if (meta_grab_op_is_resizing(window->display->grab_op)) {
        if (event->xmotion.root == window->screen->xroot) {
          if (check_use_this_motion_notify(window, event))
            update_resize(window, event->xmotion.state & ShiftMask,
                          event->xmotion.x_root, event->xmotion.y_root, FALSE);
        }
      }
      break;
...
void meta_window_set_gravity(MetaWindow *window, int gravity) {
  ...
  XChangeWindowAttributes(window->display->xdisplay, window->xwindow,
                          CWWinGravity, &attrs);

  meta_error_trap_pop(window->display, FALSE);
}

static void get_work_area_xinerama(MetaWindow *window, MetaRectangle *area,
                                   int which_xinerama) {
  GList *tmp;
  ...
  *area = window->screen->xinerama_infos[which_xinerama].rect;
  ...
  while (tmp != NULL) {
    ...
    meta_workspace_get_work_area_for_xinerama(tmp->data, which_xinerama,
                                              &workspace_work_area);
    meta_rectangle_intersect(area, &workspace_work_area, area);
    tmp = tmp->next;
  }

  meta_topic(META_DEBUG_WORKAREA,
             "Window %s xinerama %d has work area %d,%d %d x %d\n",
             window->desc, which_xinerama, area->x, area->y, area->width,
             area->height);
}
...
void meta_window_get_work_area_for_xinerama(MetaWindow *window,
  ...
  get_work_area_xinerama(window, area, which_xinerama);
}
...
void meta_window_get_work_area_all_xineramas(MetaWindow *window,
  ...
  *area = window->screen->rect;
  ...
  while (tmp != NULL) {
    MetaRectangle workspace_work_area;
    meta_workspace_get_work_area_all_xineramas(tmp->data, &workspace_work_area);
    meta_rectangle_intersect(area, &workspace_work_area, area);
    tmp = tmp->next;
  }
...
void meta_window_get_current_tile_area(MetaWindow *window,
  ...
  /* I don't know how to detect monitor configuration changes, so I have to take
   * into account that tile_monitor_number might be invalid. If this happens, I
   * replace it with whatever monitor the window is currently on. This is
   * usually the correct monitor anyway, only in some special cases is the real
   * monitor number actually required (e.g. the window is being moved with the
   * mouse but is still mostly on the wrong monitor).
   */
  if (window->tile_monitor_number >= window->screen->n_xinerama_infos) {
    window->tile_monitor_number =
        meta_screen_get_xinerama_for_window(window->screen, window)->number;
  }
  ...
  meta_window_get_work_area_for_xinerama(window, tile_monitor_number,
                                         tile_area);
...
gboolean meta_window_same_application(MetaWindow *window,
  ...
  return group != NULL && other_group != NULL && group == other_group;
}
...
void meta_window_refresh_resize_popup(MetaWindow *window) {
  if (window->display->grab_op == META_GRAB_OP_NONE) return;

  if (window->display->grab_window != window) return;
  ...
  if (window->display->grab_resize_popup == NULL) {
    ...
    if (window->size_hints.width_inc > scale ||
      ...
      window->display->grab_resize_popup = meta_ui_resize_popup_new(
          window->display->xdisplay, window->screen->number);
  }
  ...
  if (window->display->grab_resize_popup != NULL) {
    ...
    meta_ui_resize_popup_set(
        window->display->grab_resize_popup, rect, window->size_hints.base_width,
        window->size_hints.base_height, window->size_hints.width_inc,
        window->size_hints.height_inc);

    meta_ui_resize_popup_set_showing(window->display->grab_resize_popup, TRUE);
  }
}
...
void meta_window_foreach_ancestor(MetaWindow *window,
  ...
  tortoise = window;
  while (TRUE) {
    if (w->xtransient_for == None || w->transient_parent_is_root_window) break;

    w = meta_display_lookup_x_window(w->display, w->xtransient_for);

    if (w == NULL || w == tortoise) break;

    if (!(*func)(w, data)) break;
 ...
 */
static gboolean warp_grab_pointer(MetaWindow *window, MetaGrabOp grab_op,
                                  int *x, int *y) {
  MetaRectangle rect;
  MetaDisplay *display;
  ...
  *y = CLAMP(*y, 0, window->screen->rect.height - 1);
  ...
  XWarpPointer(display->xdisplay, None, window->screen->xroot, 0, 0, 0, 0, *x,
               *y);
...
void meta_window_begin_grab_op(MetaWindow *window, MetaGrabOp op,
  ...
  meta_display_begin_grab_op(window->display, window->screen, window, op, FALSE,
                             frame_action, 0 /* button */, 0, timestamp, x, y);
}
...
void meta_window_update_keyboard_resize(MetaWindow *window,
  ...
  warp_grab_pointer(window, window->display->grab_op, &x, &y);

  if (update_cursor) {
    guint32 timestamp;
    /* FIXME: Using CurrentTime is really bad mojo */
    timestamp = CurrentTime;
    meta_display_set_grab_op_cursor(window->display, NULL,
                                    window->display->grab_op, TRUE,
                                    window->display->grab_xwindow, timestamp);
  }
}
...
void meta_window_stack_just_below(MetaWindow *window,
  ...
  if (window->stack_position > below_this_one->stack_position) {
    meta_topic(META_DEBUG_STACK,
               "Setting stack position of window %s to %d (making it below "
               "window %s).\n",
               window->desc, below_this_one->stack_position,
               below_this_one->desc);
    meta_window_set_stack_position(window, below_this_one->stack_position);
  } else {
    meta_topic(META_DEBUG_STACK, "Window %s  was already below window %s.\n",
...
void meta_window_set_user_time(MetaWindow *window, guint32 timestamp) {
  ...
  if (window->net_wm_user_time_set &&
    ...
    meta_topic(META_DEBUG_STARTUP, "Window %s has _NET_WM_USER_TIME of %u\n",
     ...
     */
    if (meta_prefs_get_focus_new_windows() == META_FOCUS_NEW_WINDOWS_STRICT &&
...
void meta_window_set_demands_attention(MetaWindow *window) {
  ...
  if (workspace != window->workspace) {
    ...
    while (stack != NULL && stack->data != window) {
      ...
      if (other_window->on_all_workspaces || window->on_all_workspaces ||
  ...
  if (obscured) {
    meta_topic(META_DEBUG_WINDOW_OPS, "Marking %s as needing attention\n",
               window->desc);
...
void meta_window_unset_demands_attention(MetaWindow *window) {
  meta_topic(META_DEBUG_WINDOW_OPS, "Marking %s as not needing attention\n",
             window->desc);
...
MetaFrame *meta_window_get_frame(MetaWindow *window) { return window->frame; }

static gboolean transient_has_focus(MetaWindow *window, void *data) {
  if (window->type == META_WINDOW_MODAL_DIALOG &&
...
gboolean meta_window_has_focus(MetaWindow *window) { return window->has_focus; }

gboolean meta_window_is_shaded(MetaWindow *window) { return window->shaded; }
...
Window meta_window_get_xwindow(MetaWindow *window) { return window->xwindow; }
...
static void meta_window_finalize(GObject *object) {
  MetaWindow *window;
...
static void meta_window_class_init(MetaWindowClass *window_class) {
...
static void meta_window_init(MetaWindow *window) {}
