...
static MetaGroup *meta_group_new(MetaDisplay *display, Window group_leader) {
  MetaGroup *group;
#define N_INITIAL_PROPS 3
  ...
  if (display->groups_by_leader == NULL)
    display->groups_by_leader =
        g_hash_table_new(meta_unsigned_long_hash, meta_unsigned_long_equal);

  g_assert(g_hash_table_lookup(display->groups_by_leader, &group_leader) ==
           NULL);

  g_hash_table_insert(display->groups_by_leader, &group->group_leader, group);
  ...
  meta_topic(META_DEBUG_GROUPS, "Created new group with leader 0x%lx\n",
             group->group_leader);
...
static void meta_group_unref(MetaGroup *group) {
  ...
  group->refcount -= 1;
  if (group->refcount == 0) {
    meta_topic(META_DEBUG_GROUPS, "Destroying group with leader 0x%lx\n",
               group->group_leader);

    g_assert(group->display->groups_by_leader != NULL);

    g_hash_table_remove(group->display->groups_by_leader, &group->group_leader);
...
MetaGroup *meta_window_get_group(MetaWindow *window) {
  if (window->unmanaging) return NULL;
...
void meta_window_compute_group(MetaWindow *window) {
  ...
  if (group != NULL) {
    ...
    if (ancestor != window && ancestor->xgroup_leader != None)
      group = meta_group_new(window->display, ancestor->xgroup_leader);
    else if (window->xgroup_leader != None)
      group = meta_group_new(window->display, window->xgroup_leader);
    else
      group = meta_group_new(window->display, window->xwindow);
  ...
  meta_topic(META_DEBUG_GROUPS, "Adding %s to group with leader 0x%lx\n",
             window->desc, group->group_leader);
}

static void remove_window_from_group(MetaWindow *window) {
  if (window->group != NULL) {
    meta_topic(META_DEBUG_GROUPS, "Removing %s from group with leader 0x%lx\n",
               window->desc, window->group->group_leader);

    window->group->windows = g_slist_remove(window->group->windows, window);
...
MetaGroup *meta_display_lookup_group(MetaDisplay *display,
  ...
  if (display->groups_by_leader)
    group = g_hash_table_lookup(display->groups_by_leader, &group_leader);
...
gboolean meta_group_property_notify(MetaGroup *group, XEvent *event) {
  meta_group_reload_property(group, event->xproperty.atom);
