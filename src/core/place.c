...
static gboolean place_by_pointer(MetaWindow *window, MetaFrameBorders *borders,
                                 MetaPlacementMode placement_mode, int *new_x,
                                 int *new_y) {
  ...
  XQueryPointer(window->display->xdisplay, window->screen->xroot, &root_return,
                &child_return, &root_x_return, &root_y_return, &win_x_return,
                &win_y_return, &mask_return);

  window_width = window->frame ? window->frame->rect.width : window->rect.width;
  window_height =
      window->frame ? window->frame->rect.height : window->rect.height;
...
static void find_next_cascade(MetaWindow *window, MetaFrameBorders *borders,
                              /* visible windows on relevant workspaces */
                              GList *windows, int x, int y, int *new_x,
                              int *new_y) {
  GList *tmp;
  GList *sorted;
...
#define CASCADE_FUZZ 15
  ...
  while (tmp != NULL) {
    ...
    if (ABS(wx - cascade_x) < x_threshold &&
      ...
      if (((cascade_x + window_width) > (work_area.x + work_area.width)) ||
          ((cascade_y + window_height) > (work_area.y + work_area.height))) {
        cascade_x = MAX(0, work_area.x);
        cascade_y = MAX(0, work_area.y);
...
static void find_most_freespace(MetaWindow *window, MetaFrameBorders *borders,
                                /* visible windows on relevant workspaces */
                                MetaWindow *focus_window, int x, int y,
                                int *new_x, int *new_y) {
...
static void avoid_being_obscured_as_second_modal_dialog(
    MetaWindow *window, MetaFrameBorders *borders, int *x, int *y) {
  ...
  if (window->denied_focus_and_not_transient &&
      ...
      meta_rectangle_intersect(&window->rect, &focus_window->rect, &overlap)) {
  ...
  while (tmp != NULL) {
    ...
    switch (other->type) {
      ...
      case META_WINDOW_MENU:
        ...
        if (meta_rectangle_intersect(rect, &other_rect, &dest)) return TRUE;
        break;
    }
...
static void center_rect_in_area(MetaRectangle *rect, MetaRectangle *work_area) {
 ...
 */
static gboolean find_first_fit(MetaWindow *window, MetaFrameBorders *borders,
                               /* visible windows on relevant workspaces */
                               GList *windows, int xinerama, int x, int y,
                               int *new_x, int *new_y) {
...
#ifdef WITH_VERBOSE_MODE
  {
    ...
    meta_topic(META_DEBUG_XINERAMA, "Natural xinerama is %s\n",
               xinerama_location_string);
  }
#endif
...
void meta_window_place(MetaWindow *window, MetaFrameBorders *borders, int x,
    ...
    switch (window->type) {
      ...
      case META_WINDOW_NORMAL:
        if (window->size_hints.flags & USPosition) {
          ...
          meta_topic(META_DEBUG_PLACEMENT,
                     "Honoring USPosition for %s instead of using placement "
                     "algorithm\n",
                     window->desc);
  ...
  if ((window->type == META_WINDOW_DIALOG ||
    ...
    if (parent) {
      ...
      if (borders) y += borders->visible.top;
  ...
  if (window->type == META_WINDOW_DIALOG ||
    ...
    meta_topic(META_DEBUG_PLACEMENT,
               "Centered window %s on screen %d xinerama %d\n", window->desc,
               window->screen->number, xi->number);
   ...
   */
  {
    ...
    while (tmp != NULL) {
      ...
      if (meta_window_showing_on_its_workspace(w) && w != window &&
          (window->workspace == w->workspace || window->on_all_workspaces ||
           w->on_all_workspaces))
  ...
  if (find_first_fit(window, borders, windows, xi->number, x, y, &x, &y))
    goto done_check_denied_focus;
   ...
   * be maximized.
   */
  if (window->has_maximize_func && window->decorated && !window->fullscreen) {
    MetaRectangle workarea;
    MetaRectangle outer;

    meta_window_get_work_area_for_xinerama(window, xi->number, &workarea);
    meta_window_get_outer_rect(window, &outer);
...
done_check_denied_focus:
  ...
  if (window->denied_focus_and_not_transient) {
    ...
    if (!meta_prefs_get_center_new_windows() && !found_fit) {
      ...
      found_fit = find_first_fit(window, borders, focus_window_list, xi->number,
                                 x, y, &x, &y);
