...
void meta_window_reload_properties(MetaWindow *window, const Atom *properties,
  ...
  meta_window_reload_properties_from_xwindow(window, window->xwindow,
                                             properties, n_properties, initial);
}
...
void meta_window_reload_properties_from_xwindow(MetaWindow *window,
  ...
  meta_prop_get_values(window->display, xwindow, values, n_properties);
...
static void complain_about_broken_client(MetaWindow *window,
                                         MetaPropValue *value,
                                         gboolean initial) {
  meta_warning(
...
static void reload_net_wm_window_type(MetaWindow *window, MetaPropValue *value,
                                      gboolean initial) {
  meta_window_update_net_wm_type(window);
}

static void reload_icon(MetaWindow *window, Atom atom) {
  meta_icon_cache_property_changed(&window->icon_cache, window->display, atom);
  meta_window_queue(window, META_QUEUE_UPDATE_ICON);
}

static void reload_net_wm_icon(MetaWindow *window, MetaPropValue *value,
                               gboolean initial) {
  reload_icon(window, window->display->atom__NET_WM_ICON);
}

static void reload_kwm_win_icon(MetaWindow *window, MetaPropValue *value,
                                gboolean initial) {
  reload_icon(window, window->display->atom__KWM_WIN_ICON);
}

static gboolean gtk_border_equal(GtkBorder *a, GtkBorder *b) {
  return (a->left == b->left && a->right == b->right && a->top == b->top &&
          a->bottom == b->bottom);
}

static void meta_window_set_custom_frame_extents(MetaWindow *window,
                                                 GtkBorder *extents) {
  if (extents) {
    if (window->has_custom_frame_extents &&
        gtk_border_equal(&window->custom_frame_extents, extents))
      return;
...
static void reload_gtk_frame_extents(MetaWindow *window, MetaPropValue *value,
                                     gboolean initial) {
  if (value->type != META_PROP_VALUE_INVALID) {
...
static void reload_struts(MetaWindow *window, MetaPropValue *value,
                          gboolean initial) {
  meta_window_update_struts(window);
}

static void reload_wm_window_role(MetaWindow *window, MetaPropValue *value,
                                  gboolean initial) {
...
static void reload_net_wm_user_time_window(MetaWindow *window,
  ...
  if (value->type != META_PROP_VALUE_INVALID) {
    ...
    if (window->user_time_window != None) {
       ...
       */
      meta_display_register_x_window(window->display, &window->user_time_window,
                                     window);
      ...
      meta_window_reload_property_from_xwindow(
          window, window->user_time_window,
          window->display->atom__NET_WM_USER_TIME, initial);
    }
  }
}
 ...
 */
static gboolean owner_of_process(pid_t process, uid_t *result) {
#ifdef HAVE_GTOP
...
static gboolean set_title_text(MetaWindow *window,
                               ...
                               const char *title, Atom atom, char **target) {
   ...
   * let's place that hostname in the title */
    ...
    *target =
        g_strdup_printf(_("%s (on %s)"), title, window->wm_client_machine);
    modified = TRUE;
    ...
    if (window_owner_is_us) {
      ...
      *target = g_strdup(title);
      ...
      if (window_owner == 0) {
        ...
        *target = g_strdup_printf(_("%s (as superuser)"), title);
      } else {
        ...
        if (found_name)
           ...
           * on this machine */
          *target = g_strdup_printf(_("%s (as %s)"), title, found_name);
        else
          /* Translators: the title of a window owned by another user
           * on this machine, whose name we don't know */
          *target = g_strdup_printf(_("%s (as another user)"), title);
      }
  ...
  if (modified && atom != None)
    meta_prop_set_utf8_string_hint(window->display, window->xwindow, atom,
                                   *target);

  /* Bug 330671 -- Don't forget to clear _NET_WM_VISIBLE_(ICON_)NAME */
  if (!modified && previous_was_modified) {
    meta_error_trap_push(window->display);
    XDeleteProperty(window->display->xdisplay, window->xwindow, atom);
...
static void set_window_title(MetaWindow *window, const char *title) {
  ...
  gboolean modified = set_title_text(
      window, window->using_net_wm_visible_name, title,
      window->display->atom__NET_WM_VISIBLE_NAME, &window->title);
  ...
  if (window->frame)
    meta_ui_set_frame_title(window->screen->ui, window->frame->xwindow,
                            window->title);
}
...
static void reload_net_wm_name(MetaWindow *window, MetaPropValue *value,
  ...
  if (value->type != META_PROP_VALUE_INVALID) {
    ...
    if (!initial) meta_window_reload_property(window, XA_WM_NAME, FALSE);
  }
}
...
static void set_icon_title(MetaWindow *window, const char *title) {
  gboolean modified = set_title_text(
      window, window->using_net_wm_visible_icon_name, title,
      window->display->atom__NET_WM_VISIBLE_ICON_NAME, &window->icon_name);
...
static void reload_net_wm_icon_name(MetaWindow *window, MetaPropValue *value,
  ...
  if (value->type != META_PROP_VALUE_INVALID) {
    ...
    if (!initial) meta_window_reload_property(window, XA_WM_ICON_NAME, FALSE);
  }
}
...
static void reload_net_wm_state(MetaWindow *window, MetaPropValue *value,
  ...
  if (value->type == META_PROP_VALUE_INVALID) return;

  i = 0;
  while (i < value->v.atom_list.n_atoms) {
    if (value->v.atom_list.atoms[i] ==
        window->display->atom__NET_WM_STATE_SHADED)
      window->shaded = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom__NET_WM_STATE_MAXIMIZED_HORZ)
      window->maximize_horizontally_after_placement = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom__NET_WM_STATE_MAXIMIZED_VERT)
      window->maximize_vertically_after_placement = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom__NET_WM_STATE_HIDDEN)
      window->minimize_after_placement = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom__NET_WM_STATE_MODAL)
      window->wm_state_modal = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom__NET_WM_STATE_SKIP_TASKBAR)
      ...
      window->wm_state_below = TRUE;
             ...
             window->display->atom__NET_WM_STATE_DEMANDS_ATTENTION)
      window->wm_state_demands_attention = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom__NET_WM_STATE_STICKY)
...
static void reload_mwm_hints(MetaWindow *window, MetaPropValue *value,
  ...
  if (hints->flags & MWM_HINTS_DECORATIONS) {
    ...
    if (hints->decorations == 0) window->mwm_decorated = FALSE;
...
static void reload_wm_class(MetaWindow *window, MetaPropValue *value,
                            gboolean initial) {
  if (window->res_class) g_free(window->res_class);
  if (window->res_name) g_free(window->res_name);
...
static void reload_net_wm_desktop(MetaWindow *window, MetaPropValue *value,
  ...
  if (value->type != META_PROP_VALUE_INVALID) {
    ...
    meta_topic(META_DEBUG_PLACEMENT, "Read initial workspace prop %d for %s\n",
...
static void reload_net_startup_id(MetaWindow *window, MetaPropValue *value,
  ...
  if (!window->constructing) {
    ...
    if (meta_screen_apply_startup_properties(window->screen, window)) {
      if (window->initial_timestamp_set) timestamp = window->initial_timestamp;
      if (window->initial_workspace_set)
        workspace = meta_screen_get_workspace_by_index(
            window->screen, window->initial_workspace);
  ...
  meta_verbose("New _NET_STARTUP_ID \"%s\" for %s\n",
               window->startup_id ? window->startup_id : "unset", window->desc);
}
...
#define FLAG_TOGGLED_ON(old, new, flag) \
  (((old)->flags & (flag)) == 0 && ((new)->flags & (flag)) != 0)
...
void meta_set_normal_hints(MetaWindow *window, XSizeHints *hints) {
  ...
  if (window->size_hints.flags & PBaseSize) {
    meta_topic(META_DEBUG_GEOMETRY, "Window %s sets base size %d x %d\n",
               window->desc, window->size_hints.base_width,
  ...
  if (window->size_hints.flags & PMaxSize) {
    meta_topic(META_DEBUG_GEOMETRY, "Window %s sets max size %d x %d\n",
               window->desc, window->size_hints.max_width,
  ...
  if (window->size_hints.flags & PResizeInc) {
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               window->desc, window->size_hints.width_inc,
  ...
  if (window->size_hints.flags & PAspect) {
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               window->desc, window->size_hints.min_aspect.x,
               window->size_hints.min_aspect.y, window->size_hints.max_aspect.x,
   ...
   */
  if (window->size_hints.min_aspect.y < 1) window->size_hints.min_aspect.y = 1;
  if (window->size_hints.max_aspect.y < 1) window->size_hints.max_aspect.y = 1;
  ...
  if ((minw - basew) % winc != 0) {
    ...
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               window->desc, winc, minw, basew, window->size_hints.min_width);
    minw = window->size_hints.min_width;
  }
  if (maxw != G_MAXINT && (maxw - basew) % winc != 0) {
    ...
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               window->desc, winc, maxw, basew, window->size_hints.max_width);
    maxw = window->size_hints.max_width;
  }
  if ((minh - baseh) % hinc != 0) {
    ...
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               window->desc, hinc, minh, baseh, window->size_hints.min_height);
    minh = window->size_hints.min_height;
  }
  if (maxh != G_MAXINT && (maxh - baseh) % hinc != 0) {
    ...
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               window->desc, hinc, maxh, baseh, window->size_hints.max_height);
    maxh = window->size_hints.max_height;
  }
  ...
  if (window->size_hints.max_height < window->size_hints.min_height) {
    ...
    meta_topic(META_DEBUG_GEOMETRY,
               ...
               "disabling resize\n",
               window->desc, window->size_hints.max_height,
               window->size_hints.min_height);
...
static void reload_normal_hints(MetaWindow *window, MetaPropValue *value,
  ...
  if (value->type != META_PROP_VALUE_INVALID) {
    ...
    if (!initial) meta_window_queue(window, META_QUEUE_MOVE_RESIZE);
  }
}
...
static void reload_wm_protocols(MetaWindow *window, MetaPropValue *value,
  ...
  i = 0;
  while (i < value->v.atom_list.n_atoms) {
    if (value->v.atom_list.atoms[i] == window->display->atom_WM_TAKE_FOCUS)
      window->take_focus = TRUE;
    else if (value->v.atom_list.atoms[i] ==
             window->display->atom_WM_DELETE_WINDOW)
      window->delete_window = TRUE;
    else if (value->v.atom_list.atoms[i] == window->display->atom__NET_WM_PING)
...
static void reload_wm_hints(MetaWindow *window, MetaPropValue *value,
  ...
  if (value->type != META_PROP_VALUE_INVALID) {
    ...
    if (hints->flags & InputHint) window->input = hints->input;
    ...
    if (hints->flags & IconMaskHint) window->wm_hints_mask = hints->icon_mask;

    meta_verbose(
        "Read WM_HINTS input: %d iconic: %d group leader: 0x%lx pixmap: 0x%lx "
        "mask: 0x%lx\n",
        window->input, window->initially_iconic, window->xgroup_leader,
  ...
  meta_icon_cache_property_changed(&window->icon_cache, window->display,
                                   XA_WM_HINTS);
...
static void reload_transient_for(MetaWindow *window, MetaPropValue *value,
   ...
   */
  if (window->xtransient_for != None && window->xgroup_leader != None &&
      window->xtransient_for != window->xgroup_leader)
    meta_window_group_leader_changed(window);

  if (!window->constructing) meta_window_queue(window, META_QUEUE_MOVE_RESIZE);
}

static void reload_gtk_theme_variant(MetaWindow *window, MetaPropValue *value,
                                     gboolean initial) {
...
void meta_display_init_window_prop_hooks(MetaDisplay *display) {
  MetaWindowPropHooks hooks[] = {
      ...
      {display->atom_WM_CLIENT_MACHINE, META_PROP_VALUE_STRING,
       reload_wm_client_machine},
      {display->atom__NET_WM_PID, META_PROP_VALUE_CARDINAL, reload_net_wm_pid},
      {display->atom__NET_WM_USER_TIME, META_PROP_VALUE_CARDINAL,
      ...
      {display->atom__KWM_WIN_ICON, META_PROP_VALUE_INVALID,
       reload_kwm_win_icon},
      {display->atom__NET_WM_ICON_NAME, META_PROP_VALUE_UTF8,
       reload_net_wm_icon_name},
      {XA_WM_ICON_NAME, META_PROP_VALUE_TEXT_PROPERTY, reload_wm_icon_name},
      {display->atom__NET_WM_STATE, META_PROP_VALUE_ATOM_LIST,
       reload_net_wm_state},
      {display->atom__MOTIF_WM_HINTS, META_PROP_VALUE_MOTIF_HINTS,
       reload_mwm_hints},
      {display->atom__NET_WM_ICON_GEOMETRY, META_PROP_VALUE_INVALID, NULL},
      {XA_WM_CLASS, META_PROP_VALUE_CLASS_HINT, reload_wm_class},
      {display->atom_WM_CLIENT_LEADER, META_PROP_VALUE_INVALID,
       complain_about_broken_client},
      {display->atom_SM_CLIENT_ID, META_PROP_VALUE_INVALID,
       complain_about_broken_client},
      {display->atom_WM_WINDOW_ROLE, META_PROP_VALUE_INVALID,
       reload_wm_window_role},
      {display->atom__NET_WM_WINDOW_TYPE, META_PROP_VALUE_INVALID,
       reload_net_wm_window_type},
      {display->atom__NET_WM_DESKTOP, META_PROP_VALUE_CARDINAL,
      ...
      {display->atom__NET_WM_STRUT_PARTIAL, META_PROP_VALUE_INVALID,
       reload_struts},
      {display->atom__GNOME_WM_STRUT_AREA, META_PROP_VALUE_INVALID,
       reload_struts},
      {display->atom__NET_STARTUP_ID, META_PROP_VALUE_UTF8,
       reload_net_startup_id},
      {display->atom__NET_WM_SYNC_REQUEST_COUNTER, META_PROP_VALUE_SYNC_COUNTER,
       reload_update_counter},
      {XA_WM_NORMAL_HINTS, META_PROP_VALUE_SIZE_HINTS, reload_normal_hints},
      {display->atom_WM_PROTOCOLS, META_PROP_VALUE_ATOM_LIST,
       reload_wm_protocols},
  ...
  while (cursor->property) {
     ...
     */
    g_hash_table_insert(display->prop_hooks, GINT_TO_POINTER(cursor->property),
                        cursor);
    cursor++;
  }
}
 ...
 */
static MetaWindowPropHooks *find_hooks(MetaDisplay *display, Atom property) {
  return g_hash_table_lookup(display->prop_hooks, GINT_TO_POINTER(property));
}
