 ...
 **/
static void meta_parse_options(int *argc, char ***argv,
                               MetaArguments *meta_args) {
  MetaArguments my_args = {NULL, NULL, NULL, FALSE, FALSE, FALSE, FALSE, FALSE};
  GOptionEntry options[] = {
      {"sm-disable", 0, 0, G_OPTION_ARG_NONE, &my_args.disable_sm,
       N_("Disable connection to session manager"), NULL},
      {"replace", 0, 0, G_OPTION_ARG_NONE, &my_args.replace_wm,
       N_("Replace the running window manager with Marco"), NULL},
      {"sm-client-id", 0, 0, G_OPTION_ARG_STRING, &my_args.client_id,
       N_("Specify session management ID"), "ID"},
      {"display", 'd', 0, G_OPTION_ARG_STRING, &my_args.display_name,
       N_("X Display to use"), "DISPLAY"},
      {"sm-save-file", 0, 0, G_OPTION_ARG_FILENAME, &my_args.save_file,
       N_("Initialize session from savefile"), "FILE"},
      {"version", 'v', 0, G_OPTION_ARG_NONE, &my_args.print_version,
       N_("Print version"), NULL},
      {"sync", 0, 0, G_OPTION_ARG_NONE, &my_args.sync,
       N_("Make X calls synchronous"), NULL},
      {"composite", 'c', COMPOSITE_OPTS_FLAGS, G_OPTION_ARG_NONE,
       &my_args.composite, N_("Turn compositing on"), NULL},
      {"no-composite", 0, COMPOSITE_OPTS_FLAGS, G_OPTION_ARG_NONE,
       &my_args.no_composite, N_("Turn compositing off"), NULL},
      {"no-force-fullscreen", 0, COMPOSITE_OPTS_FLAGS, G_OPTION_ARG_NONE,
       &my_args.no_force_fullscreen,
       N_("Don't make fullscreen windows that are maximized and have no "
          "decorations"),
       NULL},
      {"no-keybindings", 0, 0, G_OPTION_ARG_NONE, &my_args.no_keybindings,
       N_("Have all keybindings disabled on startup"), NULL},
      {NULL}};
  GOptionContext *ctx;
  GError *error = NULL;
...
static void meta_select_display(gchar *display_name) {
  ...
  if (display_name)
    ...
    envVar = g_strconcat("DISPLAY=", g_getenv("MARCO_DISPLAY"), NULL);
  /* DO NOT FREE envVar, putenv() sucks */
  putenv(envVar);
}

static void meta_finalize(void) {
  MetaDisplay *display = meta_get_display();
  if (display)
    meta_display_close(
        display, CurrentTime); /* I doubt correct timestamps matter here */
...
static gboolean on_sigterm(GIOChannel *source, GIOCondition condition,
                           gpointer user_data) {
...
int main(int argc, char **argv) {
  ...
  sigset_t empty_mask;
  MetaArguments meta_args;
  const gchar *log_domains[] = {NULL,           G_LOG_DOMAIN, "Gtk",
                                "Gdk",          "GLib",       "Pango",
                                "GLib-GObject", "GThread"};
  guint i;
  GIOChannel *channel;
  ...
  if (g_getenv("MARCO_VERBOSE")) meta_set_verbose(TRUE);
  if (g_getenv("MARCO_DEBUG")) meta_set_debugging(TRUE);
  ...
  if (meta_args.replace_wm) meta_set_replace_current_wm(TRUE);
  ...
  for (i = 0; i < G_N_ELEMENTS(log_domains); i++)
    g_log_set_handler(
        log_domains[i],
        G_LOG_LEVEL_MASK | G_LOG_FLAG_FATAL | G_LOG_FLAG_RECURSION, log_handler,
        NULL);
   ...
   */
  if (!meta_ui_have_a_theme()) meta_ui_set_current_theme("Simple", FALSE);
  ...
  if (!meta_ui_have_a_theme())
    meta_fatal(_("Could not find a theme! Be sure %s exists and contains the "
                 "usual themes.\n"),
  ...
  if (meta_args.no_force_fullscreen) meta_prefs_set_force_fullscreen(FALSE);

  if (meta_args.no_keybindings) meta_set_keybindings_disabled(TRUE);
...
void meta_quit(MetaExitCode code) {
  ...
  if (g_main_loop_is_running(meta_main_loop)) g_main_loop_quit(meta_main_loop);
}
