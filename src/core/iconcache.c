...
static void get_fallback_icons(MetaScreen *screen, GdkPixbuf **iconp,
...
static gboolean find_largest_sizes(gulong *data, gulong nitems, int *width,
                                   int *height) {
  *width = 0;
  *height = 0;
  ...
  while (nitems > 0) {
    ...
    if (nitems < 3) return FALSE; /* no space for w, h */

    w = data[0];
    h = data[1];

    if (nitems < ((gulong)(w * h) + 2)) return FALSE; /* not enough data */
...
static gboolean find_best_size(gulong *data, gulong nitems, int ideal_width,
                               int ideal_height, int *width, int *height,
                               gulong **start) {
  int best_w;
  int best_h;
  gulong *best_start;
  ...
  *start = NULL;
  ...
  if (!find_largest_sizes(data, nitems, &max_width, &max_height)) return FALSE;

  if (ideal_width < 0) ideal_width = max_width;
  if (ideal_height < 0) ideal_height = max_height;
  ...
  while (nitems > 0) {
    ...
    if (nitems < ((gulong)(w * h) + 2)) break; /* not enough data */
    ...
    if (best_start == NULL) {
      ...
      if (best_size < ideal_size && this_size >= ideal_size) replace = TRUE;
       ...
       * but still >= the ideal
       */
      else if (best_size > ideal_size && this_size >= ideal_size &&
               this_size < best_size)
        replace = TRUE;
    }
...
static gboolean read_rgb_icon(MetaDisplay *display, Window xwindow,
                              int ideal_width, int ideal_height,
                              int ideal_mini_width, int ideal_mini_height,
                              int *width, int *height, guchar **pixdata,
                              int *mini_width, int *mini_height,
                              guchar **mini_pixdata) {
  Atom type;
  int format;
  gulong nitems;
  ...
  result = XGetWindowProperty(
      display->xdisplay, xwindow, display->atom__NET_WM_ICON, 0, G_MAXLONG,
      False, XA_CARDINAL, &type, &format, &nitems, &bytes_after, &data);
  err = meta_error_trap_pop_with_return(display, TRUE);

  if (err != Success || result != Success) return FALSE;
  ...
  if (!find_best_size(data_as_long, nitems, ideal_width, ideal_height, &w, &h,
                      &best)) {
    XFree(data);
    return FALSE;
  }

  if (!find_best_size(data_as_long, nitems, ideal_mini_width, ideal_mini_height,
...
static void get_pixmap_geometry(MetaDisplay *display, Pixmap pixmap, int *w,
                                int *h, int *d) {
  Window root_ignored;
  ...
  XGetGeometry(display->xdisplay, pixmap, &root_ignored, &x_ignored, &y_ignored,
...
static gboolean try_pixmap_and_mask(MetaDisplay *display, Pixmap src_pixmap,
                                    Pixmap src_mask, GdkPixbuf **iconp,
                                    int ideal_width, int ideal_height,
  ...
  if (src_pixmap == None) return FALSE;
  ...
  unscaled =
      meta_gdk_pixbuf_get_from_pixmap(NULL, src_pixmap, 0, 0, 0, 0, w, h);

  if (unscaled && src_mask != None) {
    get_pixmap_geometry(display, src_mask, &w, &h, NULL);
    mask = meta_gdk_pixbuf_get_from_pixmap(NULL, src_mask, 0, 0, 0, 0, w, h);
  }
  ...
  if (unscaled) {
    *iconp = gdk_pixbuf_scale_simple(
        unscaled,
        ideal_width > 0 ? ideal_width : gdk_pixbuf_get_width(unscaled),
        ideal_height > 0 ? ideal_height : gdk_pixbuf_get_height(unscaled),
        GDK_INTERP_BILINEAR);
    *mini_iconp = gdk_pixbuf_scale_simple(
        unscaled,
    ...
    if (*iconp && *mini_iconp)
      ...
      if (*iconp) g_object_unref(G_OBJECT(*iconp));
      if (*mini_iconp) g_object_unref(G_OBJECT(*mini_iconp));
      return FALSE;
    }
  } else
    return FALSE;
...
static void get_kwm_win_icon(MetaDisplay *display, Window xwindow,
  ...
  *mask = None;
  ...
  result = XGetWindowProperty(display->xdisplay, xwindow,
                              display->atom__KWM_WIN_ICON, 0, G_MAXLONG, False,
                              display->atom__KWM_WIN_ICON, &type, &format,
...
gboolean meta_icon_cache_get_icon_invalidated(MetaIconCache *icon_cache) {
  if (icon_cache->origin <= USING_KWM_WIN_ICON &&
    ...
    return TRUE;
  else if (icon_cache->origin <= USING_WM_HINTS && icon_cache->wm_hints_dirty)
    return TRUE;
...
static void replace_cache(MetaIconCache *icon_cache, IconOrigin origin,
                          GdkPixbuf *new_icon, GdkPixbuf *new_mini_icon) {
  clear_icon_cache(icon_cache, FALSE);
...
static GdkPixbuf *scaled_from_pixdata(guchar *pixdata, int w, int h, int new_w,
                                      int new_h) {
  GdkPixbuf *src;
  GdkPixbuf *dest;

  src = gdk_pixbuf_new_from_data(pixdata, GDK_COLORSPACE_RGB, TRUE, 8, w, h,
                                 w * 4, free_pixels, NULL);
  ...
  if (w != h) {
    ...
    if (tmp) {
      ...
      gdk_pixbuf_copy_area(src, 0, 0, w, h, tmp, (size - w) / 2,
                           (size - h) / 2);
...
gboolean meta_read_icons(MetaScreen *screen, Window xwindow, char *res_name,
  ...
  *mini_iconp = NULL;
...

  {
    ...
    if (read_rgb_icon(screen->display, xwindow, ideal_width, ideal_height,
                      ideal_mini_width, ideal_mini_height, &w, &h, &pixdata,
  ...
  if (icon_cache->origin <= USING_WM_HINTS && icon_cache->wm_hints_dirty) {
     ...
     * hints change
     */
    if ((pixmap != icon_cache->prev_pixmap || mask != icon_cache->prev_mask ||
         icon_cache->wm_hints_dirty_forced) &&
        pixmap != None) {
      icon_cache->wm_hints_dirty_forced = FALSE;

      if (try_pixmap_and_mask(screen->display, pixmap, mask, iconp, ideal_width,
                              ideal_height, mini_iconp, ideal_mini_width,
                              ideal_mini_height)) {
  ...
  if ((icon_cache->want_fallback && icon_cache->origin < USING_FALLBACK_ICON) ||
    ...
    if (*iconp == NULL || *mini_iconp == NULL)
      get_fallback_icons(screen, iconp, ideal_width, ideal_height, mini_iconp,
                         ideal_mini_width, ideal_mini_height);
