...
static void init_random_ness(void) { srand(time(NULL)); }

static void get_random_rect(MetaRectangle *rect) {
  rect->x = rand() % 1600;
  rect->y = rand() % 1200;
...
static MetaRectangle *new_meta_rect(int x, int y, int width, int height) {
...
static MetaStrut *new_meta_strut(int x, int y, int width, int height,
                                 int side) {
  MetaStrut *temporary;
...
static MetaEdge *new_screen_edge(int x, int y, int width, int height,
                                 int side_type) {
  MetaEdge *temporary;
...
static MetaEdge *new_xinerama_edge(int x, int y, int width, int height,
                                   int side_type) {
  MetaEdge *temporary;
  temporary = g_new(MetaEdge, 1);
  temporary->rect.x = x;
...
static void test_overlap_funcs(void) {
...
static void test_basic_fitting(void) {
...
static void free_strut_list(GSList *struts) {
  GSList *tmp = struts;
  while (tmp) {
    g_free(tmp->data);
    tmp = tmp->next;
...
static GSList *get_strut_list(int which) {
  GSList *ans;
...
static GList *get_screen_region(int which) {
  GList *ret;
  GSList *struts;
...
static GList *get_screen_edges(int which) {
  GList *ret;
  GSList *struts;
  MetaRectangle basic_rect;
...
static GList *get_xinerama_edges(int which_xinerama_set, int which_strut_set) {
  GList *ret;
  GSList *struts;
  GList *xins;
  ...
  ret = meta_rectangle_find_nonintersected_xinerama_edges(&screenrect, xins,
                                                          struts);
...
static void verify_lists_are_equal(GList *code, GList *answer) {
  int which = 0;
  ...
  while (code && answer) {
    ...
    if (a->x != b->x || a->y != b->y || a->width != b->width ||
        a->height != b->height) {
      g_error(
          "%dth item in code answer answer lists do not match; "
          "code rect: %d,%d + %d,%d; answer rect: %d,%d + %d,%d\n",
          which, a->x, a->y, a->width, a->height, b->x, b->y, b->width,
          b->height);
    }
  ...
  if (code) {
    ...
    g_error(
        "code list longer than answer list by %d items; "
        "first extra item: %d,%d +%d,%d\n",
        g_list_length(code), tmp->x, tmp->y, tmp->width, tmp->height);
  }

  if (answer) {
    MetaRectangle *tmp = answer->data;
    g_error(
        "answer list longer than code list by %d items; "
        "first extra item: %d,%d +%d,%d\n",
        g_list_length(answer), tmp->x, tmp->y, tmp->width, tmp->height);
  }
}

static void test_regions_okay(void) {
  GList *region;
  GList *tmp;
  ...
  /*************************************************************/
  printf(
      "The next test intentionally causes a warning, "
...
static void test_region_fitting(void) {
  GList *region;
  MetaRectangle rect;
...
static void test_clamping_to_region(void) {
  GList *region;
  MetaRectangle rect;
  ...
  for (i = 0; i < NUM_RANDOM_RUNS; i++) {
    ...
    get_random_rect(&rect);
    temp = rect;
    meta_rectangle_clamp_to_fit_into_region(region, fixed_directions, &rect,
                                            &min_size);
    g_assert(meta_rectangle_could_fit_in_region(region, &rect));
...
static gboolean rect_overlaps_region(const GList *spanning_rects,
...
static void test_clipping_to_region(void) {
  GList *region;
  ...
  meta_rectangle_clip_to_region(region, fixed_directions, &rect);
  ...
  meta_rectangle_clip_to_region(region, FIXED_DIRECTION_X, &rect);
  g_assert(meta_rectangle_equal(&rect, &temp));
...
static void test_shoving_into_region(void) {
  GList *region;
  MetaRectangle rect, temp;
  FixedDirections fixed_directions = 0;
  int i;
  ...
  meta_rectangle_shove_into_region(region, fixed_directions, &rect);
  g_assert(meta_rectangle_equal(&rect, &temp));
  ...
  meta_rectangle_shove_into_region(region, FIXED_DIRECTION_X, &rect);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(300, 1000, 400, 200);
  temp = meta_rect(1200, 1000, 400, 200);
  meta_rectangle_shove_into_region(region, FIXED_DIRECTION_Y, &rect);
  g_assert(meta_rectangle_equal(&rect, &temp));
...
static void verify_edge_lists_are_equal(GList *code, GList *answer) {
  int which = 0;
  ...
  while (code && answer) {
    ...
    if (!meta_rectangle_equal(&a->rect, &b->rect) ||
        a->side_type != b->side_type || a->edge_type != b->edge_type) {
      g_error(
          "%dth item in code answer answer lists do not match; "
          "code rect: %d,%d + %d,%d; answer rect: %d,%d + %d,%d\n",
          which, a->rect.x, a->rect.y, a->rect.width, a->rect.height, b->rect.x,
          b->rect.y, b->rect.width, b->rect.height);
    }
  ...
  if (code) {
    ...
    g_error(
        ...
        g_list_length(code), tmp->rect.x, tmp->rect.y, tmp->rect.width,
        tmp->rect.height);
  }
  ...
  if (answer) {
    ...
    g_error(
        "answer list longer than code list by %d items; "
        "first extra item rect: %d,%d +%d,%d\n",
        g_list_length(answer), tmp->rect.x, tmp->rect.y, tmp->rect.width,
        tmp->rect.height);
  }
}

static void test_find_onscreen_edges(void) {
  GList *edges;
  GList *tmp;
...
static void test_find_nonintersected_xinerama_edges(void) {
  GList *edges;
  GList *tmp;
...
static void test_gravity_resize(void) {
  ...
  meta_rectangle_resize_with_gravity(&oldrect, &rect, NorthWestGravity, 20, 5);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(50, 300, 250, 400);
  temp = meta_rect(165, 300, 20, 5);
  meta_rectangle_resize_with_gravity(&rect, &rect, NorthGravity, 20, 5);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(50, 300, 250, 400);
  temp = meta_rect(280, 300, 20, 5);
  meta_rectangle_resize_with_gravity(&rect, &rect, NorthEastGravity, 20, 5);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(50, 300, 250, 400);
  temp = meta_rect(50, 695, 50, 5);
  meta_rectangle_resize_with_gravity(&rect, &rect, SouthWestGravity, 50, 5);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(50, 300, 250, 400);
  temp = meta_rect(150, 695, 50, 5);
  meta_rectangle_resize_with_gravity(&rect, &rect, SouthGravity, 50, 5);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(50, 300, 250, 400);
  temp = meta_rect(250, 695, 50, 5);
  meta_rectangle_resize_with_gravity(&rect, &rect, SouthEastGravity, 50, 5);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(167, 738, 237, 843);
  temp = meta_rect(167, 1113, 832, 93);
  meta_rectangle_resize_with_gravity(&rect, &rect, WestGravity, 832, 93);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(167, 738, 237, 843);
  temp = meta_rect(-131, 1113, 833, 93);
  meta_rectangle_resize_with_gravity(&rect, &rect, CenterGravity, 832, 93);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(300, 1000, 400, 200);
  temp = meta_rect(270, 994, 430, 212);
  meta_rectangle_resize_with_gravity(&rect, &rect, EastGravity, 430, 211);
  g_assert(meta_rectangle_equal(&rect, &temp));

  rect = meta_rect(300, 1000, 400, 200);
  temp = meta_rect(300, 1000, 430, 211);
  meta_rectangle_resize_with_gravity(&rect, &rect, StaticGravity, 430, 211);
  g_assert(meta_rectangle_equal(&rect, &temp));
...
static void test_find_closest_point_to_line(void) {
  ...
  answer_x = 2.4;
  answer_y = 19;
  meta_rectangle_find_linepoint_closest_to_point(x1, y1, x2, y2, px, py, &rx,
                                                 &ry);
