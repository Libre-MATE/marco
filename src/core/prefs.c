...
static MetaActionTitlebar action_double_click_titlebar =
    META_ACTION_TITLEBAR_TOGGLE_MAXIMIZE;
static MetaActionTitlebar action_middle_click_titlebar =
    META_ACTION_TITLEBAR_LOWER;
static MetaActionTitlebar action_right_click_titlebar =
...
static gboolean alt_tab_expand_to_fit_title =
...
static char *workspace_names[MAX_REASONABLE_WORKSPACES] = {
    NULL,
};
...
static gboolean update_key_binding(const char *name, gchar *value);
...
static void change_notify(GSettings *settings, gchar *key, gpointer user_data);
...
static void mouse_button_mods_handler(MetaPreference, const gchar *,
                                      gboolean *);
static void button_layout_handler(MetaPreference, const gchar *, gboolean *);
static void show_desktop_skip_list_handler(MetaPreference, const gchar *,
                                           gboolean *);

static gboolean update_binding(MetaKeyPref *binding, gchar *value);
...
typedef struct {
   ...
   */
  void (*handler)(MetaPreference pref, const gchar *string_value,
...
static MetaIntPreference preferences_int[] = {
    ...
    {
        "auto-raise-delay", KEY_GENERAL_SCHEMA, META_PREF_AUTO_RAISE_DELAY,
        &auto_raise_delay, 0, 10000, 0,
...
static void handle_preference_init_enum(void) {
  ...
  while (cursor->key != NULL) {
    ...
    value = g_settings_get_enum(SETTINGS(cursor->schema), cursor->key);
    *cursor->target = value;
...
static void handle_preference_init_bool(void) {
  ...
  while (cursor->key != NULL) {
    if (cursor->target != NULL)
      *cursor->target =
          g_settings_get_boolean(SETTINGS(cursor->schema), cursor->key);
...
static void handle_preference_init_string(void) {
  ...
  while (cursor->key != NULL) {
    ...
    value = g_settings_get_string(SETTINGS(cursor->schema), cursor->key);
    ...
    if (cursor->handler) {
      ...
      if (*(cursor->target)) g_free(*(cursor->target));
...
static void handle_preference_init_int(void) {
  ...
  while (cursor->key != NULL) {
    ...
    value = g_settings_get_int(SETTINGS(cursor->schema), cursor->key);

    if (value < cursor->minimum || value > cursor->maximum) {
      /* FIXME: check if this can be avoided by GSettings */
      meta_warning(
          _("%d stored in GSettings key %s is out of range %d to %d\n"), value,
          cursor->key, cursor->minimum, cursor->maximum);
...
static gboolean handle_preference_update_enum(const gchar *key,
  ...
  while (cursor->key != NULL && strcmp(key, cursor->key) != 0) ++cursor;

  if (cursor->key == NULL) /* Didn't recognise that key. */
    return FALSE;
  ...
  *cursor->target = g_settings_get_enum(settings, key);
  ...
  if (old_value != *((gint *)cursor->target)) queue_changed(cursor->pref);
...
static gboolean handle_preference_update_bool(const gchar *key,
  ...
  if (cursor->target == NULL) /* No work for us to do. */
    return TRUE;
  ...
  *((gboolean *)cursor->target) = g_settings_get_boolean(settings, key);
  ...
  if (old_value != *((gboolean *)cursor->target)) queue_changed(cursor->pref);
...
static gboolean handle_preference_update_string(const gchar *key,
  ...
  if (cursor->handler)
    ...
    inform_listeners = (value == NULL && *(cursor->target) == NULL) ||
                       (value != NULL && *(cursor->target) != NULL &&
                        strcmp(value, *(cursor->target)) == 0);
  }

  if (inform_listeners) queue_changed(cursor->pref);
...
void meta_prefs_init(void) {
  if (settings_general != NULL) return;
  ...
  g_hash_table_insert(settings_schemas, KEY_SCREEN_BINDINGS_SCHEMA,
                      settings_screen_bindings);
  g_hash_table_insert(settings_schemas, KEY_WINDOW_BINDINGS_SCHEMA,
                      settings_window_bindings);
  g_hash_table_insert(settings_schemas, KEY_WORKSPACE_NAME_SCHEMA,
                      settings_workspace_names);
  g_hash_table_insert(settings_schemas, KEY_MATE_INTERFACE_SCHEMA,
                      settings_mate_interface);
  g_hash_table_insert(settings_schemas, KEY_MATE_TERMINAL_SCHEMA,
                      settings_mate_terminal);
  g_hash_table_insert(settings_schemas, KEY_MATE_MOUSE_SCHEMA,
                      settings_mate_mouse);

  g_signal_connect(settings_general, "changed", G_CALLBACK(change_notify),
                   NULL);
  g_signal_connect(settings_command, "changed", G_CALLBACK(change_notify),
                   NULL);
  g_signal_connect(settings_screen_bindings, "changed",
                   G_CALLBACK(change_notify), NULL);
  g_signal_connect(settings_window_bindings, "changed",
                   G_CALLBACK(change_notify), NULL);
  g_signal_connect(settings_workspace_names, "changed",
                   G_CALLBACK(change_notify), NULL);

  g_signal_connect(settings_mate_interface,
                   "changed::" KEY_MATE_INTERFACE_ACCESSIBILITY,
                   G_CALLBACK(change_notify), NULL);
  g_signal_connect(settings_mate_interface,
                   "changed::" KEY_MATE_INTERFACE_ENABLE_ANIMATIONS,
                   G_CALLBACK(change_notify), NULL);
  g_signal_connect(settings_mate_terminal,
                   "changed::" KEY_MATE_TERMINAL_COMMAND,
                   G_CALLBACK(change_notify), NULL);
  g_signal_connect(settings_mate_mouse, "changed::" KEY_MATE_MOUSE_CURSOR_THEME,
                   G_CALLBACK(change_notify), NULL);
  g_signal_connect(settings_mate_mouse, "changed::" KEY_MATE_MOUSE_CURSOR_SIZE,
                   G_CALLBACK(change_notify), NULL);
...
gboolean (*preference_update_handler[])(const gchar *, GSettings *) = {
    handle_preference_update_enum, handle_preference_update_bool,
    handle_preference_update_string, handle_preference_update_int, NULL};
...
static void change_notify(GSettings *settings, gchar *key, gpointer user_data) {
  ...
  if (g_strcmp0(schema_name, KEY_WINDOW_BINDINGS_SCHEMA) == 0 ||
    ...
    if (update_key_binding(key, str)) queue_changed(META_PREF_KEYBINDINGS);
    ...
    if (update_command(key, str)) queue_changed(META_PREF_COMMANDS);
...
static void maybe_give_disable_workarounds_warning(void) {
  ...
  if (first_disable && disable_workarounds) {
    ...
    meta_warning(
        _("Workarounds for broken applications disabled. "
...
MetaFocusMode meta_prefs_get_focus_mode(void) { return focus_mode; }
...
gboolean meta_prefs_get_raise_on_click(void) { return raise_on_click; }

const char *meta_prefs_get_theme(void) { return current_theme; }

const char *meta_prefs_get_cursor_theme(void) { return cursor_theme; }
...
int meta_prefs_get_alt_tab_max_columns(void) { return alt_tab_max_columns; }
...
gboolean meta_prefs_is_in_skip_list(char *class) {
  ...
  for (item = show_desktop_skip_list; item; item = item->next) {
    if (!g_ascii_strcasecmp(class, item->data)) return TRUE;
  }
  return FALSE;
}
...
static void titlebar_handler(MetaPreference pref, const gchar *string_value,
                             gboolean *inform_listeners) {
  PangoFontDescription *new_desc = NULL;

  if (string_value) new_desc = pango_font_description_from_string(string_value);
  ...
  if (titlebar_font && pango_font_description_equal(new_desc, titlebar_font)) {
  ...
  if (titlebar_font) pango_font_description_free(titlebar_font);

  titlebar_font = new_desc;
}

static void theme_name_handler(MetaPreference pref, const gchar *string_value,
                               gboolean *inform_listeners) {
...
static void show_desktop_skip_list_handler(MetaPreference pref,
                                           const gchar *string_value,
                                           gboolean *inform_listeners) {
  gchar **tokens;
  gchar **tok;
  ...
  if (!(tokens = g_strsplit(string_value, ",", -1))) return;
...
static gboolean button_layout_equal(const MetaButtonLayout *a,
  ...
  while (i < MAX_BUTTONS_PER_CORNER) {
    if (a->left_buttons[i] != b->left_buttons[i]) return FALSE;
    if (a->right_buttons[i] != b->right_buttons[i]) return FALSE;
...
static void button_layout_handler(MetaPreference pref,
  ...
  if (string_value) sides = g_strsplit(string_value, ":", 2);
    ...
    while (buttons[b] != NULL) {
      ...
      if (i > 0 && strcmp("spacer", buttons[b]) == 0) {
        ...
        if (f != META_BUTTON_FUNCTION_LAST && !used[f]) {
          ...
          if (f != META_BUTTON_FUNCTION_LAST) new_layout.left_buttons[i++] = f;
    ...
    while (buttons[b] != NULL) {
      ...
      if (i > 0 && strcmp("spacer", buttons[b]) == 0) {
        ...
        if (f != META_BUTTON_FUNCTION_LAST && !used[f]) {
          ...
          if (f != META_BUTTON_FUNCTION_LAST) new_layout.right_buttons[i++] = f;
...
int meta_prefs_get_num_workspaces(void) { return num_workspaces; }

MetaWrapStyle meta_prefs_get_wrap_style(void) { return wrap_style; }
...
void meta_prefs_set_num_workspaces(int n_workspaces) {
  if (n_workspaces < 1) n_workspaces = 1;
  if (n_workspaces > MAX_REASONABLE_WORKSPACES)
    n_workspaces = MAX_REASONABLE_WORKSPACES;

  g_settings_set_int(settings_general, KEY_GENERAL_NUM_WORKSPACES,
                     n_workspaces);
}
...
static void init_bindings(GSettings *settings) {
...
static gboolean update_binding(MetaKeyPref *binding, gchar *value) {
  ...
  if (value) {
    if (!meta_ui_parse_accelerator(value, &keysym, &keycode, &mods)) {
      ...
      meta_warning(_("\"%s\" found in configuration database is not a valid "
  ...
  if (binding->add_shift && 0 != keysym &&
      (META_VIRTUAL_SHIFT_MASK == mods || 0 == mods)) {
    gchar *old_setting;

    meta_warning(
        "Cannot bind \"%s\" to %s: it needs a modifier "
        "such as Ctrl or Alt.\n",
        binding->name, value);

    old_setting = meta_ui_accelerator_name(combo->keysym, combo->modifiers);
     ...
     * onto here.
     */
    g_settings_set_string(settings_screen_bindings, binding->name, old_setting);
  ...
  changed = FALSE;
  if (keysym != combo->keysym || keycode != combo->keycode ||
    ...
    meta_topic(META_DEBUG_KEYBINDINGS,
               "New keybinding for \"%s\" is keysym = 0x%x keycode = 0x%x mods "
               "= 0x%x\n",
               binding->name, combo->keysym, combo->keycode, combo->modifiers);
  } else {
    meta_topic(META_DEBUG_KEYBINDINGS, "Keybinding for \"%s\" is unchanged\n",
               binding->name);
  }
 ...
 * notify
 */
static gboolean find_and_update_binding(MetaKeyPref *bindings, const char *name,
                                        gchar *value) {
  const char *key;
  int i;
  ...
  i = 0;
  while (bindings[i].name && strcmp(key, bindings[i].name) != 0) ++i;
...
static gboolean update_command(const char *name, const char *value) {
  ...
  p = strrchr(name, '-');
  if (p == NULL) {
    meta_topic(META_DEBUG_KEYBINDINGS, "Command %s has no dash?\n", name);
    return FALSE;
  }
  ...
  if (g_ascii_isdigit(*p)) {
    ...
    if (strcmp(name, "command-screenshot") == 0) {
      ...
      meta_topic(META_DEBUG_KEYBINDINGS, "Command %s doesn't end in number?\n",
                 name);
      return FALSE;
    }
  }
  ...
  if ((commands[i] == NULL && value == NULL) ||
    ...
    meta_topic(META_DEBUG_KEYBINDINGS, "Command %d is unchanged\n", i);
    return FALSE;
  }
  ...
  meta_topic(META_DEBUG_KEYBINDINGS, "Updated command %d to \"%s\"\n", i,
             commands[i] ? commands[i] : "none");
...
const char *meta_prefs_get_terminal_command(void) { return terminal_command; }
...
static gboolean update_workspace_name(const char *name, const char *value) {
  ...
  p = strrchr(name, '-');
  if (p == NULL) {
    meta_topic(META_DEBUG_PREFS, "Workspace name %s has no dash?\n", name);
    return FALSE;
  }
  ...
  if (!g_ascii_isdigit(*p)) {
    meta_topic(META_DEBUG_PREFS, "Workspace name %s doesn't end in number?\n",
               name);
    return FALSE;
  }
  ...
  if (workspace_names[i] && value && strcmp(workspace_names[i], value) == 0) {
    meta_topic(META_DEBUG_PREFS, "Workspace name %d is unchanged\n", i);
    return FALSE;
  }
  ...
  meta_topic(META_DEBUG_PREFS, "Updated workspace name %d to \"%s\"\n", i,
             workspace_names[i] ? workspace_names[i] : "none");
...
const char *meta_prefs_get_workspace_name(int i) {
  ...
  meta_topic(META_DEBUG_PREFS, "Getting workspace name for %d: \"%s\"\n", i,
             workspace_names[i]);
...
void meta_prefs_change_workspace_name(int i, const char *name) {
  ...
  meta_topic(META_DEBUG_PREFS, "Changing name of workspace %d to %s\n", i,
             name ? name : "none");
  ...
  if ((name == NULL && workspace_names[i] == NULL) ||
    ...
    meta_topic(META_DEBUG_PREFS, "Workspace %d already has name %s\n", i,
               name ? name : "none");
    return;
  }
  ...
  if (name != NULL)
    g_settings_set_string(settings_workspace_names, key, name);
  else
    g_settings_set_string(settings_workspace_names, key, "");
...
gboolean meta_prefs_get_visual_bell(void) { return provide_visual_bell; }

gboolean meta_prefs_bell_is_audible(void) { return bell_is_audible; }
...
gboolean meta_prefs_get_auto_raise(void) { return auto_raise; }

int meta_prefs_get_auto_raise_delay(void) { return auto_raise_delay; }

gboolean meta_prefs_get_reduced_resources(void) { return reduced_resources; }

gboolean meta_prefs_get_mate_accessibility() { return mate_accessibility; }

gboolean meta_prefs_get_mate_animations() { return mate_animations; }
...
MetaKeyBindingAction meta_prefs_get_keybinding_action(const char *name) {
  ...
  while (i >= 0) {
    if (strcmp(key_bindings[i].name, name) == 0) return (MetaKeyBindingAction)i;
...
void meta_prefs_get_window_binding(const char *name, unsigned int *keysym,
  ...
  i = G_N_ELEMENTS(key_bindings) - 2; /* -2 for dummy entry at end */
  while (i >= 0) {
    if (key_bindings[i].per_window && strcmp(key_bindings[i].name, name) == 0) {
...
gboolean meta_prefs_get_compositing_manager(void) {
  if (use_force_compositor_manager) return force_compositor_manager;
  return compositing_manager;
}
...
gboolean meta_prefs_get_center_new_windows(void) { return center_new_windows; }

gboolean meta_prefs_get_allow_tiling() { return allow_tiling; }

gboolean meta_prefs_get_allow_top_tiling() { return allow_top_tiling; }

gboolean meta_prefs_get_allow_tile_cycling() { return allow_tile_cycling; }
...
gboolean meta_prefs_show_tab_border(void) { return show_tab_border; }

gboolean meta_prefs_get_force_fullscreen(void) { return force_fullscreen; }

MetaPlacementMode meta_prefs_get_placement_mode(void) { return placement_mode; }
...
void meta_prefs_set_compositing_fast_alt_tab(gboolean whether) {
  g_settings_set_boolean(settings_general, KEY_GENERAL_COMPOSITOR_FAST_ALT_TAB,
                         whether);
}

void meta_prefs_set_center_new_windows(gboolean whether) {
  g_settings_set_boolean(settings_general, KEY_GENERAL_CENTER_NEW_WINDOWS,
                         whether);
}
