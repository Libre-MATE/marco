...
#define DISPLAY_COMPOSITOR(display) \
...
static double gaussian(double r, double x, double y) {
  return ((1 / (sqrt(2 * G_PI * r))) * exp((-(x * x + y * y)) / (2 * r * r)));
}
...
static void dump_xserver_region(const char *location, MetaDisplay *display,
  ...
  if (region) {
    ...
    if (nrects > 0) {
      ...
      fprintf(stderr, "%s (XSR): %d rects, bounds: %d,%d (%d,%d)\n", location,
              nrects, bounds.x, bounds.y, bounds.width, bounds.height);
      for (i = 1; i < nrects; i++)
 ...
 *  center  +-----+-------------------+-----+
 */
static guchar sum_gaussian(conv *map, double opacity, int x, int y, int width,
                           int height) {
  ...
  for (fy = fy_start; fy < fy_end; fy++) {
    ...
    for (fx = fx_start; fx < fx_end; fx++) v += *g_data++;
  }
...
static void presum_gaussian(shadow *shad) {
  ...
  for (x = 0; x <= msize; x++) {
    ...
    for (y = 0; y <= x; y++) {
      shad->shadow_corner[25 * (msize + 1) * (msize + 1) + y * (msize + 1) +
                          x] =
          sum_gaussian(map, 1, x - centre, y - centre, msize * 2, msize * 2);

      shad->shadow_corner[25 * (msize + 1) * (msize + 1) + x * (msize + 1) +
                          y] =
          shad->shadow_corner[25 * (msize + 1) * (msize + 1) + y * (msize + 1) +
                              x];
...
static void generate_shadows(MetaCompScreen *info) {
  double radii[LAST_SHADOW_TYPE] = {[META_SHADOW_SMALL] = SHADOW_SMALL_RADIUS,
...
static XImage *make_shadow(MetaDisplay *display, MetaScreen *screen,
                           MetaShadowType shadow_type, double opacity,
  ...
  ximage = XCreateImage(xdisplay, DefaultVisual(xdisplay, screen_number), 8,
                        ZPixmap, 0, (char *)data, swidth, sheight, 8,
                        swidth * sizeof(guchar));
  if (!ximage) {
    g_free(data);
    return NULL;
  }
  ...
  if (msize > 0)
    ...
    d = sum_gaussian(shad->gaussian_map, opacity, centre, centre, width,
                     height);
  ...
  for (y = 0; y < ylimit; y++) {
    for (x = 0; x < xlimit; x++) {
      if (xlimit == msize && ylimit == msize)
        d = shad->shadow_corner[opacity_int * (msize + 1) * (msize + 1) +
                                y * (msize + 1) + x];
      else
        d = sum_gaussian(shad->gaussian_map, opacity, x - centre, y - centre,
                         width, height);
  ...
  if (x_diff > 0 && ylimit > 0) {
    for (y = 0; y < ylimit; y++) {
      if (ylimit == msize)
        ...
        d = sum_gaussian(shad->gaussian_map, opacity, centre, y - centre, width,
                         height);
  ...
  for (x = 0; x < xlimit; x++) {
    if (xlimit == msize)
      ...
      d = sum_gaussian(shad->gaussian_map, opacity, x - centre, centre, width,
                       height);
...
static XserverRegion cairo_region_to_xserver_region(Display *xdisplay,
...
static void shadow_picture_clip(Display *xdisplay, Picture shadow_picture,
                                MetaCompWindow *cw, MetaFrameBorders borders,
                                int width, int height) {
  int shadow_dx;
  int shadow_dy;
  ...
  if (visible_region == NULL) return;

  shadow_dx =
      -1 * (int)shadow_offsets_x[cw->shadow_type] - borders.invisible.left;
  shadow_dy =
      -1 * (int)shadow_offsets_y[cw->shadow_type] - borders.invisible.top;
  ...
  XFixesTranslateRegion(xdisplay, region2, shadow_dx, shadow_dy);
...
static Picture shadow_picture(MetaDisplay *display, MetaScreen *screen,
                              MetaCompWindow *cw, double opacity,
                              MetaFrameBorders borders, int width, int height,
                              int *wp, int *hp) {
  Display *xdisplay = meta_display_get_xdisplay(display);
  XImage *shadow_image;
  ...
  shadow_image =
      make_shadow(display, screen, cw->shadow_type, opacity, width, height);
  if (!shadow_image) return None;

  shadow_pixmap = XCreatePixmap(xdisplay, xroot, shadow_image->width,
  ...
  shadow_picture = XRenderCreatePicture(
      xdisplay, shadow_pixmap,
      XRenderFindStandardFormat(xdisplay, PictStandardA8), 0, 0);
  if (!shadow_picture) {
    XDestroyImage(shadow_image);
...
static MetaCompWindow *find_window_for_child_window_in_display(
    MetaDisplay *display, Window xwindow) {
  Window ignored1, *ignored2;
  Window parent;
  guint ignored_children;

  XQueryTree(meta_display_get_xdisplay(display), xwindow, &ignored1, &parent,
             &ignored2, &ignored_children);

  if (parent != None) return find_window_in_display(display, parent);

  return NULL;
}

#ifdef HAVE_PRESENT
static MetaScreen *find_screen_from_output(MetaDisplay *display,
                                           Window output) {
  int i;
  Display *xdisplay = meta_display_get_xdisplay(display);
  ...
  for (i = 0; i < ScreenCount(xdisplay); i++) {
    MetaScreen *screen =
        meta_display_screen_for_x_screen(display, ScreenOfDisplay(xdisplay, i));
    MetaCompScreen *info = meta_screen_get_compositor_data(screen);

    if (info->output == output) return screen;
  }
  return NULL;
}
#endif /* HAVE_PRESENT */

static Picture solid_picture(MetaDisplay *display, MetaScreen *screen,
                             gboolean argb, double a, double r, double g,
                             double b) {
  Display *xdisplay = meta_display_get_xdisplay(display);
  Pixmap pixmap;
  Picture picture;
  ...
  render_format = XRenderFindStandardFormat(
      xdisplay, argb ? PictStandardARGB32 : PictStandardA8);
  ...
  pa.repeat = TRUE;
  picture =
      XRenderCreatePicture(xdisplay, pixmap, render_format, CPRepeat, &pa);
  if (picture == None) {
...
static Picture root_tile(MetaScreen *screen) {
  ...
  for (p = 0; p < 2; p++) {
    ...
    if (XGetWindowProperty(xdisplay, xroot, background_atoms[p], 0, 4, FALSE,
                           AnyPropertyType, &actual_type, &actual_format,
                           &nitems, &bytes_after, &prop) == Success) {
      if (actual_type == pixmap_atom && actual_format == 32 && nitems == 1) {
  ...
  pa.repeat = TRUE;
  format =
      XRenderFindVisualFormat(xdisplay, DefaultVisual(xdisplay, screen_number));
...
static Pixmap create_root_pixmap(MetaScreen *screen) {
  MetaDisplay *display = meta_screen_get_display(screen);
  Display *xdisplay = meta_display_get_xdisplay(display);
  MetaCompScreen *info = meta_screen_get_compositor_data(screen);
  Window xroot = meta_screen_get_xroot(screen);
  ...
  pixmap = XCreatePixmap(xdisplay, xroot, screen_width, screen_height, depth);

  return pixmap;
}

static Picture create_root_buffer(MetaScreen *screen, Pixmap root_pixmap) {
  MetaDisplay *display = meta_screen_get_display(screen);
  Display *xdisplay = meta_display_get_xdisplay(display);
  MetaCompScreen *info = meta_screen_get_compositor_data(screen);
  Picture pict;
...
static void paint_root(MetaScreen *screen, Picture root_buffer) {
  ...
  XRenderComposite(xdisplay, PictOpSrc, info->root_tile, None, root_buffer, 0,
...
static gboolean window_has_shadow(MetaCompWindow *cw) {
  ...
  if (info == NULL || !info->have_shadows) return FALSE;
...
static XserverRegion win_extents(MetaCompWindow *cw) {
  ...
  if (cw->needs_shadow) {
    ...
    if (cw->window) {
      ...
      if (frame) meta_frame_calc_borders(frame, &borders);
    }

    cw->shadow_dx =
        (int)shadow_offsets_x[cw->shadow_type] + borders.invisible.left;
    cw->shadow_dy =
        (int)shadow_offsets_y[cw->shadow_type] + borders.invisible.top;
    ...
    if (!cw->shadow) {
      ...
      cw->shadow = shadow_picture(
          display, screen, cw, opacity, borders,
...
static XserverRegion border_size(MetaCompWindow *cw) {
  ...
  border = XFixesCreateRegionFromWindow(xdisplay, cw->id, WindowRegionBounding);
  meta_error_trap_pop(display, FALSE);

  g_return_val_if_fail(border != None, None);
  XFixesTranslateRegion(xdisplay, border, cw->attrs.x + cw->attrs.border_width,
...
static Picture get_window_picture(MetaCompWindow *cw) {
  ...
  if (error_code != 0) cw->back_pixmap = None;

  if (cw->back_pixmap != None) draw = cw->back_pixmap;
...
static void paint_dock_shadows(MetaScreen *screen, Picture root_buffer,
                               XserverRegion region) {
  MetaDisplay *display = meta_screen_get_display(screen);
  Display *xdisplay = meta_display_get_xdisplay(display);
  MetaCompScreen *info = meta_screen_get_compositor_data(screen);
  ...
  for (d = info->dock_windows; d; d = d->next) {
    ...
    if (cw->shadow) {
      ...
      XFixesIntersectRegion(xdisplay, shadow_clip, cw->border_clip, region);

      XFixesSetPictureClipRegion(xdisplay, root_buffer, 0, 0, shadow_clip);

      XRenderComposite(xdisplay, PictOpOver, info->black_picture, cw->shadow,
                       root_buffer, 0, 0, 0, 0, cw->attrs.x + cw->shadow_dx,
                       cw->attrs.y + cw->shadow_dy, cw->shadow_width,
                       cw->shadow_height);
...
#ifdef HAVE_PRESENT
static gboolean present_flip(MetaScreen *screen, XserverRegion region,
                             Pixmap pixmap) {
  ...
  meta_error_trap_push(display);
  XPresentPixmap(xdisplay, info->output, pixmap, present_serial, None, region,
                 0, 0, None, None, None, PresentOptionNone, 0, 1, 0, NULL, 0);
  ...
  if (error_code) {
    ...
    if (error_code == BadWindow)
      g_warning(
          "XPresent is not compatible with your current system configuration.");
...
static void paint_windows(MetaScreen *screen, GList *windows,
                          Picture root_buffer, Pixmap root_pixmap,
                          XserverRegion region) {
  MetaDisplay *display = meta_screen_get_display(screen);
  MetaCompScreen *info = meta_screen_get_compositor_data(screen);
  Display *xdisplay = meta_display_get_xdisplay(display);
  ...
  for (index = windows; index; index = index->next) {
    ...
    if (cw->attrs.map_state != IsViewable) continue;
    ...
    if (cw->picture == None) cw->picture = get_window_picture(cw);
    ...
    if (cw->border_size == None) cw->border_size = border_size(cw);

    if (cw->extents == None) cw->extents = win_extents(cw);
    ...
    if (cw->mode == WINDOW_SOLID) {
      ...
      XRenderComposite(xdisplay, PictOpSrc, cw->picture, None, root_buffer, 0,
                       0, 0, 0, x, y, wid, hei);

      if (cw->type == META_COMP_WINDOW_DESKTOP) {
        if (desktop_region) {
          XFixesUnionRegion(xdisplay, desktop_region, desktop_region,
                            paint_region);
        } else {
      ...
      XFixesSubtractRegion(xdisplay, paint_region, paint_region,
                           cw->border_size);
    }
  ...
  if (desktop_region != None) XFixesDestroyRegion(xdisplay, desktop_region);
    ...
    if (cw->picture) {
      if (cw->shadow && cw->type != META_COMP_WINDOW_DOCK) {
        ...
        XRenderComposite(xdisplay, PictOpOver, info->black_picture, cw->shadow,
                         ...
                         cw->attrs.y + cw->shadow_dy, cw->shadow_width,
                         cw->shadow_height);
        if (shadow_clip) XFixesDestroyRegion(xdisplay, shadow_clip);
      }
      ...
      XFixesIntersectRegion(xdisplay, cw->border_clip, cw->border_clip,
                            cw->border_size);
      XFixesSetPictureClipRegion(xdisplay, root_buffer, 0, 0, cw->border_clip);
      if (cw->mode == WINDOW_ARGB) {
        int x, y, wid, hei;
...
#endif /* HAVE_PRESENT */
  {
    XRenderComposite(xdisplay, PictOpSrc, root_buffer, None, info->root_picture,
...
static void paint_all(MetaScreen *screen, XserverRegion region, int b) {
  MetaCompScreen *info = meta_screen_get_compositor_data(screen);
  MetaDisplay *display = meta_screen_get_display(screen);
  Display *xdisplay = meta_display_get_xdisplay(display);
  int screen_width, screen_height;
  ...
  if (DISPLAY_COMPOSITOR(display)->show_redraw) {
    ...
    XRenderComposite(xdisplay, PictOpOver, overlay, None, info->root_picture, 0,
                     0, 0, 0, 0, 0, screen_width, screen_height);
  ...
  paint_windows(screen, info->windows, info->root_buffers[b],
                info->root_pixmaps[b], region);
}
  ...
  if (info->all_damage != None) {
    ...
    if (info->use_present) {
      if (!info->present_pending) {
        ...
        meta_error_trap_push(display);
        if (info->prev_damage) {
          XFixesUnionRegion(xdisplay, info->prev_damage, info->prev_damage,
                            damage);
        ...
        if (++info->root_current >= NUM_BUFFER) info->root_current = 0;

        if (info->prev_damage) XFixesDestroyRegion(xdisplay, info->prev_damage);
...
static void add_repair(MetaDisplay *display) {
  ...
  if (compositor->repaint_id > 0) return;

#if 1
  compositor->repaint_id = g_idle_add_full(
      G_PRIORITY_HIGH_IDLE, compositor_idle_cb, compositor, NULL);
#else
  /* Limit it to 50fps */
  compositor->repaint_id = g_timeout_add_full(
      G_PRIORITY_HIGH, 20, compositor_idle_cb, compositor, NULL);
#endif
}
#endif
...
static void repair_win(MetaCompWindow *cw) {
  ...
  if (!cw->damaged) {
    ...
    XFixesTranslateRegion(xdisplay, parts, cw->attrs.x + cw->attrs.border_width,
                          cw->attrs.y + cw->attrs.border_width);
  }
...
static void constrain_tooltip_onscreen(MetaDisplay *display, MetaScreen *screen,
                                       MetaCompWindow *cw, Window id) {
   ...
   *
   * If the panel is too tall (around > 25 or so), tooltip positioning fails
   * both tests in gdkwindowimpl.c (maybe_flip_position()) skipping
   * repositioning of the tooltip inside the workarea. This only occurs on
   * bottom panels.
   *
   * Since the calculations are based upon the monitor's workarea and the
   * 'attach' (gdk) window's rectangle, there's no way to compensate for or fool
   * gtk into displaying it correctly.  So here, we do our own check and
   * adjustment. */

  active_workspace_num = meta_core_get_active_workspace(cw->attrs.screen);

  workspace = meta_screen_get_workspace_by_index(screen, active_workspace_num);
  ...
  xinerama = meta_screen_get_xinerama_for_rect(screen, &win_rect);

  meta_workspace_get_work_area_for_xinerama(workspace, xinerama->number,
                                            &work_area);

  new_x = win_rect.x;
  new_y = win_rect.y;

  /* Valid tooltip positions seem to cheat into the panel by a few pixels -
   * maybe accounting for shadow margin.  There's no reason the fix these, but
   * they'd be caught here otherwise, so 10px of overshoot in the direction of
   * the panel is allowed. The tooltips we're out to catch are the ones on the
   * complete other side of the panel (off screren), so there won't be any
   * confusion. */
  if (win_rect.y < work_area.y - 10) {
    new_y = work_area.y;
  } else if (win_rect.y + win_rect.height >
             work_area.y + work_area.height + 10) {
  ...
  if (new_x != win_rect.x || new_y != win_rect.y) {
    if (DISPLAY_COMPOSITOR(display)->debug) {
      fprintf(stderr,
              "Constraining tooltip onscreen   x:%d -> %d, y:%d -> %d\n",
...
static void unmap_win(MetaDisplay *display, MetaScreen *screen, Window id) {
  ...
  if (cw->window && cw->window == info->focus_window) info->focus_window = NULL;
...
static void determine_mode(MetaDisplay *display, MetaScreen *screen,
                           MetaCompWindow *cw) {
  XRenderPictFormat *format;
  Display *xdisplay = meta_display_get_xdisplay(display);
...
static gboolean is_shaped(MetaDisplay *display, Window xwindow) {
  ...
  if (meta_display_has_shape(display)) {
    XShapeQueryExtents(xdisplay, xwindow, &bounding_shaped, &xws, &yws, &wws,
                       &hws, &clip_shaped, &xbs, &ybs, &wbs, &hbs);
...
static void get_window_type(MetaDisplay *display, MetaCompWindow *cw) {
  ...
  meta_prop_get_atom_list(display, cw->id, compositor->atom_net_wm_window_type,
                          &atoms, &n_atoms);
...
static void add_win(MetaScreen *screen, MetaWindow *window, Window xwindow) {
  ...
  if (xwindow == info->output) return;

  /* If already added, ignore */
  if (find_window_for_screen(screen, xwindow) != NULL) return;
  ...
  if (cw->attrs.map_state == IsViewable) map_win(display, screen, xwindow);
}
...
static void resize_win(MetaCompWindow *cw, int x, int y, int width, int height,
                       int border_width, gboolean override_redirect) {
  MetaScreen *screen = cw->screen;
  MetaDisplay *display = meta_screen_get_display(screen);
  Display *xdisplay = meta_display_get_xdisplay(display);
  MetaCompScreen *info = meta_screen_get_compositor_data(screen);
  ...
  if (cw->extents) {
    ...
    damage = None;
    if (debug) fprintf(stderr, "no extents to damage !\n");
  }
  ...
  if (cw->extents) XFixesDestroyRegion(xdisplay, cw->extents);

  cw->extents = win_extents(cw);

  if (damage) {
    if (debug) fprintf(stderr, "Inexplicable intersection with new extents!\n");
...
static void process_property_notify(MetaCompositorXRender *compositor,
  ...
  if (event->atom == compositor->atom_net_wm_window_opacity) {
    ...
    if (!meta_prop_get_cardinal(display, event->window,
                                compositor->atom_net_wm_window_opacity, &value))
      value = OPAQUE;
...
static void process_unmap(MetaCompositorXRender *compositor,
  ...
  if (cw) unmap_win(compositor->display, cw->screen, event->window);
}
...
static void process_map(MetaCompositorXRender *compositor, XMapEvent *event) {
  ...
  if (cw) map_win(compositor->display, cw->screen, event->window);
}

static void process_reparent(MetaCompositorXRender *compositor,
                             XReparentEvent *event, MetaWindow *window) {
  MetaScreen *screen;
...
static void process_create(MetaCompositorXRender *compositor,
                           XCreateWindowEvent *event, MetaWindow *window) {
  MetaScreen *screen;
...
static void process_damage(MetaCompositorXRender *compositor,
  ...
  MetaCompWindow *cw =
      find_window_in_display(compositor->display, event->drawable);
  if (cw == NULL) return;

  repair_win(cw);

#ifdef USE_IDLE_REPAINT
  if (!event->more) add_repair(compositor->display);
#endif
}
...
static void process_shape(MetaCompositorXRender *compositor,
  ...
  if (event->kind == ShapeBounding) {
    if (!event->shaped && cw->shaped) cw->shaped = FALSE;

    resize_win(cw, cw->attrs.x, cw->attrs.y, event->width + event->x,
               event->height + event->y, cw->attrs.border_width,
               cw->attrs.override_redirect);

    if (event->shaped && !cw->shaped) cw->shaped = TRUE;
...
#ifdef HAVE_PRESENT
static void xrender_present_complete(MetaScreen *screen,
...
static void process_generic(MetaCompositorXRender *compositor,
                            XGenericEvent *event) {
  ...
  switch (ge->evtype) {
    ...
    case PresentCompleteNotify: {
      if (ge->extension == compositor->present_major) {
        ...
        MetaScreen *screen =
            find_screen_from_output(compositor->display, ce->window);
        if (screen) xrender_present_complete(screen, ce);
      }
    } break;
#endif /* HAVE_PRESENT */
  }
...
static void xrender_add_window(MetaCompositor *compositor, MetaWindow *window,
...
static void xrender_manage_screen(MetaCompositor *compositor,
  ...
  if (meta_screen_get_compositor_data(screen)) return;
  ...
  pa.subwindow_mode = IncludeInferiors;
  info->root_picture = XRenderCreatePicture(
      xdisplay, info->output, visual_format, CPSubwindowMode, &pa);
  ...
  if (xrc->has_present) {
    info->present_eid =
        XPresentSelectInput(xdisplay, info->output, PresentCompleteNotifyMask);
...
static void xrender_unmanage_screen(MetaCompositor *compositor,
  ...
  if (info->root_picture) XRenderFreePicture(xdisplay, info->root_picture);

  if (info->black_picture) XRenderFreePicture(xdisplay, info->black_picture);
...
static void xrender_set_updates(MetaCompositor *compositor, MetaWindow *window,
                                gboolean updates) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
...
static void xrender_free_window(MetaCompositor *compositor,
                                MetaWindow *window) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
  ...
  if (xwindow != None) destroy_win(xrc->display, xwindow, FALSE);
#endif
}

static void xrender_process_event(MetaCompositor *compositor, XEvent *event,
                                  MetaWindow *window) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
  MetaCompositorXRender *xrc = (MetaCompositorXRender *)compositor;
  /*
  ...
  switch (event->type) {
    ...
    default:
      if (event->type ==
          meta_display_get_damage_event_base(xrc->display) + XDamageNotify)
        process_damage(xrc, (XDamageNotifyEvent *)event);
#ifdef HAVE_SHAPE
      else if (event->type ==
               meta_display_get_shape_event_base(xrc->display) + ShapeNotify)
...
static cairo_surface_t *xrender_get_window_surface(MetaCompositor *compositor,
                                                   MetaWindow *window) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
  MetaFrame *frame;
  Window xwindow;
...
static void xrender_set_active_window(MetaCompositor *compositor,
  ...
  if (old_focus_win) {
    ...
    old_focus = find_window_for_screen(
        screen,
        f ? meta_frame_get_xwindow(f) : meta_window_get_xwindow(old_focus_win));
  }

  if (window) {
    MetaFrame *f = meta_window_get_frame(window);
    new_focus =
        find_window_for_screen(screen, f ? meta_frame_get_xwindow(f)
...
static void xrender_maximize_window(MetaCompositor *compositor,
                                    MetaWindow *window) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
  MetaFrame *frame = meta_window_get_frame(window);
  Window xid =
      frame ? meta_frame_get_xwindow(frame) : meta_window_get_xwindow(window);
  MetaCompWindow *cw =
      find_window_in_display(meta_window_get_display(window), xid);
...
static void xrender_unmaximize_window(MetaCompositor *compositor,
                                      MetaWindow *window) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
  MetaFrame *frame = meta_window_get_frame(window);
  Window xid =
...
static MetaCompositor comp_info = {
    xrender_destroy,           xrender_manage_screen,
    xrender_unmanage_screen,   xrender_add_window,
    xrender_remove_window,     xrender_set_updates,
    xrender_process_event,     xrender_get_window_surface,
    xrender_set_active_window, xrender_free_window,
    xrender_maximize_window,   xrender_unmaximize_window,
};

MetaCompositor *meta_compositor_xrender_new(MetaDisplay *display) {
#ifdef HAVE_COMPOSITE_EXTENSIONS
  char *atom_names[] = {"_XROOTPMAP_ID",
                        "_XSETROOT_ID",
...
#ifdef HAVE_PRESENT
  xrc->has_present =
      XPresentQueryExtension(xdisplay, &xrc->present_major, NULL, NULL);
#endif /* HAVE_PRESENT */
