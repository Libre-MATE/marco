...
static void do_appwindow(GSimpleAction *action, GVariant *parameter,
                         gpointer user_data);

static gboolean aspect_on;

static void set_gdk_window_struts(GdkWindow *window, int left, int right,
                                  int top, int bottom) {
  long vals[12];
  ...
  XChangeProperty(
      ...
      XInternAtom(GDK_WINDOW_XDISPLAY(window), "_NET_WM_STRUT_PARTIAL", False),
      XA_CARDINAL, 32, PropModeReplace, (guchar *)vals, 12);
}
...
static void on_realize_set_struts(GtkWindow *window, gpointer data) {
  ...
  left =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(window), "meta-strut-left"));
  right =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(window), "meta-strut-right"));
  top = GPOINTER_TO_INT(g_object_get_data(G_OBJECT(window), "meta-strut-top"));
  bottom =
      GPOINTER_TO_INT(g_object_get_data(G_OBJECT(window), "meta-strut-bottom"));

  set_gdk_window_struts(gtk_widget_get_window(widget), left, right, top,
                        bottom);
}

static void set_gtk_window_struts(GtkWidget *window, int left, int right,
                                  int top, int bottom) {
  GtkWidget *widget;

  widget = GTK_WIDGET(window);

  g_object_set_data(G_OBJECT(window), "meta-strut-left", GINT_TO_POINTER(left));
  g_object_set_data(G_OBJECT(window), "meta-strut-right",
                    GINT_TO_POINTER(right));
  g_object_set_data(G_OBJECT(window), "meta-strut-top", GINT_TO_POINTER(top));
...
static void set_gdk_window_type(GdkWindow *window, const char *type) {
  ...
  atoms[0] = XInternAtom(GDK_WINDOW_XDISPLAY(window), type, False);
...
static void on_realize_set_type(GtkWindow *window, gpointer data) {
  ...
  set_gdk_window_type(gtk_widget_get_window(GTK_WIDGET(window)), type);
}
...
static void set_gtk_window_type(GtkWindow *window, const char *type) {
  ...
  if (gtk_widget_get_realized(widget))
    set_gdk_window_type(gtk_widget_get_window(widget), type);
}
...
int main(int argc, char **argv) {
  ...
  err = NULL;
  pixbuf =
      gdk_pixbuf_new_from_file(MARCO_ICON_DIR "/marco-window-demo.png", &err);
  if (pixbuf) {
...
static void make_dialog(GtkWidget *parent, int depth) {
  ...
  gtk_dialog_add_button(GTK_DIALOG(dialog), "Open child dialog",
                        GTK_RESPONSE_ACCEPT);
...
static void response_cb(GtkDialog *dialog, int response_id, void *data) {
  switch (response_id) {
    case GTK_RESPONSE_ACCEPT:
      make_dialog(
          GTK_WIDGET(dialog),
          GPOINTER_TO_INT(g_object_get_data(G_OBJECT(dialog), "depth")) + 1);
      break;
...
static void dialog_cb(GSimpleAction *action, GVariant *parameter,
                      gpointer callback_data) {
  make_dialog(GTK_WIDGET(callback_data), 1);
}

static void modal_dialog_cb(GSimpleAction *action, GVariant *parameter,
                            gpointer callback_data) {
  GtkWidget *dialog;

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(callback_data), GTK_DIALOG_DESTROY_WITH_PARENT,
      GTK_MESSAGE_INFO, GTK_BUTTONS_CLOSE, "Here is a MODAL dialog");
...
static void no_parent_dialog_cb(GSimpleAction *action, GVariant *parameter,
                                gpointer callback_data) {
  make_dialog(NULL, 1);
}

static void utility_cb(GSimpleAction *action, GVariant *parameter,
                       gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *button;
...
static void toolbar_cb(GSimpleAction *action, GVariant *parameter,
                       gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *label;
...
static void menu_cb(GSimpleAction *action, GVariant *parameter,
                    gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *label;
...
static void override_redirect_cb(GSimpleAction *action, GVariant *parameter,
                                 gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *label;
...
static void border_only_cb(GSimpleAction *action, GVariant *parameter,
                           gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *vbox;
  GtkWidget *label;
...
static gboolean focus_in_event_cb(GtkWidget *window, GdkEvent *event,
                                  gpointer data) {
  GtkWidget *widget;
...
static gboolean focus_out_event_cb(GtkWidget *window, GdkEvent *event,
                                   gpointer data) {
  GtkWidget *widget;
...
static void splashscreen_cb(GSimpleAction *action, GVariant *parameter,
                            gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *image;
  GtkWidget *vbox;
...
static void dock_cb(GSimpleAction *action, GVariant *parameter,
                    gpointer callback_data) {
...
static void override_background_color(GtkWidget *widget, GdkRGBA *rgba) {
  ...
  css =
      g_strdup_printf("* { background-color: %s; }", gdk_rgba_to_string(rgba));
...
static void desktop_cb(GSimpleAction *action, GVariant *parameter,
                       gpointer callback_data) {
  GtkWidget *window;
  GtkWidget *label;
...
static void sleep_cb(GSimpleAction *action, GVariant *parameter,
                     gpointer data) {
  sleep(1000);
}

static void toggle_aspect_ratio(GSimpleAction *action, GVariant *parameter,
                                gpointer data) {
  GtkWidget *window;
  GdkGeometry geom;
  ...
  if (window)
    gtk_window_set_geometry_hints(GTK_WINDOW(window), GTK_WIDGET(data), &geom,
                                  GDK_HINT_ASPECT);
}

static void toggle_decorated_cb(GSimpleAction *action, GVariant *parameter,
                                gpointer data) {
  GtkWidget *window;
...
static void clicked_toolbar_cb(GSimpleAction *action, GVariant *parameter,
                               gpointer data) {
  GtkWidget *dialog;

  dialog = gtk_message_dialog_new(
      GTK_WINDOW(data), GTK_DIALOG_DESTROY_WITH_PARENT, GTK_MESSAGE_INFO,
...
static void update_statusbar(GtkTextBuffer *buffer, GtkStatusbar *statusbar) {
  ...
  gtk_statusbar_pop(statusbar,
                    0); /* clear any previous message, underflow is allowed */
...
static GtkWidget *create_toolbar(void) {
  ...
  item = gtk_tool_button_new(gtk_image_new_from_icon_name(
                                 "document-open", GTK_ICON_SIZE_SMALL_TOOLBAR),
                             NULL);
  gtk_tool_item_set_tooltip_markup(
      item, "This is a demo button that locks up the demo");
  ...
  item = gtk_tool_button_new(gtk_image_new_from_icon_name(
                             ...
                             NULL);
  gtk_tool_item_set_tooltip_markup(
      item, "This is a demo button that toggles window decorations");
  ...
  item = gtk_tool_button_new(
      gtk_image_new_from_icon_name("gtk-quit", GTK_ICON_SIZE_SMALL_TOOLBAR),
      NULL);
  gtk_tool_item_set_tooltip_markup(item,
...
static void do_appwindow(GSimpleAction *action, GVariant *parameter,
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(action_group), demo_entries,
                                  G_N_ELEMENTS(demo_entries), window);
  ...
  g_signal_connect_object(buffer, "mark_set", /* cursor moved */
