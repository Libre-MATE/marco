...
static GQuark theme_parse_error_quark(void) {
...
static void add_context_to_error(GError **err, GMarkupParseContext *context) {
  ...
  str =
      g_strdup_printf(_("Line %d character %d: %s"), line, ch, (*err)->message);
  g_free((*err)->message);
  (*err)->message = str;
}
...
static void parse_info_free(ParseInfo *info) {
  ...
  if (info->theme) meta_theme_free(info->theme);

  if (info->layout) meta_frame_layout_unref(info->layout);

  if (info->op_list) meta_draw_op_list_unref(info->op_list);

  if (info->op) meta_draw_op_free(info->op);

  if (info->style) meta_frame_style_unref(info->style);

  if (info->style_set) meta_frame_style_set_unref(info->style_set);
}
...
static void push_required_version(ParseInfo *info, int version) {
  info->required_versions =
      g_slist_prepend(info->required_versions, GINT_TO_POINTER(version));
}

static void pop_required_version(ParseInfo *info) {
  g_return_if_fail(info->required_versions != NULL);

  info->required_versions =
      g_slist_delete_link(info->required_versions, info->required_versions);
}

static int peek_required_version(ParseInfo *info) {
...
#define MAX_ATTRS 24
  ...
  if (attrs[0].required) attrs[0].name++; /* skip past it */
  *first_attribute_retloc = NULL;
  ...
  while (name != NULL) {
    ...
    if (attrs[n_attrs].required) attrs[n_attrs].name++; /* skip past it */
...
static gboolean check_no_attributes(GMarkupParseContext *context,
  ...
  /* Can be present anywhere */
  if (attribute_names[0] && strcmp(attribute_names[i], "version") == 0) i++;
...
static gboolean parse_positive_integer(const char *str, int *val,
                                       GMarkupParseContext *context,
                                       MetaTheme *theme, GError **error) {
  char *end;
  long l;
  int j;
  ...
  *val = 0;
  ...
  if (META_THEME_ALLOWS(theme, META_THEME_UBIQUITOUS_CONSTANTS) &&
    ...
    if (end == NULL || end == str) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Could not parse \"%s\" as an integer"), str);
      return FALSE;
    }
  ...
  if (l > MAX_REASONABLE) {
    set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
              _("Integer %ld is too large, current max is %d"), l,
              MAX_REASONABLE);
    return FALSE;
  }
...
static gboolean parse_double(const char *str, double *val,
  ...
  *val = g_ascii_strtod(str, &end);
  ...
  if (end == NULL || end == str) {
    set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
              _("Could not parse \"%s\" as a floating point number"), str);
    return FALSE;
  }
...
static gboolean parse_boolean(const char *str, gboolean *val,
                              GMarkupParseContext *context, GError **error) {
...
static gboolean parse_rounding(const char *str, guint *val,
                               GMarkupParseContext *context, MetaTheme *theme,
                               GError **error) {
  if (strcmp("true", str) == 0)
...
static gboolean parse_angle(const char *str, double *val,
                            GMarkupParseContext *context, GError **error) {
  if (!parse_double(str, val, context, error)) return FALSE;
...
static gboolean parse_alpha(const char *str, MetaAlphaGradientSpec **spec_ret,
                            GMarkupParseContext *context, GError **error) {
  char **split;
  int i;
  int n_alphas;
  ...
  *spec_ret = NULL;
  ...
  while (i < n_alphas) {
    ...
    if (v < (0.0 - 1e-6) || v > (1.0 + 1e-6)) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Alpha must be between 0.0 (invisible) and 1.0 (fully "
                  "opaque), was %g\n"),
                v);
...
static MetaColorSpec *parse_color(MetaTheme *theme, const char *str,
                                  GError **err) {
  char *referent;

  if (META_THEME_ALLOWS(theme, META_THEME_COLOR_CONSTANTS) &&
      meta_theme_lookup_color_constant(theme, str, &referent)) {
    if (referent) return meta_color_spec_new_from_string(referent, err);
...
static gboolean parse_title_scale(const char *str, double *val,
                                  GMarkupParseContext *context,
                                  GError **error) {
  double factor;
...
static void parse_toplevel_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("info")) {
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!name", &name, "!value",
                           &value, NULL))
      return;
    ...
    if (value[0] == '.' || value[0] == '+' || value[0] == '-' ||
        (value[0] >= '0' && value[0] <= '9')) {
      if (strchr(value, '.')) {
        if (!parse_double(value, &dval, context, error)) return;

        if (!meta_theme_define_float_constant(info->theme, name, dval, error)) {
        ...
        if (!meta_theme_define_int_constant(info->theme, name, ival, error)) {
          add_context_to_error(error, context);
          return;
        }
      }
    } else {
      if (!meta_theme_define_color_constant(info->theme, name, value, error)) {
        add_context_to_error(error, context);
        return;
      }
    }
    ...
    if (!locate_attributes(
            context, element_name, attribute_names, attribute_values, error,
            "!name", &name, "parent", &parent, "has_title", &has_title,
            "title_scale", &title_scale, "rounded_top_left", &rounded_top_left,
            "rounded_top_right", &rounded_top_right, "rounded_bottom_left",
            &rounded_bottom_left, "rounded_bottom_right", &rounded_bottom_right,
            "hide_buttons", &hide_buttons, NULL))
      return;
    ...
    if (hide_buttons &&
        !parse_boolean(hide_buttons, &hide_buttons_val, context, error))
      return;
    ...
    if (rounded_top_left &&
        !parse_rounding(rounded_top_left, &rounded_top_left_val, context,
                        info->theme, error))
      return;
    if (rounded_top_right &&
        !parse_rounding(rounded_top_right, &rounded_top_right_val, context,
                        info->theme, error))
      return;
    if (rounded_bottom_left &&
        !parse_rounding(rounded_bottom_left, &rounded_bottom_left_val, context,
                        info->theme, error))
      return;
    if (rounded_bottom_right &&
        !parse_rounding(rounded_bottom_right, &rounded_bottom_right_val,
                        context, info->theme, error))
      return;

    title_scale_val = 1.0;
    if (title_scale &&
        !parse_title_scale(title_scale, &title_scale_val, context, error))
      return;

    if (meta_theme_lookup_layout(info->theme, name)) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("<%s> name \"%s\" used a second time"), element_name, name);
      return;
    }
    ...
    if (parent) {
      ...
      if (parent_layout == NULL) {
        set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                  _("<%s> parent \"%s\" has not been defined"), element_name,
                  parent);
        return;
      }
    }
    ...
    if (META_THEME_ALLOWS(info->theme, META_THEME_HIDDEN_BUTTONS) &&
        hide_buttons_val)
      info->layout->hide_buttons = (hide_buttons_val != FALSE);

    if (title_scale) info->layout->title_scale = title_scale_val;
    ...
    if (rounded_bottom_right)
      info->layout->bottom_right_corner_rounded_radius =
          rounded_bottom_right_val;
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!name", &name, "parent",
                           &parent, "geometry", &geometry, "background",
                           &background, "alpha", &alpha, NULL))
      return;
    ...
    if (geometry) {
      ...
      if (layout == NULL) {
        set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                  _("<%s> geometry \"%s\" has not been defined"), element_name,
                  geometry);
        return;
      }
    ...
    if (layout == NULL) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("<%s> must specify either a geometry or a parent that has a "
                  "geometry"),
                element_name);
      return;
    }
    ...
    if (background != NULL &&
        META_THEME_ALLOWS(info->theme, META_THEME_FRAME_BACKGROUNDS)) {
      info->style->window_background_color =
          meta_color_spec_new_from_string(background, error);
      ...
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("You must specify a background for an alpha value to be "
                  "meaningful"));
      return;
    }
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!type", &type_name,
                           "!style_set", &style_set_name, NULL))
      return;

    type = meta_frame_type_from_string(type_name);

    if (type == META_FRAME_TYPE_LAST || (type == META_FRAME_TYPE_ATTACHED &&
                                         peek_required_version(info) < 3002)) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Unknown type \"%s\" on <%s> element"), type_name,
                element_name);
      return;
    }

    style_set = meta_theme_lookup_style_set(info->theme, style_set_name);

    if (style_set == NULL) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Unknown style_set \"%s\" on <%s> element"), style_set_name,
                element_name);
      return;
    }
    ...
    set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
              _("Element <%s> is not allowed below <%s>"), element_name,
              "metacity_theme");
  }
}
...
static void parse_distance(GMarkupParseContext *context,
  ...
  val = 0;
  if (!parse_positive_integer(value, &val, context, info->theme, error)) return;
  ...
  if (strcmp(name, "left_width") == 0)
    ...
    if (!(info->layout->button_sizing == META_BUTTON_SIZING_LAST ||
      ...
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Cannot specify both \"button_width\"/\"button_height\" and "
...
static void parse_aspect_ratio(GMarkupParseContext *context,
  ...
  val = 0;
  if (!parse_double(value, &val, context, error)) return;
...
static void parse_border(GMarkupParseContext *context,
  ...
  if (!locate_attributes(context, element_name, attribute_names,
                         attribute_values, error, "!name", &name, "!top", &top,
...
static void parse_geometry_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("distance")) {
    parse_distance(context, element_name, attribute_names, attribute_values,
                   info, error);
    push_state(info, STATE_DISTANCE);
  } else if (ELEMENT_IS("border")) {
    parse_border(context, element_name, attribute_names, attribute_values, info,
                 error);
    push_state(info, STATE_BORDER);
  } else if (ELEMENT_IS("aspect_ratio")) {
    parse_aspect_ratio(context, element_name, attribute_names, attribute_values,
                       info, error);
...
static void parse_draw_op_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("line")) {
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!color", &color, "!x1",
                           &x1, "!y1", &y1, "!x2", &x2, "!y2", &y2,
                           "dash_on_length", &dash_on_length, "dash_off_length",
                           &dash_off_length, "width", &width, NULL))
      return;
    ...
    dash_on_val = 0;
    if (dash_on_length && !parse_positive_integer(dash_on_length, &dash_on_val,
                                                  context, info->theme, error))
      return;

    dash_off_val = 0;
    if (dash_off_length &&
        !parse_positive_integer(dash_off_length, &dash_off_val, context,
                                info->theme, error))
      return;
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!color", &color, "!x", &x,
    ...
    filled_val = FALSE;
    if (filled && !parse_boolean(filled, &filled_val, context, error)) return;
    ...
    op->data.rectangle.height = meta_draw_spec_new(info->theme, height, NULL);
    ...
    if (!locate_attributes(
            context, element_name, attribute_names, attribute_values, error,
            "!color", &color, "!x", &x, "!y", &y, "!width", &width, "!height",
            &height, "filled", &filled, "start_angle", &start_angle,
            "extent_angle", &extent_angle, "from", &from, "to", &to, NULL))
      return;
    ...
    if (start_angle == NULL) {
      if (!parse_angle(from, &start_angle_val, context, error)) return;

      start_angle_val = (180 - start_angle_val) / 360.0;
    } else {
      if (!parse_angle(start_angle, &start_angle_val, context, error)) return;
    }

    if (extent_angle == NULL) {
      if (!parse_angle(to, &extent_angle_val, context, error)) return;

      extent_angle_val = ((180 - extent_angle_val) / 360.0) - start_angle_val;
    } else {
      if (!parse_angle(extent_angle, &extent_angle_val, context, error)) return;
    }
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!x", &x, "!y", &y,
...
#endif
    alpha_spec = NULL;
    if (!parse_alpha(alpha, &alpha_spec, context, error)) return;
    ...
    color_spec = parse_color(info->theme, color, error);
    if (color_spec == NULL) {
      if (alpha_spec) meta_alpha_gradient_spec_free(alpha_spec);
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!type", &type, "!x", &x,
                           "!y", &y, "!width", &width, "!height", &height,
                           "alpha", &alpha, NULL))
      return;
    ...
    alpha_spec = NULL;
    if (alpha && !parse_alpha(alpha, &alpha_spec, context, error)) return;
    ...
    info->op->data.gradient.width =
        meta_draw_spec_new(info->theme, width, NULL);
    info->op->data.gradient.height =
        meta_draw_spec_new(info->theme, height, NULL);
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!x", &x, "!y", &y,
                           "!width", &width, "!height", &height, "alpha",
                           &alpha, "!filename", &filename, "colorize",
                           &colorize, "fill_type", &fill_type, NULL))
      return;
    ...
    for (h = 0; h < pixbuf_height; h++) {
      for (w = 1; w < pixbuf_width; w++) {
        ...
        if (c < pixbuf_n_channels) break;
      }
      if (w < pixbuf_width) break;
    }
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!state", &state, "!shadow",
                           &shadow, "!arrow", &arrow, "!x", &x, "!y", &y,
                           "!width", &width, "!height", &height, "filled",
                           &filled, NULL))
      return;
    ...
    if (((int)state_val) == -1) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Did not understand state \"%s\" for <%s> element"), state,
                element_name);
      return;
    }
    ...
    if (((int)shadow_val) == -1) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Did not understand shadow \"%s\" for <%s> element"), shadow,
                element_name);
      return;
    }
    ...
    if (((int)arrow_val) == -1) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Did not understand arrow \"%s\" for <%s> element"), arrow,
                element_name);
      return;
    }
    ...
    op->data.gtk_arrow.height = meta_draw_spec_new(info->theme, height, NULL);
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!state", &state, "!x", &x,
                           "!y1", &y1, "!y2", &y2, NULL))
      return;
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!x", &x, "!y", &y,
                           "!width", &width, "!height", &height, "alpha",
                           &alpha, "fill_type", &fill_type, NULL))
      return;
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!color", &color, "!x", &x,
                           "!y", &y, "ellipsize_width", &ellipsize_width, NULL))
      return;
    ...
    if (ellipsize_width)
      op->data.title.ellipsize_width =
          meta_draw_spec_new(info->theme, ellipsize_width, NULL);
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "x", &x, "y", &y, "width",
                           &width, "height", &height, "!name", &name, NULL))
      return;
    ...
    op_list = meta_theme_lookup_draw_op_list(info->theme, name);
    if (op_list == NULL) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("No <draw_ops> called \"%s\" has been defined"), name);
      return;
    }
    ...
    op->data.op_list.width =
        meta_draw_spec_new(info->theme, width ? width : "width", NULL);
    op->data.op_list.height =
        meta_draw_spec_new(info->theme, height ? height : "height", NULL);
    ...
    if (!locate_attributes(
            context, element_name, attribute_names, attribute_values, error,
            "x", &x, "y", &y, "width", &width, "height", &height, "!name",
            &name, "tile_xoffset", &tile_xoffset, "tile_yoffset", &tile_yoffset,
            "!tile_width", &tile_width, "!tile_height", &tile_height, NULL))
      return;
    ...
    op->data.tile.height =
        meta_draw_spec_new(info->theme, height ? height : "height", NULL);
    op->data.tile.tile_xoffset = meta_draw_spec_new(
        info->theme, tile_xoffset ? tile_xoffset : "0", NULL);
    op->data.tile.tile_yoffset = meta_draw_spec_new(
        info->theme, tile_yoffset ? tile_yoffset : "0", NULL);
    op->data.tile.tile_width =
        meta_draw_spec_new(info->theme, tile_width, NULL);
    op->data.tile.tile_height =
        meta_draw_spec_new(info->theme, tile_height, NULL);
...
static void parse_gradient_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("color")) {
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!value", &value, NULL))
      return;
    ...
    info->op->data.gradient.gradient_spec->color_specs = g_slist_append(
        info->op->data.gradient.gradient_spec->color_specs, color_spec);
...
static void parse_style_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("piece")) {
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!position", &position,
                           "draw_ops", &draw_ops, NULL))
      return;

    info->piece = meta_frame_piece_from_string(position);
    if (info->piece == META_FRAME_PIECE_LAST) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Unknown position \"%s\" for frame piece"), position);
      return;
    }

    if (info->style->pieces[info->piece] != NULL) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Frame style already has a piece at position %s"), position);
      return;
    }
    ...
    if (draw_ops) {
      ...
      op_list = meta_theme_lookup_draw_op_list(info->theme, draw_ops);
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!function", &function,
                           "!state", &state, "draw_ops", &draw_ops, NULL))
      return;

    info->button_type = meta_button_type_from_string(function, info->theme);
    if (info->button_type == META_BUTTON_TYPE_LAST) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Unknown function \"%s\" for button"), function);
      return;
    }
    ...
    if (meta_theme_earliest_version_with_button(info->button_type) >
      ...
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Button function \"%s\" does not exist in this version (%d, "
                  "need %d)"),
...
static void parse_style_set_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("frame")) {
    ...
    if (!locate_attributes(context, element_name, attribute_names,
                           attribute_values, error, "!focus", &focus, "!state",
                           &state, "resize", &resize, "!style", &style, NULL))
      return;

    frame_focus = meta_frame_focus_from_string(focus);
    if (frame_focus == META_FRAME_FOCUS_LAST) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("\"%s\" is not a valid value for focus attribute"), focus);
      return;
    }
    ...
    if (frame_state == META_FRAME_STATE_LAST) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("\"%s\" is not a valid value for state attribute"), focus);
      return;
    }
    ...
    switch (frame_state) {
      ...
      case META_FRAME_STATE_SHADED:
        if (META_THEME_ALLOWS(info->theme,
                              META_THEME_UNRESIZABLE_SHADED_STYLES)) {
          if (resize == NULL)
        ...
        } else /* v1 theme */
        {
          if (resize != NULL) {
            set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                      _("Should not have \"resize\" attribute on <%s> element "
    ...
    switch (frame_state) {
      case META_FRAME_STATE_NORMAL:
        if (info->style_set->normal_styles[frame_resize][frame_focus]) {
          set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                    _("Style has already been specified for state %s resize %s "
                      "focus %s"),
                    state, resize, focus);
          return;
        }
      ...
      case META_FRAME_STATE_TILED_LEFT_AND_SHADED:
        ...
        meta_frame_style_ref(frame_style);
        info->style_set->tiled_left_and_shaded_styles[frame_focus] =
            frame_style;
        break;
      case META_FRAME_STATE_TILED_RIGHT_AND_SHADED:
        ...
        meta_frame_style_ref(frame_style);
        info->style_set->tiled_right_and_shaded_styles[frame_focus] =
            frame_style;
        break;
...
static void parse_piece_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("draw_ops")) {
    if (info->op_list) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Can't have a two draw_ops for a <piece> element (theme "
                  "specified a draw_ops attribute and also a <draw_ops> "
...
static void parse_button_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("draw_ops")) {
    if (info->op_list) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Can't have a two draw_ops for a <button> element (theme "
                  "specified a draw_ops attribute and also a <draw_ops> "
                  "element, or specified two elements)"));
      return;
    }
...
static void parse_menu_icon_element(GMarkupParseContext *context,
  ...
  if (ELEMENT_IS("draw_ops")) {
    if (info->op_list) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Can't have a two draw_ops for a <menu_icon> element (theme "
                  "specified a draw_ops attribute and also a <draw_ops> "
                  "element, or specified two elements)"));
      return;
    }
...
static const char *find_version(const char **attribute_names,
                                const char **attribute_values) {
  int i;

  for (i = 0; attribute_names[i]; i++) {
    if (strcmp(attribute_names[i], "version") == 0) return attribute_values[i];
  }
 ...
 */
static gboolean check_version(GMarkupParseContext *context,
                              const char *version_str, gboolean *satisfied,
                              guint *minimum_required, GError **error) {
  ...
  *minimum_required = 0;
  ...
  if (!version_regex)
    version_regex =
        g_regex_new("^\\s*([<>]=?)\\s*(\\d+)(\\.\\d+)?\\s*$", 0, 0, NULL);
  ...
  if (minor_str && minor_str[0]) version += atoi(minor_str + 1);
...
static void start_element_handler(GMarkupParseContext *context,
  ...
  if (version != NULL) {
    ...
    if (required_version < 3000) {
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("\"version\" attribute cannot be used in "
                  "metacity-theme-1.xml or metacity-theme-2.xml"));
      return;
    }
    ...
    if (peek_state(info) == STATE_START) {
      if (satisfied) {
        ...
        set_error(error, context, THEME_PARSE_ERROR, THEME_PARSE_ERROR_TOO_OLD,
                  _("Theme requires version %s but latest supported theme "
                    "version is %d.%d"),
  ...
  switch (peek_state(info)) {
    ...
    case STATE_DESCRIPTION:
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Element <%s> is not allowed inside a "
                  "name/author/date/description element"),
                element_name);
      break;
    case STATE_CONSTANT:
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
    ...
    case STATE_ASPECT_RATIO:
      set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE,
                _("Element <%s> is not allowed inside a "
                  "distance/border/aspect_ratio element"),
                element_name);
      break;
    case STATE_DRAW_OPS:
    ...
    case STATE_BUTTON:
      ...
      break;
    case STATE_SHADOW:
      parse_shadow_element(context, element_name, attribute_names,
                           attribute_values, info, error);
      break;
    case STATE_PADDING:
      parse_padding_element(context, element_name, attribute_names,
                            attribute_values, info, error);
      break;
...
static void end_element_handler(GMarkupParseContext *context,
  ...
  switch (peek_state(info)) {
    ...
    case STATE_FRAME_STYLE:
      ...
      if (!meta_frame_style_validate(info->style, peek_required_version(info),
                                     error)) {
        add_context_to_error(error, context);
      }
...
#define NO_TEXT(element_name)                                     \
  set_error(error, context, G_MARKUP_ERROR, G_MARKUP_ERROR_PARSE, \
            _("No text is allowed inside element <%s>"), element_name)
...
static void text_handler(GMarkupParseContext *context, const gchar *text,
  ...
  if (info->skip_level > 0) return;

  if (all_whitespace(text, text_len)) return;
 ...
 */
static gboolean theme_error_is_fatal(GError *error) {
  return !(error->domain == G_FILE_ERROR ||
           (error->domain == THEME_PARSE_ERROR &&
            error->code == THEME_PARSE_ERROR_TOO_OLD));
}

static MetaTheme *load_theme(const char *theme_dir, const char *theme_name,
                             guint major_version, GError **error) {
  GMarkupParseContext *context;
  ParseInfo info;
  char *text;
  gsize length;
  ...
  if (!g_file_get_contents(theme_file, &text, &length, error)) goto out;
  ...
  if (!g_markup_parse_context_parse(context, text, length, error)) goto out;
...
static gboolean keep_trying(GError **error) {
  if (*error && !theme_error_is_fatal(*error)) {
    g_clear_error(error);
    return TRUE;
  }
...
MetaTheme *meta_theme_load(const char *theme_name, GError **err) {
  ...
  for (major_version = THEME_MAJOR_VERSION; (major_version > 0);
    ...
    theme_dir = g_build_filename(g_get_user_data_dir(), "themes", theme_name,
                                 THEME_SUBDIR, NULL);
    ...
    for (i = 0; xdg_data_dirs[i] != NULL; i++) {
      theme_dir = g_build_filename(xdg_data_dirs[i], "themes", theme_name,
                                   THEME_SUBDIR, NULL);
    ...
    theme_dir = g_build_filename(MARCO_DATADIR, "themes", theme_name,
                                 THEME_SUBDIR, NULL);
    retval = load_theme(theme_dir, theme_name, major_version, &error);
    g_free(theme_dir);
    if (!keep_trying(&error)) goto out;
