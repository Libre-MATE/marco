...
static gboolean meta_tile_preview_draw(GtkWidget *widget, cairo_t *cr,
                                       gpointer user_data) {
  ...
  if (preview->has_alpha) {
    ...
    cairo_rectangle(cr, OUTLINE_WIDTH - 0.5, OUTLINE_WIDTH - 0.5,
                    preview->tile_rect.width - 2 * (OUTLINE_WIDTH - 1) - 1,
                    preview->tile_rect.height - 2 * (OUTLINE_WIDTH - 1) - 1);
    cairo_stroke(cr);
  }

  cairo_rectangle(cr, 0.5, 0.5, preview->tile_rect.width - 1,
  ...
  if (preview->has_alpha) {
    ...
    cairo_set_source_rgba(cr, preview->preview_color->red,
                          ...
                          preview->preview_color->blue, 1.0);
  }
...
static void on_preview_window_style_set(GtkWidget *widget, GtkStyle *previous,
                                        gpointer user_data) {
  MetaTilePreview *preview = user_data;
  ...
  gtk_style_context_get(context, gtk_style_context_get_state(context),
                        "background-color", &preview->preview_color, NULL);
...
void meta_tile_preview_free(MetaTilePreview *preview) {
  ...
  if (preview->preview_color) gdk_rgba_free(preview->preview_color);
...
void meta_tile_preview_show(MetaTilePreview *preview, MetaRectangle *tile_rect,
  ...
  meta_core_lower_beneath_focus_window(
  ...
  gdk_window_move_resize(window, preview->tile_rect.x, preview->tile_rect.y,
