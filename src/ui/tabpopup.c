...
static gboolean outline_window_draw(GtkWidget *widget, cairo_t *cr,
                                    gpointer data) {
  ...
  cairo_rectangle(cr, 0.5, 0.5, te->rect.width - 1, te->rect.height - 1);
  cairo_stroke(cr);

  cairo_rectangle(cr, te->inner_rect.x - 0.5, te->inner_rect.y - 0.5,
                  te->inner_rect.width + 1, te->inner_rect.height + 1);
  cairo_stroke(cr);
...
static TabEntry *tab_entry_new(const MetaTabEntry *entry, gboolean outline,
                               gint scale) {
  TabEntry *te;
  ...
  if (te->icon) {
    ...
    if (entry->hidden) te->dimmed_icon = dimm_icon(entry->icon);
  }
...
MetaTabPopup *meta_ui_tab_popup_new(const MetaTabEntry *entries,
                                    ...
                                    gboolean expand_for_titles, gint border) {
  MetaTabPopup *popup;
  int i, left, top;
  int height;
  GtkWidget *grid;
  ...
  if (meta_prefs_get_compositing_manager()) {
    ...
    gtk_style_context_add_class(
        gtk_widget_get_style_context(GTK_WIDGET(popup->window)),
        GTK_STYLE_CLASS_OSD);
  }
  ...
  for (i = 0; i < entry_count; ++i) {
    TabEntry *new_entry =
        tab_entry_new(&entries[i], border & BORDER_OUTLINE_WINDOW, scale);
  ...
  gtk_window_set_default_size(GTK_WINDOW(popup->window), default_window_width,
                              -1);

  return popup;
}

static void free_tab_entry(gpointer data, gpointer user_data) {
  TabEntry *te;

  te = data;

  g_free(te->title);
  if (te->icon) g_object_unref(G_OBJECT(te->icon));
  if (te->dimmed_icon) g_object_unref(G_OBJECT(te->dimmed_icon));
...
void meta_ui_tab_popup_free(MetaTabPopup *popup) {
  ...
  if (popup->outline_window != NULL) gtk_widget_destroy(popup->outline_window);
...
static void display_entry(MetaTabPopup *popup, TabEntry *te) {
  ...
  if (popup->border & BORDER_OUTLINE_WINDOW) {
    ...
    gtk_window_resize(GTK_WINDOW(popup->outline_window), te->rect.width,
                      te->rect.height);
    ...
    gdk_window_shape_combine_region(
        gtk_widget_get_window(popup->outline_window), region, 0, 0);
...
void meta_ui_tab_popup_forward(MetaTabPopup *popup) {
  if (popup->current != NULL) popup->current = popup->current->next;

  if (popup->current == NULL) popup->current = popup->entries;
...
void meta_ui_tab_popup_backward(MetaTabPopup *popup) {
  if (popup->current != NULL) popup->current = popup->current->prev;

  if (popup->current == NULL) popup->current = g_list_last(popup->entries);
...
static void display_widget(MetaTabPopup *popup, GtkWidget *w) {
  if (w != NULL) {
    GList *c = popup->entries;
    while (c != NULL && ((TabEntry *)c->data)->widget != w) c = c->next;
    if (c != NULL) {
      popup->current = c;
      TabEntry *te = c->data;
      display_entry(popup, te);
...
void meta_ui_tab_popup_down(MetaTabPopup *popup) {
  if (popup->current != NULL) {
    ...
    GtkWidget *w = gtk_grid_get_child_at(GTK_GRID(popup->grid), te->grid_left,
                                         te->grid_top + 1);
...
void meta_ui_tab_popup_mouse_press(MetaTabPopup *popup, gint x, gint y) {
  ...
  while (tmp != NULL && !found) {
    ...
    gint wx, wy;
    if (gtk_widget_translate_coordinates(popup->window, te->widget, x, y, &wx,
                                         &wy)) {
      GtkAllocation alloc;
      gtk_widget_get_allocation(te->widget, &alloc);
      found = (0 <= wx && wx < alloc.width && 0 <= wy && wy < alloc.height);
      if (found) {
        popup->current = tmp;
        display_entry(popup, te);
      }
...
#define META_SELECT_IMAGE(obj) \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), META_TYPE_SELECT_IMAGE, MetaSelectImage))
...
static gboolean meta_select_image_draw(GtkWidget *widget, cairo_t *cr);

static GtkImageClass *parent_class;

GType meta_select_image_get_type(void) {
  ...
  if (!image_type) {
    ...
    image_type = g_type_register_static(GTK_TYPE_IMAGE, "MetaSelectImage",
                                        &image_info, 0);
  }
...
static void meta_select_image_get_preferred_width(GtkWidget *widget,
                                                  gint *minimum_width,
                                                  gint *natural_width) {
...
static void meta_select_image_get_preferred_height(GtkWidget *widget,
                                                   gint *minimum_height,
                                                   gint *natural_height) {
  GTK_WIDGET_CLASS(parent_class)
...
#define META_SELECT_WORKSPACE(obj)                               \
  (G_TYPE_CHECK_INSTANCE_CAST((obj), META_TYPE_SELECT_WORKSPACE, \
                              MetaSelectWorkspace))
...
static GtkWidget *selectable_workspace_new(MetaWorkspace *workspace,
  ...
  if (workspace->screen->rect.width < workspace->screen->rect.height) {
    mini_workspace_ratio = (double)workspace->screen->rect.width /
                           (double)workspace->screen->rect.height;
    mini_workspace_height = (int)((double)current->rect.height *
                                  MINI_WORKSPACE_SCREEN_FRACTION / entry_count);
    mini_workspace_width =
        (int)((double)mini_workspace_height * mini_workspace_ratio);
  } else {
    mini_workspace_ratio = (double)workspace->screen->rect.height /
                           (double)workspace->screen->rect.width;
    mini_workspace_width = (int)((double)current->rect.width *
                                 MINI_WORKSPACE_SCREEN_FRACTION / entry_count);
    mini_workspace_height =
        (int)((double)mini_workspace_width * mini_workspace_ratio);
  }
...
static gboolean meta_select_workspace_draw(GtkWidget *widget, cairo_t *cr);

GType meta_select_workspace_get_type(void) {
  ...
  if (!workspace_type) {
    ...
    workspace_type = g_type_register_static(
        GTK_TYPE_DRAWING_AREA, "MetaSelectWorkspace", &workspace_info, 0);
  }
 ...
 * workspace.
 **/
static WnckWindowDisplayInfo meta_convert_meta_to_wnck(MetaWindow *window,
                                                       MetaScreen *screen) {
...
static gboolean meta_select_workspace_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  while (tmp != NULL) {
    ...
    if (window->skip_pager || !meta_window_showing_on_its_workspace(window) ||
        window->unmaps_pending || ignoreable_sticky) {
      --n_windows;
    } else {
  ...
  wnck_draw_workspace(
      widget, cr, SELECT_OUTLINE_WIDTH, SELECT_OUTLINE_WIDTH,
      allocation.width - SELECT_OUTLINE_WIDTH * 2,
      allocation.height - SELECT_OUTLINE_WIDTH * 2,
      workspace->screen->rect.width, workspace->screen->rect.height, NULL,
      (workspace->screen->active_workspace == workspace), windows, n_windows);
  ...
  if (META_SELECT_WORKSPACE(widget)->selected) {
    ...
    if (meta_prefs_get_compositing_manager()) {
      /* compositing manager creates a dark background: show the selection in a
       * light color */
      meta_gtk_style_get_light_color(context, GTK_STATE_FLAG_SELECTED, &color);
    } else {
      /* non-compositing creates a light background: show the selection in a
       * dark color */
    ...
    cairo_rectangle(cr, SELECT_OUTLINE_WIDTH / 2.0, SELECT_OUTLINE_WIDTH / 2.0,
