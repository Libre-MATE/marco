...
typedef void (*RenderGradientFunc)(cairo_t *cr, int width, int height);

static void draw_checkerboard(cairo_t *cr, int width, int height) {
...
static void render_simple(cairo_t *cr, int width, int height,
                          MetaGradientType type, gboolean with_alpha) {
  GdkPixbuf *pixbuf;
  GdkRGBA from, to;
  ...
  pixbuf = meta_gradient_create_simple(width, height, &from, &to, type);
  ...
  if (with_alpha) {
    ...
    meta_gradient_add_alpha(pixbuf, alphas, G_N_ELEMENTS(alphas),
...
static void render_vertical_func(cairo_t *cr, int width, int height) {
  render_simple(cr, width, height, META_GRADIENT_VERTICAL, FALSE);
}

static void render_horizontal_func(cairo_t *cr, int width, int height) {
  render_simple(cr, width, height, META_GRADIENT_HORIZONTAL, FALSE);
}

static void render_diagonal_func(cairo_t *cr, int width, int height) {
  render_simple(cr, width, height, META_GRADIENT_DIAGONAL, FALSE);
}

static void render_diagonal_alpha_func(cairo_t *cr, int width, int height) {
  render_simple(cr, width, height, META_GRADIENT_DIAGONAL, TRUE);
}

static void render_multi(cairo_t *cr, int width, int height,
...
#define N_COLORS 5
  ...
  pixbuf = meta_gradient_create_multi(width, height, colors, N_COLORS, type);
...
static void render_vertical_multi_func(cairo_t *cr, int width, int height) {
  render_multi(cr, width, height, META_GRADIENT_VERTICAL);
}

static void render_horizontal_multi_func(cairo_t *cr, int width, int height) {
  render_multi(cr, width, height, META_GRADIENT_HORIZONTAL);
}

static void render_diagonal_multi_func(cairo_t *cr, int width, int height) {
  render_multi(cr, width, height, META_GRADIENT_DIAGONAL);
}

static void render_interwoven_func(cairo_t *cr, int width, int height) {
  GdkPixbuf *pixbuf;
#define N_COLORS 4
  ...
  pixbuf = meta_gradient_create_interwoven(width, height, colors, height / 10,
                                           colors + 2, height / 14);
...
static gboolean draw_callback(GtkWidget *widget, cairo_t *cr, gpointer data) {
  ...
  (*func)(cr, allocation.width, allocation.height);
