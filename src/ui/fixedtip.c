...
static gboolean draw_handler(GtkWidget *widget, cairo_t *cr) {
...
void meta_fixed_tip_show(int root_x, int root_y, const char *markup_text) {
  ...
  if (tip == NULL) {
    ...
    if (visual != NULL) gtk_widget_set_visual(tip, visual);
    ...
    g_signal_connect(tip, "destroy", G_CALLBACK(gtk_widget_destroyed), &tip);
  }
