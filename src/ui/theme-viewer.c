...
#else
#define BUTTON_LAYOUT_COMBINATIONS \
  ((MAX_BUTTONS_PER_CORNER + 1) * (MAX_BUTTONS_PER_CORNER + 1))
#endif

enum { FONT_SIZE_SMALL, FONT_SIZE_NORMAL, FONT_SIZE_LARGE, FONT_SIZE_LAST };

static MetaTheme *global_theme = NULL;
static GtkWidget *previews[META_FRAME_TYPE_LAST * FONT_SIZE_LAST +
                           BUTTON_LAYOUT_COMBINATIONS] = {
    NULL,
};
...
static void override_font(GtkWidget *widget, const gchar *font) {
  gchar *css;
  GtkCssProvider *provider;
...
static GtkWidget *preview_collection(int font_size,
  ...
  g_action_map_add_action_entries(G_ACTION_MAP(action_group),
                                  ...
                                  G_N_ELEMENTS(theme_viewer_entries), NULL);
  gtk_widget_insert_action_group(sw, "theme-viewer",
  ...
  while (i < META_FRAME_TYPE_LAST) {
    ...
    meta_preview_set_frame_flags(META_PREVIEW(preview), get_window_flags(i));
  ...
  while (left < MAX_BUTTONS_PER_CORNER) {
    ...
    while (right < MAX_BUTTONS_PER_CORNER) {
      ...
      static MetaButtonFunction left_functions[MAX_BUTTONS_PER_CORNER] = {
          META_BUTTON_FUNCTION_MENU, META_BUTTON_FUNCTION_MINIMIZE,
          META_BUTTON_FUNCTION_MAXIMIZE, META_BUTTON_FUNCTION_CLOSE};
      static MetaButtonFunction right_functions[MAX_BUTTONS_PER_CORNER] = {
          META_BUTTON_FUNCTION_MINIMIZE, META_BUTTON_FUNCTION_MAXIMIZE,
          META_BUTTON_FUNCTION_CLOSE, META_BUTTON_FUNCTION_MENU};
...
int main(int argc, char **argv) {
  ...
  if (strcmp(global_theme->name, global_theme->readable_name) == 0)
    gtk_window_set_title(GTK_WINDOW(window), global_theme->readable_name);
  else {
    ...
    gchar *title = g_strconcat(global_theme->name, " - ",
                               global_theme->readable_name, NULL);
  ...
  gtk_style_context_get(style, GTK_STATE_FLAG_NORMAL, GTK_STYLE_PROPERTY_FONT,
                        &font_desc, NULL);
  ...
  collection = preview_collection(FONT_SIZE_NORMAL, font_desc);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), collection,
                           gtk_label_new(_("Normal Title Font")));

  collection = preview_collection(FONT_SIZE_SMALL, font_desc);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), collection,
                           gtk_label_new(_("Small Title Font")));

  collection = preview_collection(FONT_SIZE_LARGE, font_desc);
  gtk_notebook_append_page(GTK_NOTEBOOK(notebook), collection,
...
static MetaFrameFlags get_flags(GtkWidget *widget) {
  return META_FRAME_ALLOWS_DELETE | META_FRAME_ALLOWS_MENU |
         META_FRAME_ALLOWS_MINIMIZE | META_FRAME_ALLOWS_MAXIMIZE |
         META_FRAME_ALLOWS_VERTICAL_RESIZE |
         META_FRAME_ALLOWS_HORIZONTAL_RESIZE | META_FRAME_HAS_FOCUS |
         META_FRAME_ALLOWS_SHADE | META_FRAME_ALLOWS_MOVE;
...
static int get_text_height(GtkWidget *widget) {
  ...
  gtk_style_context_get(style, GTK_STATE_FLAG_NORMAL, GTK_STYLE_PROPERTY_FONT,
                        &font_desc, NULL);
  text_height = meta_pango_font_desc_get_text_height(
...
#define ITERATIONS 100
  ...
  meta_theme_get_frame_borders(global_theme, META_FRAME_TYPE_NORMAL,
                               get_text_height(widget), get_flags(widget),
                               &borders);
  ...
  while (i < ITERATIONS) {
     ...
     */
    pixmap = gdk_window_create_similar_surface(
    ...
    meta_theme_draw_frame(
        global_theme, gtk_widget_get_style_context(widget), cr,
        META_FRAME_TYPE_NORMAL, get_flags(widget), client_width, client_height,
        layout, get_text_height(widget), &button_layout, button_states,
        meta_preview_get_mini_icon(), meta_preview_get_icon());
  ...
  milliseconds_to_draw_frame =
      (g_timer_elapsed(timer, NULL) / (double)ITERATIONS) * 1000;

  g_print(
      _("Drew %d frames in %g client-side seconds (%g milliseconds per frame) "
        "and %g seconds wall clock time including X server resources (%g "
        "milliseconds per frame)\n"),
      ITERATIONS, ((double)end - (double)start) / CLOCKS_PER_SEC,
      (((double)end - (double)start) / CLOCKS_PER_SEC / (double)ITERATIONS) *
          1000,
      g_timer_elapsed(timer, NULL), milliseconds_to_draw_frame);
