...
static MenuItem menuitems[] = {
    ...
    {META_MENU_OP_MINIMIZE, MENU_ITEM_IMAGE, MARCO_STOCK_MINIMIZE, FALSE,
     N_("Mi_nimize")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_MAXIMIZE, MENU_ITEM_IMAGE, MARCO_STOCK_MAXIMIZE, FALSE,
     N_("Ma_ximize")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_UNMAXIMIZE, MENU_ITEM_IMAGE, MARCO_STOCK_RESTORE, FALSE,
     N_("Unma_ximize")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    ...
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_RECOVER, MENU_ITEM_NORMAL, NULL, FALSE,
     N_("Move Titlebar On_screen")},
    {META_MENU_OP_WORKSPACES, MENU_ITEM_SEPARATOR, NULL, FALSE,
     NULL}, /* separator */
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_ABOVE, MENU_ITEM_CHECKBOX, NULL, FALSE, N_("Always on _Top")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_UNABOVE, MENU_ITEM_CHECKBOX, NULL, TRUE,
     N_("Always on _Top")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_STICK, MENU_ITEM_RADIOBUTTON, NULL, FALSE,
     N_("_Always on Visible Workspace")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_UNSTICK, MENU_ITEM_RADIOBUTTON, NULL, FALSE,
     N_("_Only on This Workspace")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_MOVE_LEFT, MENU_ITEM_NORMAL, NULL, FALSE,
     N_("Move to Workspace _Left")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_MOVE_RIGHT, MENU_ITEM_NORMAL, NULL, FALSE,
     N_("Move to Workspace R_ight")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_MOVE_UP, MENU_ITEM_NORMAL, NULL, FALSE,
     N_("Move to Workspace _Up")},
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_MOVE_DOWN, MENU_ITEM_NORMAL, NULL, FALSE,
     N_("Move to Workspace _Down")},
    {0, MENU_ITEM_WORKSPACE_LIST, NULL, FALSE, NULL},
    {0, MENU_ITEM_SEPARATOR, NULL, FALSE, NULL}, /* separator */
    /* Translators: Translate this string the same way as you do in libwnck! */
    {META_MENU_OP_DELETE, MENU_ITEM_IMAGE, MARCO_STOCK_DELETE, FALSE,
     N_("_Close")}};
...
static void menu_closed(GtkMenu* widget, gpointer data) {
  ...
  (*menu->func)(menu, GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
                menu->client_xwindow, gtk_get_current_event_time(), 0, 0,
                menu->data);
...
static void activate_cb(GtkWidget* menuitem, gpointer data) {
  ...
  (*md->menu->func)(
      md->menu, GDK_DISPLAY_XDISPLAY(gdk_display_get_default()),
      md->menu->client_xwindow, gtk_get_current_event_time(), md->op,
 ...
 */
static char* get_workspace_name_with_accel(Display* display, Window xroot,
                                           int index) {
  const char* name;
  int number;
  int charcount = 0;
  ...
  if (sscanf(name, _("Workspace %d%n"), &number, &charcount) != 0 &&
    ...
    if (number == 10)
      ...
      new_name =
          g_strdup_printf(_("Workspace %s%d"), number < 10 ? "_" : "", number);
    return new_name;
  } else {
    /*
...
static GtkWidget* menu_item_new(MenuItem* menuitem, int workspace_id) {
  ...
  if (menuitem->type == MENU_ITEM_NORMAL) {
    ...
    GtkWidget* image =
        gtk_image_new_from_icon_name(menuitem->stock_id, GTK_ICON_SIZE_MENU);
    ...
    if (ops & menuitem.op || menuitem.op == 0) {
      ...
      if (menuitem.type == MENU_ITEM_WORKSPACE_LIST) {
        if (ops & META_MENU_OP_WORKSPACES) {
          ...
          MenuItem to_another_workspace = {0, MENU_ITEM_NORMAL, NULL, FALSE,
          ...
          for (j = 0; j < n_workspaces; j++) {
            ...
            meta_core_get_menu_accelerator(META_MENU_OP_WORKSPACES, j + 1, &key,
                                           &mods);
            ...
            if ((active_workspace == (unsigned)j) &&
            ...
            g_object_set_data(G_OBJECT(submi), "workspace", GINT_TO_POINTER(j));

            g_signal_connect_data(G_OBJECT(submi), "activate",
                                  G_CALLBACK(activate_cb), md,
        ...
        meta_core_get_menu_accelerator(menuitems[i].op, -1, &key, &mods);

        if (insensitive & menuitem.op) gtk_widget_set_sensitive(mi, FALSE);
        ...
        g_signal_connect_data(G_OBJECT(mi), "activate", G_CALLBACK(activate_cb),
...
void meta_window_menu_popup(MetaWindowMenu* menu, int root_x, int root_y,
  ...
  gtk_menu_popup(GTK_MENU(menu->menu), NULL, NULL, popup_position_func, pt,
                 button, timestamp);
