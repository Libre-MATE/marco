...
static void meta_accel_label_get_preferred_height(GtkWidget *widget,
                                                  gint *minimum, gint *natural);
static gboolean meta_accel_label_draw(GtkWidget *widget, cairo_t *cr);

G_DEFINE_TYPE(MetaAccelLabel, meta_accel_label, GTK_TYPE_LABEL);

static void meta_accel_label_class_init(MetaAccelLabelClass *class) {
...
static void meta_accel_label_get_preferred_width(GtkWidget *widget,
  ...
  GTK_WIDGET_CLASS(meta_accel_label_parent_class)
...
static gboolean meta_accel_label_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  if (allocation.width >= requisition.width + ac_width) {
    ...
    if (direction == GTK_TEXT_DIR_RTL) cairo_translate(cr, ac_width, 0);
    ...
    accel_layout =
        gtk_widget_create_pango_layout(widget, accel_label->accel_string);
    ...
    gtk_render_layout(gtk_widget_get_style_context(widget), cr, x, y,
                      accel_layout);
...
static void meta_accel_label_update(MetaAccelLabel *accel_label) {
  ...
  if (accel_label->accel_mods & META_VIRTUAL_CONTROL_MASK) {
    if (seen_mod) g_string_append(gstring, class->mod_separator);
  ...
  if (ch && (g_unichar_isgraph(ch) || ch == ' ') &&
    ...
    if (tmp[0] != 0 && tmp[1] == 0) tmp[0] = g_ascii_toupper(tmp[0]);
