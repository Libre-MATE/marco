...
#define GDK_COLOR_RGBA(color)                          \
  ((guint32)(0xff | ((int)((color).red * 255) << 24) | \
...
static cairo_surface_t *scale_surface(cairo_surface_t *surface,
  ...
  scaled =
      cairo_surface_create_similar(surface, content, (int)width, (int)height);
...
static cairo_surface_t *get_surface_from_pixbuf(GdkPixbuf *pixbuf,
  ...
  if (fill_type != META_IMAGE_FILL_TILE) {
    ...
    scaled = scale_surface(surface, pixbuf_width, pixbuf_height, width, height,
  ...
  if (fill_type == META_IMAGE_FILL_TILE || vertical_stripes ||
      horizontal_stripes) {
...
static GdkPixbuf *colorize_pixbuf(GdkPixbuf *orig, GdkRGBA *new_color) {
  GdkPixbuf *pixbuf;
  double intensity;
  int x, y;
  const guchar *src;
  ...
  pixbuf = gdk_pixbuf_new(
      gdk_pixbuf_get_colorspace(orig), gdk_pixbuf_get_has_alpha(orig),
      gdk_pixbuf_get_bits_per_sample(orig), gdk_pixbuf_get_width(orig),
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < width; x++) {
      ...
      if (intensity <= 0.5) {
        ...
        dg = new_color->green +
             (1.0 - new_color->green) * (intensity - 0.5) * 2.0;
        db =
            new_color->blue + (1.0 - new_color->blue) * (intensity - 0.5) * 2.0;
      }
...
static void color_composite(const GdkRGBA *bg, const GdkRGBA *fg, double alpha,
                            GdkRGBA *color) {
  *color = *bg;
 ...
 */
static gboolean validate_geometry_value(int val, const char *name,
                                        GError **error) {
  if (val < 0) {
    g_set_error(error, META_THEME_ERROR, META_THEME_ERROR_FRAME_GEOMETRY,
                _("frame geometry does not specify \"%s\" dimension"), name);
    return FALSE;
  } else
    return TRUE;
}
...
#define CHECK_GEOMETRY_VALUE(vname) \
  if (!validate_geometry_value(layout->vname, #vname, error)) return FALSE

#define CHECK_GEOMETRY_BORDER(bname) \
  if (!validate_geometry_border(&layout->bname, #bname, error)) return FALSE
...
void meta_frame_layout_get_borders(const MetaFrameLayout *layout,
  ...
  if (flags & META_FRAME_FULLSCREEN) return;

  g_return_if_fail(layout != NULL);

  if (!layout->has_title) text_height = 0;

  buttons_height = layout->button_height + layout->button_border.top +
                   layout->button_border.bottom;
  title_height = text_height + layout->title_vertical_pad +
...
static MetaButtonType map_button_function_to_type(MetaButtonFunction function) {
  switch (function) {
    ...
    switch (function) {
      ...
      case META_BUTTON_FUNCTION_UNABOVE:
        if (flags & META_FRAME_ABOVE)
          ...
          return NULL;
      case META_BUTTON_FUNCTION_UNSTICK:
        if (flags & META_FRAME_STUCK) return &fgeom->unstick_rect;
      default:
...
static gboolean strip_button(
    MetaButtonSpace *func_rects[MAX_BUTTONS_PER_CORNER],
    GdkRectangle *bg_rects[MAX_BUTTONS_PER_CORNER], int *n_rects,
...
void meta_frame_layout_calc_geometry(const MetaFrameLayout *layout,
  ...
  meta_frame_layout_get_borders(layout, text_height, flags, &borders);
  ...
  switch (layout->button_sizing) {
    case META_BUTTON_SIZING_ASPECT:
      button_height = borders.visible.top - layout->button_border.top -
                      layout->button_border.bottom;
  ...
  memset(ADDRESS_OF_BUTTON_RECTS(fgeom), '\0', LENGTH_OF_BUTTON_RECTS);
  ...
  if (!layout->hide_buttons) {
    ...
    for (i = 0; i < MAX_BUTTONS_PER_CORNER &&
                button_layout->left_buttons[i] != META_BUTTON_FUNCTION_LAST;
         i++) {
      left_func_rects[n_left] = rect_for_function(
          fgeom, flags, button_layout->left_buttons[i], theme);
      if (left_func_rects[n_left] != NULL) {
        left_buttons_has_spacer[n_left] =
            button_layout->left_buttons_has_spacer[i];
        if (button_layout->left_buttons_has_spacer[i]) ++n_left_spacers;
    ...
    for (i = 0; i < MAX_BUTTONS_PER_CORNER &&
         ...
         i++) {
      right_func_rects[n_right] = rect_for_function(
          fgeom, flags, button_layout->right_buttons[i], theme);
      if (right_func_rects[n_right] != NULL) {
        right_buttons_has_spacer[n_right] =
            button_layout->right_buttons_has_spacer[i];
        if (button_layout->right_buttons_has_spacer[i]) ++n_right_spacers;
  ...
  while (n_left > 0 || n_right > 0) {
    ...
    space_available =
        fgeom->width - layout->left_titlebar_edge - layout->right_titlebar_edge;

    space_used_by_buttons = 0;

    space_used_by_buttons += button_width * n_left;
    space_used_by_buttons +=
        (int)(0.75 * (double)(button_width * n_left_spacers));
    space_used_by_buttons += layout->button_border.left * n_left;
    space_used_by_buttons += layout->button_border.right * n_left;

    space_used_by_buttons += button_width * n_right;
    space_used_by_buttons +=
        (int)(0.75 * (double)(button_width * n_right_spacers));
     ...
     */
    if (strip_button(left_func_rects, left_bg_rects, &n_left,
                     &fgeom->above_rect))
      continue;
    else if (strip_button(right_func_rects, right_bg_rects, &n_right,
                          &fgeom->above_rect))
      continue;
    else if (strip_button(left_func_rects, left_bg_rects, &n_left,
                          &fgeom->stick_rect))
      continue;
    else if (strip_button(right_func_rects, right_bg_rects, &n_right,
                          &fgeom->stick_rect))
  ...
  button_y = (borders.visible.top - (button_height + layout->button_border.top +
                                     layout->button_border.bottom)) /
                 2 +
  ...
  while (i >= 0) {
    ...
    if (x <
        0) /* if we go negative, leave the buttons we don't get to as 0-width */
      break;
    ...
    if (flags & META_FRAME_MAXIMIZED || flags & META_FRAME_TILED_LEFT ||
      ...
      if (i == n_right - 1)
        rect->clickable.width += layout->right_titlebar_edge +
                                 layout->right_width +
  ...
  for (i = 0; i < n_left; i++) {
    ...
    if (left_buttons_has_spacer[i]) x += (int)(0.75 * (double)button_width);
  ...
  fgeom->title_rect.height = borders.visible.top - layout->title_border.top -
  ...
  if (borders.visible.bottom + borders.visible.right >= min_size_for_rounding)
    fgeom->bottom_right_corner_rounded_radius =
        layout->bottom_right_corner_rounded_radius;
}
...
static cairo_pattern_t *create_cairo_pattern_from_gradient_spec(
    const MetaGradientSpec *spec, const MetaAlphaGradientSpec *alpha_spec,
...
void meta_gradient_spec_render(const MetaGradientSpec *spec,
                               ...
                               cairo_t *cr, GtkStyleContext *context, gint x,
...
cairo_pattern_t *meta_alpha_gradient_spec_get_mask(
    const MetaAlphaGradientSpec *spec) {
  gint n_alphas;
  cairo_pattern_t *pattern;
  gint i;
  ...
  for (i = 0; i < n_alphas; i++)
    cairo_pattern_add_color_stop_rgba(pattern, i / (gfloat)(n_alphas - 1), 0, 0,
                                      0, spec->alphas[i] / 255.0);
...
void meta_color_spec_free(MetaColorSpec *spec) {
  ...
  switch (spec->type) {
    ...
    case META_COLOR_SPEC_SHADE:
      if (spec->data.shade.base) meta_color_spec_free(spec->data.shade.base);
...
MetaColorSpec *meta_color_spec_new_from_string(const char *str, GError **err) {
  ...
  if (strncmp(str, "gtk:custom", 10) == 0) {
    ...
    if (str[10] != '(') {
      g_set_error(
          err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
          _("GTK custom color specification must have color name and fallback "
            "in parentheses, e.g. gtk:custom(foo,bar); could not parse \"%s\""),
          str);
      return NULL;
    }
    ...
    while (*fallback_str_start && *fallback_str_start != ',') {
      if (!(g_ascii_isalnum(*fallback_str_start) ||
        ...
        g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                    _("Invalid character '%c' in color_name parameter of "
                      "gtk:custom, only A-Za-z0-9-_ are valid"),
                    *fallback_str_start);
        return NULL;
      }
      fallback_str_start++;
    ...
    if (color_name_start == NULL || fallback_str_start == NULL || end == NULL) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Gtk:custom format is \"gtk:custom(color_name,fallback)\", "
    ...
    color_name =
        g_strndup(color_name_start, fallback_str_start - color_name_start - 1);
    ...
    if (*bracket == '\0') {
      g_set_error(
          err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
          _("GTK color specification must have the state in brackets, e.g. "
    ...
    if (*end_bracket == '\0') {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("GTK color specification must have a close bracket after "
                    "the state, e.g. gtk:fg[NORMAL] where NORMAL is the state; "
                    "could not parse \"%s\""),
                  str);
      return NULL;
    }
    ...
    if (split[0] == NULL || split[1] == NULL || split[2] == NULL ||
        split[3] == NULL) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Blend format is \"blend/bg_color/fg_color/alpha\", \"%s\" "
    ...
    if (split[0] == NULL || split[1] == NULL || split[2] == NULL) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Shade format is \"shade/base_color/factor\", \"%s\" does "
                    "not fit the format"),
                  str);
      g_strfreev(split);
      return NULL;
...
static void get_background_color_real(GtkStyleContext *context,
...
void meta_gtk_style_get_light_color(GtkStyleContext *style, GtkStateFlags state,
                                    GdkRGBA *color) {
  get_background_color(style, state, color);
  gtk_style_shade(color, color, LIGHTNESS_MULT);
}

void meta_gtk_style_get_dark_color(GtkStyleContext *style, GtkStateFlags state,
                                   GdkRGBA *color) {
  get_background_color(style, state, color);
  gtk_style_shade(color, color, DARKNESS_MULT);
}

static void meta_set_color_from_style(GdkRGBA *color, GtkStyleContext *context,
                                      GtkStateFlags state,
                                      MetaGtkColorComponent component) {
  GdkRGBA other;
...
static void meta_set_custom_color_from_style(GdkRGBA *color,
                                             GtkStyleContext *context,
                                             char *color_name,
...
void meta_color_spec_render(MetaColorSpec *spec, GtkStyleContext *style,
                            GdkRGBA *color) {
  g_return_if_fail(spec != NULL);
  ...
  switch (spec->type) {
    ...
    case META_COLOR_SPEC_GTK:
      meta_set_color_from_style(color, style, spec->data.gtk.state,
                                spec->data.gtk.component);
      break;

    case META_COLOR_SPEC_GTK_CUSTOM:
      meta_set_custom_color_from_style(color, style,
    ...
    case META_COLOR_SPEC_SHADE: {
      ...
      gtk_style_shade(&spec->data.shade.color, &spec->data.shade.color,
                      spec->data.shade.factor);
...
static void free_tokens(PosToken *tokens, int n_tokens) {
  ...
  for (i = 0; i < n_tokens; i++)
    if (tokens[i].type == POS_TOKEN_VARIABLE) g_free(tokens[i].d.v.name);
 ...
 * \ingroup tokenizer
 */
static gboolean parse_number(const char *p, const char **end_return,
                             PosToken *next, GError **err) {
  const char *start = p;
  char *end;
  gboolean is_float;
  char *num_str;

  while (*p && (*p == '.' || g_ascii_isdigit(*p))) ++p;

  if (p == start) {
    char buf[7] = {'\0'};
    buf[g_unichar_to_utf8(g_utf8_get_char(p), buf)] = '\0';
    g_set_error(
        err, META_THEME_ERROR, META_THEME_ERROR_BAD_CHARACTER,
  ...
  *end_return = p;
  ...
  if (is_float) {
    ...
    if (end == num_str) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Coordinate expression contains floating point number '%s' "
    ...
    if (end == num_str) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Coordinate expression contains integer '%s' which could "
                    "not be parsed"),
                  num_str);
      g_free(num_str);
      return FALSE;
 ...
 * \ingroup tokenizer
 */
static gboolean pos_tokenize(const char *expr, PosToken **tokens_p,
                             int *n_tokens_p, GError **err) {
  PosToken *tokens;
  int n_tokens;
  int allocated;
  const char *p;
    ...
    switch (*p) {
      ...
      case '`':
        ...
        if (next->d.o.op != POS_OP_NONE) {
          ...
          g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                      _("Coordinate expression contained unknown operator at "
                        "the start of this text: \"%s\""),
                      p);
      ...
      default:
        if (IS_VARIABLE_CHAR(*p)) {
          ...
          if (!parse_number(p, &end, next, err)) goto error;
 ...
 * \ingroup parser
 */
typedef enum { POS_EXPR_INT, POS_EXPR_DOUBLE, POS_EXPR_OPERATOR } PosExprType;
...
static gboolean do_operation(PosExpr *a, PosExpr *b, PosOperatorType op,
                             GError **err) {
  /* Promote types to double if required */
  if (a->type == POS_EXPR_DOUBLE || b->type == POS_EXPR_DOUBLE) {
  ...
  if (a->type == POS_EXPR_INT) {
    switch (op) {
      ...
      case POS_OP_DIVIDE:
        if (b->d.int_val == 0) {
          g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_DIVIDE_BY_ZERO,
    ...
    switch (op) {
      ...
      case POS_OP_DIVIDE:
        ...
        break;
      case POS_OP_MOD:
        g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_MOD_ON_FLOAT,
                    _("Coordinate expression tries to use mod operator on a "
...
static gboolean do_operations(PosExpr *exprs, int *n_exprs, int precedence,
                              GError **err) {
  int i;
  ...
  while (i < *n_exprs) {
    ...
    if (exprs[i - 1].type == POS_EXPR_OPERATOR) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Coordinate expression has an operator \"%s\" where an "
    ...
    if (exprs[i].type != POS_EXPR_OPERATOR) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Coordinate expression had an operand where an operator "
                    "was expected"));
      return FALSE;
    }
    ...
    if (i == (*n_exprs - 1)) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Coordinate expression ended with an operator instead of "
                    "an operand"));
      return FALSE;
    }
    ...
    if (exprs[i + 1].type == POS_EXPR_OPERATOR) {
      g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Coordinate expression has operator \"%c\" following "
                    "operator \"%c\" with no operand in between"),
                  exprs[i + 1].d.operator, exprs[i].d.operator);
      return FALSE;
    }
    ...
    switch (precedence) {
      case 2:
        switch (exprs[i].d.operator) {
          ...
          case POS_OP_MULTIPLY:
            compress = TRUE;
            if (!do_operation(&exprs[i - 1], &exprs[i + 1], exprs[i].d.operator,
                              err))
              return FALSE;
            break;
        }
    ...
    if (compress) {
       ...
       */
      if ((i + 2) < *n_exprs) {
        memmove(&exprs[i], &exprs[i + 2], sizeof(PosExpr) * (*n_exprs - i - 2));
      }
 ...
 * \ingroup parser
 */
static gboolean pos_eval_get_variable(PosToken *t, int *result,
  ...
  if (env->theme) {
    if (t->d.v.name_quark == env->theme->quark_width)
      ...
      *result = env->frame_y_center;
      ...
      g_set_error(
          err, META_THEME_ERROR, META_THEME_ERROR_UNKNOWN_VARIABLE,
    ...
    if (strcmp(t->d.v.name, "width") == 0)
      ...
      *result = env->rect.height;
    else if (env->object_width >= 0 && strcmp(t->d.v.name, "object_width") == 0)
      *result = env->object_width;
    else if (env->object_height >= 0 &&
 ...
 * \ingroup parser
 */
static gboolean pos_eval_helper(PosToken *tokens, int n_tokens,
                                const MetaPositionExprEnv *env, PosExpr *result,
                                GError **err) {
...
#define MAX_EXPRS 32
  ...
  for (i = 0; i < n_tokens; i++) {
    ...
    if (paren_level == 0) {
      switch (t->type) {
        ...
        case POS_TOKEN_OPEN_PAREN:
          ++paren_level;
          if (paren_level == 1) first_paren = i;
          break;

        case POS_TOKEN_CLOSE_PAREN:
          g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_BAD_PARENS,
                      _("Coordinate expression had a close parenthesis with no "
                        "open parenthesis"));
          return FALSE;
      ...
      switch (t->type) {
        ...
        case POS_TOKEN_CLOSE_PAREN:
          if (paren_level == 1) {
            ...
            if (!pos_eval_helper(&tokens[first_paren + 1], i - first_paren - 1,
                                 env, &exprs[n_exprs], err))
              return FALSE;
  ...
  if (paren_level > 0) {
    g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_BAD_PARENS,
                _("Coordinate expression had an open parenthesis with no close "
                  "parenthesis"));
    return FALSE;
  }
  ...
  if (n_exprs == 0) {
    g_set_error(err, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                _("Coordinate expression doesn't seem to have any operators or "
                  "operands"));
    return FALSE;
  }
  ...
  precedence = 2;
  while (precedence >= 0) {
    if (!do_operations(exprs, &n_exprs, precedence, err)) return FALSE;
    --precedence;
  }
 ...
 * \ingroup parser
 */
static gboolean pos_eval(MetaDrawSpec *spec, const MetaPositionExprEnv *env,
                         int *val_p, GError **err) {
  PosExpr expr;
...
gboolean meta_parse_position_expression(MetaDrawSpec *spec,
  ...
  if (x_return) *x_return = env->rect.x + val;
  if (y_return) *y_return = env->rect.y + val;
 ...
 */
gboolean meta_theme_replace_constants(MetaTheme *theme, PosToken *tokens,
                                      int n_tokens, GError **err) {
  int i;
  double dval;
  int ival;
...
static int parse_x_position_unchecked(MetaDrawSpec *spec,
  ...
  if (!meta_parse_position_expression(spec, env, &retval, NULL, &error)) {
    meta_warning(
        _("Theme contained an expression that resulted in an error: %s\n"),
        error->message);
...
MetaDrawSpec *meta_draw_spec_new(MetaTheme *theme, const char *expr,
                                 GError **error) {
  MetaDrawSpec *spec;
  ...
  spec->constant =
      meta_theme_replace_constants(theme, spec->tokens, spec->n_tokens, NULL);
  if (spec->constant) {
...
void meta_draw_op_free(MetaDrawOp *op) {
  ...
  switch (op->type) {
    ...
    case META_DRAW_RECTANGLE:
      if (op->data.rectangle.color_spec) g_free(op->data.rectangle.color_spec);
    ...
    case META_DRAW_ARC:
      if (op->data.arc.color_spec) g_free(op->data.arc.color_spec);
    ...
    case META_DRAW_TILE:
      if (op->data.tile.op_list) meta_draw_op_list_unref(op->data.tile.op_list);
...
static GdkPixbuf *apply_alpha(GdkPixbuf *pixbuf, MetaAlphaGradientSpec *spec,
                              gboolean force_copy) {
...
static GdkPixbuf *pixbuf_tile(GdkPixbuf *tile, int width, int height) {
  ...
  pixbuf = gdk_pixbuf_new(GDK_COLORSPACE_RGB, gdk_pixbuf_get_has_alpha(tile), 8,
                          width, height);
  ...
  while (i < width) {
    ...
    while (j < height) {
      ...
      gdk_pixbuf_copy_area(tile, 0, 0, w, h, pixbuf, i, j);
...
static GdkPixbuf *replicate_rows(GdkPixbuf *src, int src_x, int src_y,
                                 int width, int height) {
  unsigned int n_channels = gdk_pixbuf_get_n_channels(src);
  unsigned int src_rowstride = gdk_pixbuf_get_rowstride(src);
  unsigned char *pixels =
      (gdk_pixbuf_get_pixels(src) + src_y * src_rowstride + src_x * n_channels);
  ...
  result =
      gdk_pixbuf_new(GDK_COLORSPACE_RGB, n_channels == 4, 8, width, height);
...
static GdkPixbuf *replicate_cols(GdkPixbuf *src, int src_x, int src_y,
                                 int width, int height) {
  unsigned int n_channels = gdk_pixbuf_get_n_channels(src);
  unsigned int src_rowstride = gdk_pixbuf_get_rowstride(src);
  unsigned char *pixels =
...
static GdkPixbuf *scale_and_alpha_pixbuf(GdkPixbuf *src,
                                         ...
                                         MetaImageFillType fill_type, int width,
                                         int height, gboolean vertical_stripes,
                                         gboolean horizontal_stripes) {
  GdkPixbuf *pixbuf;
  ...
  if (gdk_pixbuf_get_width(pixbuf) == width &&
    ...
    if (fill_type == META_IMAGE_FILL_TILE) {
      ...
      if (dest_w == src_w && dest_h == src_h) {
        ...
        temp_pixbuf =
            gdk_pixbuf_scale_simple(src, dest_w, dest_h, GDK_INTERP_BILINEAR);
      }
  ...
  if (pixbuf) pixbuf = apply_alpha(pixbuf, alpha_spec, pixbuf == src);
...
static GdkPixbuf *draw_op_as_pixbuf(const MetaDrawOp *op,
                                    ...
                                    const MetaDrawInfo *info, int width,
                                    int height) {
  ...
  switch (op->type) {
    ...
    case META_DRAW_RECTANGLE:
      if (op->data.rectangle.filled) {
        ...
        meta_color_spec_render(op->data.rectangle.color_spec, style, &color);
    ...
    case META_DRAW_TINT: {
      ...
      has_alpha = op->data.tint.alpha_spec &&
      ...
      if (!has_alpha) {
        ...
        meta_gradient_add_alpha(pixbuf, op->data.tint.alpha_spec->alphas,
    ...
    case META_DRAW_IMAGE: {
      if (op->data.image.colorize_spec) {
        ...
        meta_color_spec_render(op->data.image.colorize_spec, style, &color);
        ...
        if (op->data.image.colorize_cache_pixbuf == NULL ||
          ...
          ((MetaDrawOp *)op)->data.image.colorize_cache_pixbuf =
              colorize_pixbuf(op->data.image.pixbuf, &color);
          ((MetaDrawOp *)op)->data.image.colorize_cache_pixel =
              GDK_COLOR_RGB(color);
        }

        if (op->data.image.colorize_cache_pixbuf) {
          pixbuf = scale_and_alpha_pixbuf(
              op->data.image.colorize_cache_pixbuf, op->data.image.alpha_spec,
              op->data.image.fill_type, width, height,
              op->data.image.vertical_stripes,
              op->data.image.horizontal_stripes);
        }
      } else {
        pixbuf = scale_and_alpha_pixbuf(
            op->data.image.pixbuf, op->data.image.alpha_spec,
            op->data.image.fill_type, width, height,
            op->data.image.vertical_stripes, op->data.image.horizontal_stripes);
      }
      break;
    }
    ...
    case META_DRAW_ICON:
      if (info->mini_icon && width <= gdk_pixbuf_get_width(info->mini_icon) &&
          height <= gdk_pixbuf_get_height(info->mini_icon))
        pixbuf = scale_and_alpha_pixbuf(
            info->mini_icon, op->data.icon.alpha_spec, op->data.icon.fill_type,
            width, height, FALSE, FALSE);
      else if (info->icon)
        pixbuf = scale_and_alpha_pixbuf(info->icon, op->data.icon.alpha_spec,
...
static cairo_surface_t *draw_op_as_surface(const MetaDrawOp *op,
                                           GtkStyleContext *style,
  ...
  switch (op->type) {
    case META_DRAW_IMAGE: {
      if (op->data.image.colorize_spec) {
        ...
        if (op->data.image.colorize_cache_pixbuf) {
          surface = get_surface_from_pixbuf(
              op->data.image.colorize_cache_pixbuf, op->data.image.fill_type,
              width, height, op->data.image.vertical_stripes,
              op->data.image.horizontal_stripes);
        }
      } else {
        surface = get_surface_from_pixbuf(
            op->data.image.pixbuf, op->data.image.fill_type, width, height,
            op->data.image.vertical_stripes, op->data.image.horizontal_stripes);
      }
      break;
    }
    ...
    case META_DRAW_ICON:
      if (info->mini_icon && width <= gdk_pixbuf_get_width(info->mini_icon) &&
          height <= gdk_pixbuf_get_height(info->mini_icon))
        surface =
            get_surface_from_pixbuf(info->mini_icon, op->data.icon.fill_type,
                                    width, height, FALSE, FALSE);
      else if (info->icon)
...
static void fill_env(MetaPositionExprEnv *env, const MetaDrawInfo *info,
  ...
  env->mini_icon_width =
      info->mini_icon ? gdk_pixbuf_get_width(info->mini_icon) : 0;
  env->mini_icon_height =
      info->mini_icon ? gdk_pixbuf_get_height(info->mini_icon) : 0;
...
static void meta_draw_op_draw_with_env(const MetaDrawOp *op,
  ...
  switch (op->type) {
    case META_DRAW_LINE: {
      ...
      if (!op->data.line.x2 && !op->data.line.y2 && op->data.line.width == 0) {
    ...
    case META_DRAW_TINT: {
      ...
      if (!needs_alpha) {
        ...
        pixbuf = draw_op_as_pixbuf(op, style_gtk, info, rwidth, rheight);
    ...
    case META_DRAW_GRADIENT: {
      ...
      meta_gradient_spec_render(op->data.gradient.gradient_spec,
                                op->data.gradient.alpha_spec, cr, style_gtk, rx,
                                ry, rwidth, rheight);
    } break;
    ...
    case META_DRAW_IMAGE: {
      ...
      if (op->data.image.pixbuf) {
        ...
        env->object_height =
            gdk_pixbuf_get_height(op->data.image.pixbuf) / scale;
      }
      ...
      if (surface) {
        ...
        if (op->data.image.alpha_spec) {
          ...
          pattern =
              meta_alpha_gradient_spec_get_mask(op->data.image.alpha_spec);
    ...
    case META_DRAW_TITLE:
      if (info->title_layout) {
        ...
        if (op->data.title.ellipsize_width) {
          ...
          ellipsize_width =
              parse_x_position_unchecked(op->data.title.ellipsize_width, env);
          /* HACK: parse_x_position_unchecked adds in env->rect.x, subtract out
           * again */
          ellipsize_width -= env->rect.x;

          pango_layout_set_width(info->title_layout, -1);
          pango_layout_get_pixel_extents(info->title_layout, &ink_rect,
                                         &logical_rect);

          /* Pango's idea of ellipsization is with respect to the logical rect.
           * correct for this, by reducing the ellipsization width by the
           * overflow of the un-ellipsized text on the right... it's always the
           * visual right we want regardless of bidi, since since the X we pass
           * in to cairo_move_to() is always the left edge of the line.
           */
          right_bearing = (ink_rect.x + ink_rect.width) -
          ...
          if (ellipsize_width < logical_rect.width)
            pango_layout_set_width(info->title_layout,
                                   PANGO_SCALE * ellipsize_width);
        } else if (rx - env->rect.x + env->title_width >= env->rect.width) {
          const double alpha_margin = 30.0;
          int text_space = env->rect.x + env->rect.width - (rx - env->rect.x) -
                           env->right_width;
          ...
          linpat = cairo_pattern_create_linear(rx, ry, text_space,
                                               env->title_height);
          cairo_pattern_add_color_stop_rgba(linpat, 0, color.red, color.green,
                                            color.blue, color.alpha);
          cairo_pattern_add_color_stop_rgba(linpat, startalpha, color.red,
                                            color.green, color.blue,
                                            color.alpha);
          cairo_pattern_add_color_stop_rgba(linpat, 1, color.red, color.green,
                                            color.blue, 0);
    ...
    case META_DRAW_OP_LIST: {
      ...
      meta_draw_op_list_draw_with_style(op->data.op_list.op_list, style_gtk, cr,
                                        info, d_rect);
    } break;
    ...
    case META_DRAW_TILE: {
      ...
      tile_xoffset =
          parse_x_position_unchecked(op->data.tile.tile_xoffset, env);
      tile_yoffset =
          parse_y_position_unchecked(op->data.tile.tile_yoffset, env);
      ...
      while (tile.x < (rx + rwidth)) {
        ...
        while (tile.y < (ry + rheight)) {
          meta_draw_op_list_draw_with_style(op->data.tile.op_list, style_gtk,
                                            cr, info, tile);
...
void meta_draw_op_draw_with_style(const MetaDrawOp *op,
  ...
  meta_draw_op_draw_with_env(op, style_gtk, cr, info, logical_region, &env);
}

void meta_draw_op_draw(const MetaDrawOp *op, GtkWidget *widget, cairo_t *cr,
                       const MetaDrawInfo *info, MetaRectangle logical_region) {
  meta_draw_op_draw_with_style(op, gtk_widget_get_style_context(widget), cr,
                               info, logical_region);
}
...
void meta_draw_op_list_unref(MetaDrawOpList *op_list) {
  ...
  if (op_list->refcount == 0) {
    ...
    for (i = 0; i < op_list->n_ops; i++) meta_draw_op_free(op_list->ops[i]);
...
void meta_draw_op_list_draw_with_style(const MetaDrawOpList *op_list,
  ...
  for (i = 0; i < op_list->n_ops; i++) {
    ...
    if (op->type == META_DRAW_CLIP) {
      ...
      cairo_rectangle(cr, parse_x_position_unchecked(op->data.clip.x, &env),
...
void meta_draw_op_list_draw(const MetaDrawOpList *op_list, GtkWidget *widget,
                            cairo_t *cr, const MetaDrawInfo *info,
                            MetaRectangle rect)

{
  meta_draw_op_list_draw_with_style(
      op_list, gtk_widget_get_style_context(widget), cr, info, rect);
}
...
gboolean meta_draw_op_list_contains(MetaDrawOpList *op_list,
  ...
  for (i = 0; i < op_list->n_ops; i++) {
    if (op_list->ops[i]->type == META_DRAW_OP_LIST) {
      if (op_list->ops[i]->data.op_list.op_list == child) return TRUE;

      if (meta_draw_op_list_contains(op_list->ops[i]->data.op_list.op_list,
                                     child))
        return TRUE;
    } else if (op_list->ops[i]->type == META_DRAW_TILE) {
      if (op_list->ops[i]->data.tile.op_list == child) return TRUE;

      if (meta_draw_op_list_contains(op_list->ops[i]->data.tile.op_list, child))
        return TRUE;
    }
  }
...
MetaFrameStyle *meta_frame_style_new(MetaFrameStyle *parent) {
  ...
  style->parent = parent;
  if (parent) meta_frame_style_ref(parent);
...
static void free_button_ops(
    MetaDrawOpList *op_lists[META_BUTTON_TYPE_LAST][META_BUTTON_STATE_LAST]) {
  int i, j;

  for (i = 0; i < META_BUTTON_TYPE_LAST; i++)
    for (j = 0; j < META_BUTTON_STATE_LAST; j++)
      if (op_lists[i][j]) meta_draw_op_list_unref(op_lists[i][j]);
}
...
void meta_frame_style_unref(MetaFrameStyle *style) {
  ...
  if (style->refcount == 0) {
    ...
    for (i = 0; i < META_FRAME_PIECE_LAST; i++)
      if (style->pieces[i]) meta_draw_op_list_unref(style->pieces[i]);

    if (style->layout) meta_frame_layout_unref(style->layout);
    ...
    if (style->parent) meta_frame_style_unref(style->parent);
...
static MetaButtonState map_button_state(
    MetaButtonType button_type, const MetaFrameGeometry *fgeom,
  ...
  switch (button_type) {
    ...
    case META_BUTTON_TYPE_RIGHT_RIGHT_BACKGROUND:
      if (fgeom->n_right_buttons > 0)
        function =
            fgeom->button_layout.right_buttons[fgeom->n_right_buttons - 1];
      break;
...
static MetaDrawOpList *get_button(MetaFrameStyle *style, MetaButtonType type,
  ...
  if (op_list == NULL && (type == META_BUTTON_TYPE_LEFT_LEFT_BACKGROUND ||
                          type == META_BUTTON_TYPE_LEFT_RIGHT_BACKGROUND))
    return get_button(style, META_BUTTON_TYPE_LEFT_MIDDLE_BACKGROUND, state);

  if (op_list == NULL && (type == META_BUTTON_TYPE_RIGHT_LEFT_BACKGROUND ||
                          type == META_BUTTON_TYPE_RIGHT_RIGHT_BACKGROUND))
    return get_button(style, META_BUTTON_TYPE_RIGHT_MIDDLE_BACKGROUND, state);
    ...
    if (i >= META_BUTTON_TYPE_CLOSE) {
      for (j = 0; j < META_BUTTON_STATE_LAST; j++) {
        if (get_button(style, i, j) == NULL &&
            meta_theme_earliest_version_with_button(i) <=
                current_theme_version) {
          g_set_error(
              error, META_THEME_ERROR, META_THEME_ERROR_FAILED,
              _("<button function=\"%s\" state=\"%s\" draw_ops=\"whatever\"/> "
                "must be specified for this frame style"),
              meta_button_type_to_string(i), meta_button_state_to_string(j));
          return FALSE;
        }
      }
    }
...
static void get_button_rect(MetaButtonType type, const MetaFrameGeometry *fgeom,
                            int middle_background_offset, GdkRectangle *rect) {
  switch (type) {
    case META_BUTTON_TYPE_LEFT_LEFT_BACKGROUND:
...
void meta_frame_style_draw_with_style(
    MetaFrameStyle *style, GtkStyleContext *style_gtk, cairo_t *cr,
    const MetaFrameGeometry *fgeom, int client_width, int client_height,
    PangoLayout *title_layout, int text_height,
    MetaButtonState button_states[META_BUTTON_TYPE_LAST], GdkPixbuf *mini_icon,
    GdkPixbuf *icon) {
  /* BOOKMARK */
  int i, j;
  ...
  visible_rect.width =
      fgeom->width - borders->invisible.left - borders->invisible.right;
  visible_rect.height =
  ...
  right_titlebar_edge.x =
      titlebar_rect.x + titlebar_rect.width - right_titlebar_edge.width;
  ...
  bottom_titlebar_edge.y =
      titlebar_rect.y + titlebar_rect.height - bottom_titlebar_edge.height;

  left_edge.x = visible_rect.x;
  left_edge.y = visible_rect.y + borders->visible.top;
  left_edge.width = borders->visible.left;
  left_edge.height =
      visible_rect.height - borders->visible.top - borders->visible.bottom;
  ...
  bottom_edge.y =
      visible_rect.y + visible_rect.height - borders->visible.bottom;
  bottom_edge.width = visible_rect.width;
  bottom_edge.height = borders->visible.bottom;

  if (title_layout)
    pango_layout_get_pixel_extents(title_layout, NULL, &extents);
  ...
  while (i < META_FRAME_PIECE_LAST) {
    ...
    switch ((MetaFramePiece)i) {
      ...
      case META_FRAME_PIECE_TITLEBAR_MIDDLE:
        ...
        rect.height = titlebar_rect.height - top_titlebar_edge.height -
                      bottom_titlebar_edge.height;
        break;
    ...
    if (gdk_cairo_get_clip_rectangle(cr, NULL)) {
      ...
      if (op_list) {
        ...
        meta_draw_op_list_draw_with_style(op_list, style_gtk, cr, &draw_info,
                                          m_rect);
      }
    }
    ...
    if ((i + 1) == META_FRAME_PIECE_OVERLAY) {
      ...
      while (j < META_BUTTON_TYPE_LAST) {
        ...
        button_state =
            map_button_state(j, fgeom, middle_bg_offset, button_states);
        ...
        if (op_list) {
          ...
          if (gdk_cairo_get_clip_rectangle(cr, NULL)) {
            ...
            meta_draw_op_list_draw_with_style(op_list, style_gtk, cr,
                                              &draw_info, m_rect);
          }
...
void meta_frame_style_draw(MetaFrameStyle *style, GtkWidget *widget,
                           cairo_t *cr, const MetaFrameGeometry *fgeom,
                           int client_width, int client_height,
                           PangoLayout *title_layout, int text_height,
                           MetaButtonState button_states[META_BUTTON_TYPE_LAST],
                           GdkPixbuf *mini_icon, GdkPixbuf *icon) {
  meta_frame_style_draw_with_style(
      style, gtk_widget_get_style_context(widget), cr, fgeom, client_width,
      client_height, title_layout, text_height, button_states, mini_icon, icon);
}
...
MetaFrameStyleSet *meta_frame_style_set_new(MetaFrameStyleSet *parent) {
  ...
  if (parent) meta_frame_style_set_ref(parent);
...
static void free_focus_styles(
    MetaFrameStyle *focus_styles[META_FRAME_FOCUS_LAST]) {
  int i;

  for (i = 0; i < META_FRAME_FOCUS_LAST; i++)
    if (focus_styles[i]) meta_frame_style_unref(focus_styles[i]);
}
...
void meta_frame_style_set_unref(MetaFrameStyleSet *style_set) {
  ...
  if (style_set->refcount == 0) {
    ...
    if (style_set->parent) meta_frame_style_set_unref(style_set->parent);
...
static MetaFrameStyle *get_style(MetaFrameStyleSet *style_set,
                                 MetaFrameState state, MetaFrameResize resize,
    ...
    default: {
      ...
      if (style == NULL) {
        if (state == META_FRAME_STATE_TILED_LEFT ||
          ...
          style = get_style(style_set, META_FRAME_STATE_NORMAL, resize, focus);
        else if (state == META_FRAME_STATE_TILED_LEFT_AND_SHADED ||
                 state == META_FRAME_STATE_TILED_RIGHT_AND_SHADED)
          style = get_style(style_set, META_FRAME_STATE_SHADED, resize, focus);
      }
...
static gboolean check_state(MetaFrameStyleSet *style_set, MetaFrameState state,
                            GError **error) {
  int i;

  for (i = 0; i < META_FRAME_FOCUS_LAST; i++) {
    if (get_style(style_set, state, META_FRAME_RESIZE_NONE, i) == NULL) {
      /* Translators: This error occurs when a <frame> tag is missing
       * in theme XML.  The "<frame ...>" is intended as a noun phrase,
       * and the "missing" qualifies it.  You should translate "whatever".
       */
      g_set_error(error, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                  _("Missing <frame state=\"%s\" resize=\"%s\" focus=\"%s\" "
                    "style=\"whatever\"/>"),
...
gboolean meta_frame_style_set_validate(MetaFrameStyleSet *style_set,
  ...
  if (!check_state(style_set, META_FRAME_STATE_SHADED, error)) return FALSE;

  if (!check_state(style_set, META_FRAME_STATE_MAXIMIZED, error)) return FALSE;
...
MetaTheme *meta_theme_get_current(void) { return meta_current_theme; }
...
void meta_theme_set_current(const char *name, gboolean force_reload) {
  ...
  if (new_theme == NULL) {
    ...
    if (meta_current_theme) meta_theme_free(meta_current_theme);
...
MetaTheme *meta_theme_new(void) {
  ...
  theme->layouts_by_name = g_hash_table_new_full(
      g_str_hash, g_str_equal, g_free, (GDestroyNotify)meta_frame_layout_unref);

  theme->draw_op_lists_by_name = g_hash_table_new_full(
      g_str_hash, g_str_equal, g_free, (GDestroyNotify)meta_draw_op_list_unref);

  theme->styles_by_name = g_hash_table_new_full(
      g_str_hash, g_str_equal, g_free, (GDestroyNotify)meta_frame_style_unref);
  ...
  theme->quark_mini_icon_height =
      g_quark_from_static_string("mini_icon_height");
...
void meta_theme_free(MetaTheme *theme) {
  ...
  if (theme->integer_constants) g_hash_table_destroy(theme->integer_constants);
  if (theme->images_by_filename)
    g_hash_table_destroy(theme->images_by_filename);
  if (theme->layouts_by_name) g_hash_table_destroy(theme->layouts_by_name);
  if (theme->draw_op_lists_by_name)
    g_hash_table_destroy(theme->draw_op_lists_by_name);
  if (theme->styles_by_name) g_hash_table_destroy(theme->styles_by_name);
...
gboolean meta_theme_validate(MetaTheme *theme, GError **error) {
  ...
  for (i = 0; i < (int)META_FRAME_TYPE_LAST; i++)
    if (i != (int)META_FRAME_TYPE_ATTACHED &&
      ...
      g_set_error(
          error, META_THEME_ERROR, META_THEME_ERROR_FAILED,
          _("No frame style set for window type \"%s\" in theme \"%s\", add a "
            "<window type=\"%s\" style_set=\"whatever\"/> element"),
          meta_frame_type_to_string(i), theme->name,
...
GdkPixbuf *meta_theme_load_image(MetaTheme *theme, const char *filename,
                                 guint size_of_theme_icons, GError **error) {
  GdkPixbuf *pixbuf;
  int scale;

  pixbuf = g_hash_table_lookup(theme->images_by_filename, filename);
  ...
  if (pixbuf == NULL) {
    if (g_str_has_prefix(filename, "theme:") &&
      ...
      pixbuf = gtk_icon_theme_load_icon_for_scale(
          gtk_icon_theme_get_default(), filename + 6, size_of_theme_icons,
          scale, 0, error);
      if (pixbuf == NULL) return NULL;
    } else {
      char *full_path;
      ...
      pixbuf =
          gdk_pixbuf_new_from_file_at_size(full_path, width, height, error);
    ...
    g_hash_table_replace(theme->images_by_filename, g_strdup(filename), pixbuf);
  }
...
static MetaFrameStyle *theme_get_style(MetaTheme *theme, MetaFrameType type,
                                       MetaFrameFlags flags) {
  ...
  switch (flags & (META_FRAME_ALLOWS_VERTICAL_RESIZE |
...
MetaFrameStyle *meta_theme_get_frame_style(MetaTheme *theme, MetaFrameType type,
                                           MetaFrameFlags flags) {
  MetaFrameStyle *style;
...
double meta_theme_get_title_scale(MetaTheme *theme, MetaFrameType type,
                                  MetaFrameFlags flags) {
  MetaFrameStyle *style;
...
void meta_theme_draw_frame(MetaTheme *theme, GtkStyleContext *style_gtk,
                           cairo_t *cr, MetaFrameType type,
                           MetaFrameFlags flags, int client_width,
                           int client_height, PangoLayout *title_layout,
                           int text_height,
                           const MetaButtonLayout *button_layout,
                           MetaButtonState button_states[META_BUTTON_TYPE_LAST],
                           GdkPixbuf *mini_icon, GdkPixbuf *icon) {
  ...
  meta_frame_layout_calc_geometry(style->layout, text_height, flags,
                                  client_width, client_height, button_layout,
                                  &fgeom, theme);

  meta_frame_style_draw_with_style(style, style_gtk, cr, &fgeom, client_width,
                                   client_height, title_layout, text_height,
                                   button_states, mini_icon, icon);
}

void meta_theme_draw_frame_by_name(
    MetaTheme *theme, GtkWidget *widget, cairo_t *cr, const gchar *style_name,
    MetaFrameFlags flags, int client_width, int client_height,
    PangoLayout *title_layout, int text_height,
    const MetaButtonLayout *button_layout,
    MetaButtonState button_states[META_BUTTON_TYPE_LAST], GdkPixbuf *mini_icon,
    GdkPixbuf *icon) {
  ...
  meta_frame_style_draw(style, widget, cr, &fgeom, client_width, client_height,
                        title_layout, text_height, button_states, mini_icon,
                        icon);
}

void meta_theme_get_frame_borders(MetaTheme *theme, MetaFrameType type,
                                  int text_height, MetaFrameFlags flags,
                                  MetaFrameBorders *borders) {
  MetaFrameStyle *style;
  ...
  meta_frame_layout_get_borders(style->layout, text_height, flags, borders);
}

void meta_theme_calc_geometry(MetaTheme *theme, MetaFrameType type,
                              int text_height, MetaFrameFlags flags,
                              int client_width, int client_height,
                              const MetaButtonLayout *button_layout,
                              MetaFrameGeometry *fgeom) {
...
void meta_theme_insert_layout(MetaTheme *theme, const char *name,
                              MetaFrameLayout *layout) {
...
void meta_theme_insert_draw_op_list(MetaTheme *theme, const char *name,
                                    MetaDrawOpList *op_list) {
...
void meta_theme_insert_style(MetaTheme *theme, const char *name,
                             MetaFrameStyle *style) {
...
void meta_theme_insert_style_set(MetaTheme *theme, const char *name,
                                 MetaFrameStyleSet *style_set) {
...
gboolean meta_theme_define_int_constant(MetaTheme *theme, const char *name,
                                        int value, GError **error) {
  if (theme->integer_constants == NULL)
    theme->integer_constants =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

  if (!first_uppercase(name)) {
    g_set_error(error, META_THEME_ERROR, META_THEME_ERROR_FAILED,
                _("User-defined constants must begin with a capital letter; "
                  "\"%s\" does not"),
                name);
    return FALSE;
  }

  if (g_hash_table_lookup_extended(theme->integer_constants, name, NULL,
                                   NULL)) {
    g_set_error(error, META_THEME_ERROR, META_THEME_ERROR_FAILED,
  ...
  g_hash_table_insert(theme->integer_constants, g_strdup(name),
                      GINT_TO_POINTER(value));

  return TRUE;
}

gboolean meta_theme_lookup_int_constant(MetaTheme *theme, const char *name,
                                        int *value) {
  gpointer old_value;

  *value = 0;

  if (theme->integer_constants == NULL) return FALSE;
...
gboolean meta_theme_define_float_constant(MetaTheme *theme, const char *name,
                                          double value, GError **error) {
  double *d;

  if (theme->float_constants == NULL)
    theme->float_constants =
  ...
  *d = value;
  ...
  g_hash_table_insert(theme->float_constants, g_strdup(name), d);

  return TRUE;
}

gboolean meta_theme_lookup_float_constant(MetaTheme *theme, const char *name,
                                          double *value) {
  double *d;

  *value = 0.0;

  if (theme->float_constants == NULL) return FALSE;
...
gboolean meta_theme_define_color_constant(MetaTheme *theme, const char *name,
                                          const char *value, GError **error) {
  if (theme->color_constants == NULL)
    theme->color_constants =
        g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
  ...
  g_hash_table_insert(theme->color_constants, g_strdup(name), g_strdup(value));
 ...
 */
gboolean meta_theme_lookup_color_constant(MetaTheme *theme, const char *name,
                                          char **value) {
  char *result;

  *value = NULL;

  if (theme->color_constants == NULL) return FALSE;
...
PangoFontDescription *meta_gtk_widget_get_font_desc(
    GtkWidget *widget, double scale, const PangoFontDescription *override) {
  PangoFontDescription *font_desc;
  gint font_size;

  GtkStyleContext *style = gtk_widget_get_style_context(widget);
  GtkStateFlags state = gtk_widget_get_state_flags(widget);
  gtk_style_context_get(style, state, GTK_STYLE_PROPERTY_FONT, &font_desc,
                        NULL);

  if (override) pango_font_description_merge(font_desc, override, TRUE);
