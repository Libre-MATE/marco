...
static void meta_ui_accelerator_parse(const char *accel, guint *keysym,
                                      guint *keycode, GdkModifierType *keymask);
...
static gboolean maybe_redirect_mouse_event(XEvent *xevent) {
  GdkDisplay *gdisplay;
  MetaUI *ui;
  GdkSeat *seat;
  GdkDevice *gdevice;
   ...
   */
  if (gdk_display_device_is_grabbed(gdisplay, gdevice)) return FALSE;
  ...
  switch (xevent->type) {
    ...
    case ButtonRelease:
      if (xevent->type == ButtonPress) {
        ...
        if (xevent->xbutton.button == ui->button_click_number &&
            ...
            ABS(xevent->xbutton.x - ui->button_click_x) <=
                double_click_distance &&
            ABS(xevent->xbutton.y - ui->button_click_y) <=
                double_click_distance) {
    ...
    case EnterNotify:
    case LeaveNotify:
      gevent = gdk_event_new(xevent->type == EnterNotify ? GDK_ENTER_NOTIFY
                                                         : GDK_LEAVE_NOTIFY);
...
static GdkFilterReturn filter_func(GdkXEvent *xevent, GdkEvent *event,
                                   gpointer data) {
  g_return_val_if_fail(ef != NULL, GDK_FILTER_CONTINUE);

  if ((*ef->func)(xevent, ef->data) || maybe_redirect_mouse_event(xevent))
    return GDK_FILTER_REMOVE;
  else
    return GDK_FILTER_CONTINUE;
}

void meta_ui_add_event_func(Display *xdisplay, MetaEventFunc func,
                            gpointer data) {
...
void meta_ui_remove_event_func(Display *xdisplay, MetaEventFunc func,
                               gpointer data) {
  g_return_if_fail(ef != NULL);
...
void meta_ui_get_frame_borders(MetaUI *ui, Window frame_xwindow,
                               MetaFrameBorders *borders) {
  meta_frames_get_borders(ui->frames, frame_xwindow, borders);
}

void meta_ui_get_corner_radiuses(MetaUI *ui, Window xwindow, float *top_left,
                                 float *top_right, float *bottom_left,
                                 float *bottom_right) {
  meta_frames_get_corner_radiuses(ui->frames, xwindow, top_left, top_right,
...
static void set_background_none(Display *xdisplay, Window xwindow) {
  ...
  XChangeWindowAttributes(xdisplay, xwindow, CWBackPixmap, &attrs);
}

Window meta_ui_create_frame_window(MetaUI *ui, Display *xdisplay,
                                   Visual *xvisual, gint x, gint y, gint width,
                                   gint height) {
  ...
  if (!xvisual)
    ...
    visual = gdk_x11_screen_lookup_visual(screen, XVisualIDFromVisual(xvisual));
  }
   ...
   */
  attrs.event_mask = GDK_EXPOSURE_MASK | GDK_BUTTON_PRESS_MASK |
  ...
  window = gdk_window_new(gdk_screen_get_root_window(screen), &attrs,
                          attributes_mask);
...
void meta_ui_move_resize_frame(MetaUI *ui, Window frame, int x, int y,
...
void meta_ui_map_frame(MetaUI *ui, Window xwindow) {
  ...
  if (window) gdk_window_show_unraised(window);
}
...
void meta_ui_unmap_frame(MetaUI *ui, Window xwindow) {
  ...
  if (window) gdk_window_hide(window);
}

void meta_ui_update_frame_style(MetaUI *ui, Window xwindow) {
...
void meta_ui_apply_frame_shape(MetaUI *ui, Window xwindow, int new_window_width,
                               int new_window_height,
                               gboolean window_has_shape) {
  meta_frames_apply_shapes(ui->frames, xwindow, new_window_width,
                           new_window_height, window_has_shape);
}

cairo_region_t *meta_ui_get_frame_bounds(MetaUI *ui, Window xwindow,
                                         int window_width, int window_height) {
  return meta_frames_get_frame_bounds(ui->frames, xwindow, window_width,
                                      window_height);
}
...
MetaWindowMenu *meta_ui_window_menu_new(MetaUI *ui, Window client_xwindow,
                                        MetaMenuOp ops, MetaMenuOp insensitive,
                                        unsigned long active_workspace,
                                        int n_workspaces,
                                        MetaWindowMenuFunc func,
                                        gpointer data) {
  return meta_window_menu_new(ui->frames, ops, insensitive, client_xwindow,
                              active_workspace, n_workspaces, func, data);
}

void meta_ui_window_menu_popup(MetaWindowMenu *menu, int root_x, int root_y,
...
GdkPixbuf *meta_gdk_pixbuf_get_from_pixmap(GdkPixbuf *dest, Pixmap xpixmap,
  ...
  if (!XGetGeometry(display, xpixmap, &root_return, &x_ret, &y_ret, &w_ret,
                    &h_ret, &bw_ret, &depth_ret))
    return NULL;

  if (depth_ret == 1) {
    surface = cairo_xlib_surface_create_for_bitmap(
        display, xpixmap, GDK_SCREEN_XSCREEN(gdk_screen_get_default()), w_ret,
        h_ret);
  } else {
    if (!XGetWindowAttributes(display, root_return, &attrs)) return NULL;

    surface =
        cairo_xlib_surface_create(display, xpixmap, attrs.visual, w_ret, h_ret);
  }

  retval = gdk_pixbuf_get_from_surface(surface, src_x, src_y, width, height);
...
static GdkPixbuf *load_default_window_icon(int size, int scale) {
  ...
  return gtk_icon_theme_load_icon_for_scale(theme, icon_name, size, scale, 0,
                                            NULL);
}
...
static GdkPixbuf *load_window_icon_from_name(char *name, int size, int scale) {
  GtkIconTheme *theme = gtk_icon_theme_get_default();
  GdkPixbuf *pixbuf = NULL;

  /* If the res_name window property maps to an icon, use that */
  pixbuf = gtk_icon_theme_load_icon_for_scale(theme, name, size, scale,
  ...
  for (i = 0; results[i]; i++) {
    for (j = 0; results[i][j]; j++) {
      /* We are only interested in the top ranking result, so we use that and
       * free up the rest */
      if (desktop_id == NULL) desktop_id = g_strdup(results[i][j]);
    }
    g_strfreev(results[i]);
  }
  g_free(results);

  if (desktop_id == NULL) return NULL;

  /* Now that we have the desktop file ID, we extract the icon from it and
   * render it */
  info = g_desktop_app_info_new(desktop_id);
  gicon = g_app_info_get_icon(G_APP_INFO(info));
  icon_info = gtk_icon_theme_lookup_by_gicon_for_scale(
      theme, gicon, size, scale, GTK_ICON_LOOKUP_FORCE_SIZE);
  if (icon_info) {
...
void meta_ui_theme_get_frame_borders(MetaUI *ui, MetaFrameType type,
  ...
  if (meta_ui_have_a_theme()) {
    ...
    if (!font_desc) {
      ...
      gtk_style_context_get(style, gtk_style_context_get_state(style), "font",
                            &free_font_desc, NULL);
    ...
    meta_theme_get_frame_borders(meta_theme_get_current(), type, text_height,
                                 flags, borders);

    if (free_font_desc) pango_font_description_free(free_font_desc);
  } else {
    meta_frame_borders_clear(borders);
  }

  if (style != NULL) g_object_unref(style);
}
...
gboolean meta_ui_parse_accelerator(const char *accel, unsigned int *keysym,
  ...
  *mask = 0;
  ...
  if (!accel[0] || strcmp(accel, "disabled") == 0) return TRUE;

  meta_ui_accelerator_parse(accel, &gdk_sym, &gdk_code, &gdk_mask);
  if (gdk_mask == 0 && gdk_sym == 0 && gdk_code == 0) return FALSE;

  if (gdk_sym == None && gdk_code == 0) return FALSE;
  ...
  *keycode = gdk_code;
  ...
  if (gdk_mask & GDK_SHIFT_MASK) *mask |= META_VIRTUAL_SHIFT_MASK;
  if (gdk_mask & GDK_CONTROL_MASK) *mask |= META_VIRTUAL_CONTROL_MASK;
  if (gdk_mask & GDK_MOD1_MASK) *mask |= META_VIRTUAL_ALT_MASK;
  if (gdk_mask & GDK_MOD2_MASK) *mask |= META_VIRTUAL_MOD2_MASK;
  if (gdk_mask & GDK_MOD3_MASK) *mask |= META_VIRTUAL_MOD3_MASK;
  if (gdk_mask & GDK_MOD4_MASK) *mask |= META_VIRTUAL_MOD4_MASK;
  if (gdk_mask & GDK_MOD5_MASK) *mask |= META_VIRTUAL_MOD5_MASK;
  if (gdk_mask & GDK_SUPER_MASK) *mask |= META_VIRTUAL_SUPER_MASK;
  if (gdk_mask & GDK_HYPER_MASK) *mask |= META_VIRTUAL_HYPER_MASK;
  if (gdk_mask & GDK_META_MASK) *mask |= META_VIRTUAL_META_MASK;
...
gchar *meta_ui_accelerator_name(unsigned int keysym, MetaVirtualModifier mask) {
  ...
  if (mask & META_VIRTUAL_SHIFT_MASK) mods |= GDK_SHIFT_MASK;
  if (mask & META_VIRTUAL_CONTROL_MASK) mods |= GDK_CONTROL_MASK;
  if (mask & META_VIRTUAL_ALT_MASK) mods |= GDK_MOD1_MASK;
  if (mask & META_VIRTUAL_MOD2_MASK) mods |= GDK_MOD2_MASK;
  if (mask & META_VIRTUAL_MOD3_MASK) mods |= GDK_MOD3_MASK;
  if (mask & META_VIRTUAL_MOD4_MASK) mods |= GDK_MOD4_MASK;
  if (mask & META_VIRTUAL_MOD5_MASK) mods |= GDK_MOD5_MASK;
  if (mask & META_VIRTUAL_SUPER_MASK) mods |= GDK_SUPER_MASK;
  if (mask & META_VIRTUAL_HYPER_MASK) mods |= GDK_HYPER_MASK;
  if (mask & META_VIRTUAL_META_MASK) mods |= GDK_META_MASK;
...
gboolean meta_ui_parse_modifier(const char *accel, MetaVirtualModifier *mask) {
  ...
  *mask = 0;
  ...
  if (accel == NULL || !accel[0] || strcmp(accel, "disabled") == 0) return TRUE;
