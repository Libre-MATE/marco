...
static void meta_preview_get_preferred_width(GtkWidget *widget, gint *minimal,
                                             gint *natural);
static void meta_preview_get_preferred_height(GtkWidget *widget, gint *minimal,
                                              gint *natural);
static void meta_preview_size_allocate(GtkWidget *widget,
                                       GtkAllocation *allocation);
static gboolean meta_preview_draw(GtkWidget *widget, cairo_t *cr);
...
static void meta_preview_class_init(MetaPreviewClass *class) {
...
static void meta_preview_init(MetaPreview *preview) {
  ...
  preview->flags = META_FRAME_ALLOWS_DELETE | META_FRAME_ALLOWS_MENU |
...
static void ensure_info(MetaPreview *preview) {
  ...
  if (preview->layout == NULL) {
    ...
    if (preview->theme)
      scale = meta_theme_get_title_scale(preview->theme, preview->type,
                                         preview->flags);
    else
      scale = 1.0;

    preview->layout = gtk_widget_create_pango_layout(widget, preview->title);

    font_desc = meta_gtk_widget_get_font_desc(widget, scale, NULL);

    preview->text_height = meta_pango_font_desc_get_text_height(
...
static gboolean meta_preview_draw(GtkWidget *widget, cairo_t *cr) {
  ...
  client_width = allocation.width - preview->borders.total.left -
                 preview->borders.total.right - border_width * 2;
  client_height = allocation.height - preview->borders.total.top -
                  preview->borders.total.bottom - border_width * 2;
  ...
  if (preview->theme) {
    meta_theme_draw_frame(
        preview->theme, gtk_widget_get_style_context(widget), cr, preview->type,
        preview->flags, client_width, client_height, preview->layout,
        preview->text_height, &preview->button_layout, button_states,
...
static void meta_preview_get_preferred_width(GtkWidget *widget, gint *minimum,
                                             gint *natural) {
  MetaPreview *preview;
  int border_width;
  GtkWidget *child;
...
static void meta_preview_get_preferred_height(GtkWidget *widget, gint *minimum,
                                              gint *natural) {
  MetaPreview *preview;
  int border_width;
  GtkWidget *child;
...
static void meta_preview_size_allocate(GtkWidget *widget,
  ...
  if (child && gtk_widget_get_visible(child)) {
    ...
    child_allocation.x =
        widget_allocation.x + border_width + preview->borders.total.left;
    child_allocation.y =
        widget_allocation.y + border_width + preview->borders.total.top;

    child_allocation.width =
        MAX(1, widget_allocation.width - border_width * 2 -
                   preview->borders.total.left - preview->borders.total.right);
    child_allocation.height =
        MAX(1, widget_allocation.height - border_width * 2 -
                   preview->borders.total.top - preview->borders.total.bottom);
...
GdkPixbuf *meta_preview_get_icon(void) {
  ...
  if (default_icon == NULL) {
    ...
    if (icon_exists)
      default_icon = gtk_icon_theme_load_icon(theme, META_DEFAULT_ICON_NAME,
                                              META_DEFAULT_ICON_SIZE, 0, NULL);
    else
      default_icon = gtk_icon_theme_load_icon(theme, "image-missing",
                                              META_DEFAULT_ICON_SIZE, 0, NULL);
...
cairo_region_t *meta_preview_get_clip_region(MetaPreview *preview,
  ...
  if (preview->theme == NULL) return window_xregion;

  /* Otherwise, we do have a theme, so calculate the corners */
  frame_style =
      meta_theme_get_frame_style(preview->theme, META_FRAME_TYPE_NORMAL, flags);
