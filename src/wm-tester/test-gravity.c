...
static int gravities[10] = {NorthWestGravity, NorthGravity,  NorthEastGravity,
...
static void calculate_position(int i, int is_doubled, int *x, int *y) {
  if (i == 9) {
    *x = 150;
    *y = 150;
  } else {
...
int main(int argc, char **argv) {
  ...
  for (i = 0; i < 10; i++) {
    ...
    w = XCreateSimpleWindow(d, RootWindow(d, screen), x, y, WINDOW_WIDTH,
                            WINDOW_HEIGHT, 0, WhitePixel(d, screen),
                            WhitePixel(d, screen));
    ...
    if (noframes) {
      ...
      XChangeProperty(d, w, mwm_atom, mwm_atom, 32, PropModeReplace,
                      (unsigned char *)&mwm,
  ...
  while (1) {
    ...
    if (ev.xany.type == ConfigureNotify) {
      ...
      if (i >= 0) {
        ...
        XClearArea(d, windows[i], 0, 0, ev.xconfigure.width,
                   ev.xconfigure.height, True);

        if (!ev.xconfigure.send_event)
          XTranslateCoordinates(d, windows[i], DefaultRootWindow(d), 0, 0,
      ...
      if (i >= 0) {
        ...
        gc = XCreateGC(d, windows[i], GCForeground, &values);

        sprintf(buf, "%d,%d", window_rects[i].x, window_rects[i].y);

        XDrawString(d, windows[i], gc, 10, 15, buf, strlen(buf));

        sprintf(buf, "%dx%d", window_rects[i].width, window_rects[i].height);

        XDrawString(d, windows[i], gc, 10, 35, buf, strlen(buf));
      ...
      if (i >= 0) {
        ...
        if (ev.xbutton.button == Button1) {
          ...
          if (doubled[i])
            ...
            XMoveResizeWindow(d, windows[i], x, y, WINDOW_WIDTH * 2,
                              WINDOW_HEIGHT * 2);
