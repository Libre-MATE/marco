...
int main(int argc, char **argv) {
  ...
  if (!(do_evil || do_icon_windows)) return 1;

  if (do_evil) set_up_the_evil();

  if (do_icon_windows) set_up_icon_windows();
...
static gint evil_timeout(gpointer data) {
  ...
  while (i < n_windows) {
    ...
    gtk_window_move(
        GTK_WINDOW(w),
        g_random_int_range(0, WidthOfScreen(gdk_x11_screen_get_xscreen(
                                  gdk_screen_get_default()))),
        g_random_int_range(0, HeightOfScreen(gdk_x11_screen_get_xscreen(
                                  gdk_screen_get_default()))));
  ...
  if (len > destroy_count) {
    ...
    while (i < n_windows) {
      ...
      w = g_slist_nth_data(evil_windows, g_random_int_range(0, len));
      if (w) {
        --len;
...
static void set_up_the_evil(void) { g_timeout_add(400, evil_timeout, NULL); }
...
static void set_up_icon_windows(void) {
  ...
  while (i < n_windows) {
    ...
    pix = gtk_icon_theme_load_icon(gtk_icon_theme_get_default(), "gtk-save",
                                   size, 0, NULL);
